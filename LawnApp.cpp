...
/*void LawnApp::DoNewModOptions(bool theFromGameSelector)
{
	//FinishModelessDialogs();

	NewOptionsDialog* aDialog = new NewOptionsDialog(this, theFromGameSelector);
	CenterDialog(aDialog, IMAGE_OPTIONS_MENUBACK->mWidth, IMAGE_OPTIONS_MENUBACK->mHeight);
	AddDialog(Dialogs::DIALOG_NEWMODOPTIONS, aDialog);
	mWidgetManager->SetFocus(aDialog);
}*/
...
bool LawnApp::KillNewOptionsDialog()
{
	...
	bool wantHardmode = aNewOptionsDialog->mHardModeCheckbox->IsChecked();
	bool wantCoop = aNewOptionsDialog->mCoopCheckbox->IsChecked();
	SwitchScreenMode(wantWindowed, want3D, wantHardmode, wantCoop, false);
...
/*bool LawnApp::KillNewModOptionsDialog()
{
	NewOptionsDialog* aNewOptionsDialog = (NewOptionsDialog*)GetDialog(Dialogs::DIALOG_NEWMODOPTIONS);
	if (aNewOptionsDialog == nullptr)
		return false;
	...
	bool want3D = aNewOptionsDialog->mHardwareAccelerationCheckbox->IsChecked();
	bool wantHardmode = aNewOptionsDialog->mHardModeCheckbox->IsChecked();
	SwitchScreenMode(wantWindowed, want3D, wantHardmode, false);

	KillDialog(Dialogs::DIALOG_NEWMODOPTIONS);
	ClearUpdateBacklog();
	return true;
}*/
...
void LawnApp::Init()
{
	...
	ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_TITLESCREEN, true);
...
bool LawnApp::UpdatePlayerProfileForFinishingLevel()
{
	...
	if (IsAdventureMode())
	{
		...
		if (!HasFinishedAdventure() && (mBoard->mLevel == 34 || mBoard->mLevel == 54))
		{
	...
	{
		aUnlockedNewChallenge = !HasBeatenChallenge(mGameMode);
		if (!mPlayerInfo->mHardmodeIsOff)
		{
			mPlayerInfo->mChallengeHardmodeRecords[GetCurrentChallengeIndex()]++;
		}
		else
		{
	...
	{
		aUnlockedNewChallenge = (!mPlayerInfo->mHardmodeIsOff ? !HasBeatenChallengeHardmode(mGameMode) : !HasBeatenChallenge(mGameMode));
		if (!mPlayerInfo->mHardmodeIsOff)
		{
			mPlayerInfo->mChallengeHardmodeRecords[GetCurrentChallengeIndex()]++;
		}
...
void LawnApp::CheckForGameEnd()
{
	...
	if (IsAdventureMode())
	{
		...
		if (IsFirstTimeAdventureMode() && aLevel < 70)
		{
	...
	else if (IsVersusLevel())
	{
		...
		{
			ShowChallengeScreen(ChallengePage::CHALLENGE_PAGE_VERSUS);
		}
	}
	else if (IsMinigamePage2())
	{
		KillBoard();
...
void LawnApp::ButtonDepress(int theId)
{
	if (theId % 10000 >= 2000 && theId % 10000 < 3000)  // 按钮编号 theId ∈ [2000, 3000) 时，表示按下 theId - 2000 编号的对话中的“是”按钮
	{
		...
		/*case Dialogs::DIALOG_NEWMODOPTIONS:
			KillNewModOptionsDialog();
			return;*/
		...
		case Dialogs::DIALOG_SKIPCONFIRM:
			KillDialog(Dialogs::DIALOG_SKIPCONFIRM);
			if (mBoard->mLevel == 49)
				mPlayerInfo->SetLevel(61);
			else mPlayerInfo->SetLevel(49);
			mPlayerInfo->AddCoins(10000);
			mPlayerInfo->mHasUnlockedMinigames = true;
			mPlayerInfo->mHasUnlockedPuzzleMode = true;
			for (int i = 1; i < (int)NUM_GAME_MODES - 47; i++)
			{
				if (i != (int)GameMode::GAMEMODE_TREE_OF_WISDOM && i != (int)GameMode::GAMEMODE_SCARY_POTTER_ENDLESS && i != (int)GameMode::GAMEMODE_SURVIVAL_NORMAL_STAGE_6 && i != (int)GameMode::GAMEMODE_SURVIVAL_NORMAL_STAGE_7 && i != (int)GameMode::GAMEMODE_SURVIVAL_HARD_STAGE_6 && i != (int)GameMode::GAMEMODE_SURVIVAL_HARD_STAGE_7 &&
					i != (int)GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_ENDLESS && i != (int)GameMode::GAMEMODE_SURVIVAL_ENDLESS_STAGE_3 && i != (int)GameMode::GAMEMODE_SURVIVAL_ENDLESS_STAGE_6 && i != (int)GameMode::GAMEMODE_SURVIVAL_ENDLESS_STAGE_7 && i != (int)GameMode::GAMEMODE_CHALLENGE_ICE)
...
{
	return mGameMode >= GameMode::GAMEMODE_SURVIVAL_NORMAL_STAGE_1 && mGameMode <= GameMode::GAMEMODE_SURVIVAL_ENDLESS_STAGE_7;
}
...
bool LawnApp::IsPuzzleMode2()
{
	return
		(mGameMode >= GameMode::GAMEMODE_SCARY_POTTER_11 && mGameMode <= GameMode::GAMEMODE_SCARY_POTTER_20)
		|| (mGameMode >= GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_11 && mGameMode <= GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_15);
}

bool LawnApp::IsMinigamePage2()
{
	return
		(mGameMode >= GameMode::GAMEMODE_CHALLENGE_ART_CHALLENGE_WALLNUT && mGameMode <= GameMode::GAMEMODE_CHALLENGE_SQUIRREL) ||
		(mGameMode >= GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS_3 && mGameMode <= GameMode::GAMEMODE_CHALLENGE_BOMBALL);
}
...
bool LawnApp::IsArtChallenge()
{
	...
	return 
		...
		mGameMode == GameMode::GAMEMODE_CHALLENGE_SEASTAR;
}
...
bool LawnApp::IsIZombieLevel()
{
	...
	return
		...
		mGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_11 ||
...
bool LawnApp::IsWallnutBowlingLevel()
{
	...
	if (mGameMode == GameMode::GAMEMODE_CHALLENGE_WALLNUT_BOWLING || mGameMode == GameMode::GAMEMODE_CHALLENGE_WALLNUT_BOWLING_2 || mGameMode == GameMode::GAMEMODE_CHALLENGE_WALLNUT_BOWLING_3)
		return true;
...
bool LawnApp::IsDarkLevel()
{
	return (mBoard && ((mGameMode == GameMode::GAMEMODE_ADVENTURE && mPlayerInfo->mLevel == 65) || mGameMode == GAMEMODE_CHALLENGE_DARK));
}

bool LawnApp::IsVersusLevel()
{
	return (mBoard && (mGameMode == GAMEMODE_CHALLENGE_VERSUS || mGameMode == GAMEMODE_CHALLENGE_VERSUS_FOREST || mGameMode == GAMEMODE_CHALLENGE_VERSUS_POOL || mGameMode == GAMEMODE_CHALLENGE_VERSUS_NIGHT || mGameMode == GAMEMODE_CHALLENGE_VERSUS_FOG || mGameMode == GAMEMODE_CHALLENGE_VERSUS_ROOF || mGameMode == GAMEMODE_CHALLENGE_VERSUS_MOON || mGameMode == GAMEMODE_CHALLENGE_VERSUS_OLD));
}
...
bool LawnApp::IsScaryPotterLevel()
{
	if (mGameMode >= GameMode::GAMEMODE_SCARY_POTTER_1 && mGameMode <= GameMode::GAMEMODE_SCARY_POTTER_ENDLESS)
		return true;
	else if (mGameMode >= GameMode::GAMEMODE_SCARY_POTTER_11 && mGameMode <= GameMode::GAMEMODE_SCARY_POTTER_20)
		return true;
...
bool LawnApp::IsMiniBossLevel()
{
	...
	return
		...
		(IsAdventureMode() && mPlayerInfo->mLevel == 70);
}
...
SeedType LawnApp::GetAwardSeedForLevel(int theLevel)
{
	...
	if (aSeedsHasGot > SEED_GATLINGPEA)
	{
		aSeedsHasGot = SEED_GATLINGPEA;
	}
...
bool LawnApp::HasSeedType(SeedType theSeedType)
{
	...
	if (theSeedType == SeedType::SEED_ICYFUME)
	{
		return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_ICYFUME] > 0;
	}
	if (theSeedType == SeedType::SEED_SUPERCHOMP)
	{
		return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_SUPERCHOMP] > 0;
	}
	if (theSeedType == SeedType::SEED_FLAMEWOOD)
	{
		return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_FLAMEWOOD] > 0;
	}
	if (theSeedType == SeedType::SEED_QUEENPEA)
	{
		return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_ICEQUEEN] > 0;
	}
	if (theSeedType == SeedType::SEED_VINENUT)
	{
		return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_VINENUT] > 0;
	}
	if (theSeedType == SeedType::SEED_THORNMOSS)
	{
		return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_THORNMOSS] > 0;
	}
	if (theSeedType == SeedType::SEED_BRAVESHROOM)
	{
		return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_BRAVESHROOM] > 0;
	}
	if (theSeedType == SeedType::SEED_ECACTUS)
	{
		return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_ECACTUS] > 0;
	}
	if (theSeedType == SeedType::SEED_FLAMEPEA)
	{
		return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_FIRE_PEASHOOTER] > 0;
	}
	if (theSeedType == SeedType::SEED_PEPPER)
	{
		return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_PEPPERPULT] > 0;
	}
	...
	{
		return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_PLASMAPEA] > 0;
	}
	if (theSeedType == SeedType::SEED_SHOOTINGSTAR)
	{
		return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_SHOOTINGSTAR] > 0;
	}
...
{
	if ((mGameMode == GAMEMODE_CHALLENGE_VERSUS_OLD && ((theSeedType > SEED_MELONPULT && theSeedType < SEED_VERSUS_NORMAL) || theSeedType > SEED_VERSUS_GARGANTUAR || theSeedType == SEED_VERSUS_BALLOON) && theSeedType != SEED_VERSUS_TRASHCAN) && mBoard) return false;
...
bool LawnApp::CanSpawnYetis()
{
	...
	return (HasFinishedAdventure() && (mPlayerInfo->mFinishedAdventure >= 2 || mPlayerInfo->mLevel >= aZombieDef.mStartingLevel)) || mGameMode == GAMEMODE_CHALLENGE_ICE;
}
...
bool LawnApp::HasBeatenChallengeHardmode(GameMode theGameMode)
{
	if (mPlayerInfo == nullptr)
		return false;
	...
	if (IsSurvivalNormal(theGameMode))
	{
		return mPlayerInfo->mChallengeHardmodeRecords[aChallengeIndex] >= SURVIVAL_NORMAL_FLAGS;
	}
	if (IsSurvivalHard(theGameMode))
	{
		return mPlayerInfo->mChallengeHardmodeRecords[aChallengeIndex] >= SURVIVAL_HARD_FLAGS;
	}
	if (IsSurvivalEndless(theGameMode) || IsEndlessScaryPotter(theGameMode) || IsEndlessIZombie(theGameMode))
	{
		return false;
	}
	return mPlayerInfo->mChallengeHardmodeRecords[aChallengeIndex] > 0;
}
...
void LawnApp::CrazyDaveEnter()
{
	...
	if (mBoard && mBoard->mLevel > 60 && mBoard->mLevel <= 70)
	{
		aReanimType = ReanimationType::REANIM_BRAVE_DAVE;
	}
	else
	{
		aReanimType = ReanimationType::REANIM_CRAZY_DAVE;
	}
	Reanimation* aCrazyDaveReanim = AddReanimation(0.0f, 0.0f, 0, aReanimType);
	...
	if (mCrazyDaveState != CrazyDaveState::CRAZY_DAVE_TALKING || doSound)
	{
		if (doHanding)
		{
			...
			if (doSound)
			{
				...
				{
					if (mBoard && mBoard->mLevel > 60 && mBoard->mLevel <= 70) PlayFoley(FoleyType::FOLEY_BRAVE_DAVE_1);
					else PlayFoley(FoleyType::FOLEY_CRAZY_DAVE_SHORT);
				}
				else if (theMessage.find(_S("{SCREAM}")) != SexyString::npos)
				{
					if (mBoard && mBoard->mLevel > 60 && mBoard->mLevel <= 70) PlayFoley(FoleyType::FOLEY_BRAVE_DAVE_2);
					else PlayFoley(FoleyType::FOLEY_CRAZY_DAVE_SCREAM);
				}
				else
				{
					if (mBoard && mBoard->mLevel > 60 && mBoard->mLevel <= 70) PlayFoley(FoleyType::FOLEY_BRAVE_DAVE_3);
					else PlayFoley(FoleyType::FOLEY_CRAZY_DAVE_LONG);
				}
			}
		...
		else if (theMessage.find(_S("{SHAKE}")) != SexyString::npos)
		{
			...
			if (doSound)
			{
				if (mBoard && mBoard->mLevel > 60 && mBoard->mLevel <= 70) PlayFoley(FoleyType::FOLEY_BRAVE_DAVE_5);
				else PlayFoley(FoleyType::FOLEY_CRAZY_DAVE_CRAZY);
			}
		...
		else if (theMessage.find(_S("{SCREAM2}")) != SexyString::npos)
		{
			...
			if (doSound)
			{
				if (mBoard && mBoard->mLevel > 60 && mBoard->mLevel <= 70) PlayFoley(FoleyType::FOLEY_BRAVE_DAVE_4);
				else PlayFoley(FoleyType::FOLEY_CRAZY_DAVE_SCREAM_2);
			}
		...
		else
		{
			...
			else
			{
				...
				if (doSound)
				{
					...
					if (mBoard && mBoard->mLevel > 60 && mBoard->mLevel <= 70) PlayFoley(FoleyType::FOLEY_BRAVE_DAVE_6);
					else PlayFoley(FoleyType::FOLEY_CRAZY_DAVE_EXTRA_LONG);
				}
...
void LawnApp::UpdateCrazyDave()
{
		...
		mCrazyDaveState == CrazyDaveState::CRAZY_DAVE_HANDING_TALKING || mCrazyDaveState == CrazyDaveState::CRAZY_DAVE_HANDING_IDLING)
	{
		...
		if (mCrazyDaveBlinkCounter <= 0)
		{
			...
			Reanimation* aBlinkReanim = AddReanimation(0.0f, 0.0f, 0, aReanimType);
...
int LawnApp::GetNumPreloadingTasks()
{
	...
	if (mPlayerInfo)
	{
		...
		for (ZombieType i = ZombieType::ZOMBIE_BASIC; i < ZombieType::NUM_ZOMBIE_TYPES;i = (ZombieType)((int)i + 1))
		{
...
void LawnApp::PreloadForUser()
{
	...
	ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_READY_SET_FIGHT, true);
	ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_READY_SET_ZOMBIE, true);
	...
	ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_SUN_BRAIN, true);
	...
	if (mPlayerInfo)
	{
		...
		for (ZombieType i = ZombieType::ZOMBIE_BASIC; i < ZombieType::NUM_ZOMBIE_TYPES;i = (ZombieType)((int)i + 1))
		{
			...
			if (i == ZombieType::ZOMBIE_BOSS || i == ZombieType::ZOMBIE_CATAPULT || i == ZombieType::ZOMBIE_GARGANTUAR ||
				i == ZombieType::ZOMBIE_DIGGER || i == ZombieType::ZOMBIE_ZOMBONI)
			{
				continue;
			}
...
int LawnApp::GetNumTrophiesHardmode(ChallengePage thePage)
{
	int aNumTrophies = 0;
	...
	{
		const ChallengeDefinition& aDef = GetChallengeDefinition(i);
		if (aDef.mPage == thePage && HasBeatenChallengeHardmode(aDef.mChallengeMode))
		{
			aNumTrophies++;
		}
	}
...
{
	return  50 + 33 + 14 - GetNumTrophiesHardmode(CHALLENGE_PAGE_SURVIVAL) - GetNumTrophiesHardmode(CHALLENGE_PAGE_CHALLENGE) - GetNumTrophiesHardmode(CHALLENGE_PAGE_PUZZLE);
}
...
SexyString LawnGetCurrentLevelName()
{
	...
	{
		return StrFormat(_S("F%d"), gLawnApp->GetStageString(gLawnApp->mBoard->mLevel-10).c_str());
	}
...
//0x4560E0
void LawnApp::SwitchScreenMode(bool wantWindowed, bool is3d, bool wantHardmode, bool wantCoop, bool force)
{
	if (wantHardmode) mPlayerInfo->mHardmodeIsOff = false;
	else if (!wantHardmode) mPlayerInfo->mHardmodeIsOff = true;
	if (wantCoop) mPlayerInfo->mIsNotCoop = false;
	else if (!wantCoop) mPlayerInfo->mIsNotCoop = true;
