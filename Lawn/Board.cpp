...
Board::Board(LawnApp* theApp)
{
	...
	mCobActivated = false;
	mRageDelay = 0;
	if (mApp->IsDarkLevel()) mDarknessImage = (Image*)mApp->mReanimatorCache->MakeBlankMemoryImage(3000, 3000);
	//if (mApp->mGameMode == GAMEMODE_CHALLENGE_VERSUS) mPreviewImage = (Image*)mApp->mReanimatorCache->MakeBlankMemoryImage(BOARD_WIDTH, BOARD_HEIGHT);
	if (mApp->IsDarkLevel()) mDarknessDarkerImage = (Image*)mApp->mReanimatorCache->MakeBlankMemoryImage(3000, 3000);
	mBoardAge = 0;
	mGridXCob = 0;
	mGridYCob = 0;
	...
	mPlayer2 = new PlayerController();
	mPlayer2->mX = GridToPixelX(gridNumber, 0) + 33; //33 is kinda to center the arrow lel
	mPlayer2->mY = GridToPixelY(gridNumber, 0);
	mPlayer2->mKeyUp = 'w';
	mPlayer2->mKeyLeft = 'a';
	mPlayer2->mKeyDown = 's';
	mPlayer2->mKeyRight = 'd';
	mPlayer2->mKeyLB = 'q';
	mPlayer2->mKeyRB = 'e';
	mPlayer2->mKeyA = 'j';
	mPlayer2->mKeyB = 'k';
	mPlayer2->mKeyX = 'b';
	mPlayer2->mControllerSpeed = 5;
	mPlayer2->mPlayerID = 0; //We forgot Samen //forgot what
	mPlayer2->mAbilityType = ABILITY_BUTTER;
	mPlayer2->mControllerType = CONTROLLER_PLANT;
	mPlayer2->mSeedBankIndex = 0;

	//Lets try to add coop first. Once you got the plant viewing done, then we can add the zombie preview and zombie planting
	//sounds good
	
// for me I did this shinanigans
	//what
	//uh so only player 2 is keyboard? how about player 1?
	//I wanted to add both players in eventually 
	//just wanted to focus on player2 to see if everything even works
	...
	mBloodCounterCooldown = 0;
	mBombAllCounter = 25;
	spawnSpeed = false;
	mSpawnBombAll = false;
	mTargetCounter = 0;
	mDummyCounter = 0;
	mDummyCounterEnemy = 0;
	...
	if (!mApp->IsVersusLevel()) mMenuButton->mDrawStoneButton = true;
	...
	{
		if (!mApp->IsVersusLevel()) mMenuButton->SetLabel(_S("[MENU_BUTTON]"));
...
Board::~Board()
{
	...
	{
		delete mMenuButton;
		spawnSpeed = NULL; //reset speedbutton
		mSpeedIsOn = false;
		BloodMoonIsOn = false;
	}
	if (mStoreButton)
...
GridItem* Board::GetWoodLogAt(int theGridX, int theGridY)
{
	return GetGridItemAt(GridItemType::GRIDITEM_WOOD_LOG, theGridX, theGridY);
}

GridItem* Board::GetBushAt(int theGridX, int theGridY)
{
	return GetGridItemAt(GridItemType::GRIDITEM_BUSH, theGridX, theGridY);
}
	...
	while (IterateGridItems(aGridItem))
	{
		if (aGridItem->mGridX == theGridX && aGridItem->mGridY == theGridY)
		{
			if (aGridItem->mGridItemType == GridItemType::GRIDITEM_GRAVESTONE || 
				...
				aGridItem->mGridItemType == GridItemType::GRIDITEM_WOOD_LOG || 
				aGridItem->mGridItemType == GridItemType::GRIDITEM_BUSH || 
				aGridItem->mGridItemType == GridItemType::GRIDITEM_LADDER)
				return false;
		}
	}
	if (GetTopPlantAt(theGridX, theGridY, TOPPLANT_ANY)) return false;
	return true;
}
...
GridItem* Board::AddAWoodLog(int theGridX, int theGridY)
{
	GridItem* aWoodLog = mGridItems.DataArrayAlloc();
	aWoodLog->mGridItemType = GridItemType::GRIDITEM_WOOD_LOG;
	aWoodLog->mRenderOrder = MakeRenderOrder(RenderLayer::RENDER_LAYER_GRAVE_STONE, theGridY, 3);
	aWoodLog->mGridX = theGridX;
	aWoodLog->mGridY = theGridY;
	aWoodLog->mGridItemRect = Rect(0, 0, 40, 80);
	return aWoodLog;
}

GridItem* Board::AddABush(int theGridX, int theGridY)
{
	GridItem* aBush = mGridItems.DataArrayAlloc();
	aBush->mGridItemType = GridItemType::GRIDITEM_BUSH;
	aBush->mGridItemCounter = -Rand(50);
	aBush->mRenderOrder = MakeRenderOrder(RenderLayer::RENDER_LAYER_ZOMBIE, theGridY, 9);
	aBush->mGridX = theGridX;
	aBush->mGridY = theGridY;
...
void Board::AddBushes(int theGridX, int theCount, MTRand& theLevelRNG)
{
	TOD_ASSERT(theCount <= MAX_GRID_SIZE_Y);
	...
	while (i < theCount)
	{
		...
		if (CanAddGraveStoneAt(theGridX, aGridY))
		{
			GridItem* aGraveStone = AddABush(theGridX, aGridY);
			++i;
		}
	}
}
...
{
	for (ZombieType aZombieType = ZombieType::ZOMBIE_BASIC; aZombieType < ZombieType::NUM_ZOMBIE_TYPES; aZombieType = (ZombieType)((int)aZombieType + 1))
	{
...
void Board::PickZombieWaves()
{
	...
	if (mApp->IsAdventureMode())
	{
		...
		{
			mNumWaves = gZombieWaves[ClampInt(mLevel - 1, 0,69)];
	...
	else
	{
		...
		if (mApp->IsSurvivalMode() || aGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND)
			...
			mNumWaves = 0;
		else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_GLADIANTUAR || aGameMode == GameMode::GAMEMODE_CHALLENGE_SNOWPEA)
			mNumWaves = 1;
	...
	for (int aWave = 0; aWave < mNumWaves; aWave++)
	{
		...
		if (aIsFlagWave)
		{
				...
				mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS_3 && mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS_4 && mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_SMASH && mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_GIGA && mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_GLITCH)
			{
				...
				{
					PutZombieInWave(ZombieType::ZOMBIE_BASIC, aWave, &aZombiePicker);
				}
		...
		else if (mApp->IsShovelLevel() || mApp->IsBungeeBlitzLevel() || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_PORTAL_COMBAT || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WOOD ||
			  	 mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_INVISIGHOUL || mApp->mGameMode == GAMEMODE_CHALLENGE_BOMBALL)
		{
			aZombiePoints *= 2;
		}
		...
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_GLADIANTUAR)
		{
			PutZombieInWave(ZombieType::ZOMBIE_GLADIANTUAR_GIGA, aWave, &aZombiePicker);
		}
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_SNOWPEA)
		{
			PutZombieInWave(ZombieType::ZOMBIE_SNOWPEA_BOSS, aWave, &aZombiePicker);
		}
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES && aIsFinalWave)
		{
			PutZombieInWave(ZombieType::ZOMBIE_GARGANTUAR, aWave, &aZombiePicker);
		}
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES && aWave == 10)
		{
			PutZombieInWave(ZombieType::ZOMBIE_DANCER, aWave, &aZombiePicker);
		}
		if (mApp->IsBigTroubleLevel() && aWave == 10)
		{
			PutZombieInWave(ZombieType::ZOMBIE_FOOTBALL, aWave, &aZombiePicker);
		}
		if (mApp->IsDarkLevel() && aWave == 14)
		{
			PutZombieInWave(ZombieType::ZOMBIE_BIGFOOT, aWave, &aZombiePicker);
		}
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_SEASTAR && aWave == 9)
		{
			PutZombieInWave(ZombieType::ZOMBIE_FOOTBALL, aWave, &aZombiePicker);
		}
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_SEASTAR && aWave == 19)
		{
			PutZombieInWave(ZombieType::ZOMBIE_GIGA_FOOTBALL, aWave, &aZombiePicker);
		}
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES && (aWave == 15 || aIsFinalWave))
		{
			PutZombieInWave(ZombieType::ZOMBIE_FOOTBALL, aWave, &aZombiePicker);
		}
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_ICE && aIsFinalWave)
		{
			PutZombieInWave(ZombieType::ZOMBIE_YETI, aWave, &aZombiePicker);
		}
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_ICE && (aWave == 15 || aIsFinalWave))
		{
			PutZombieInWave(ZombieType::ZOMBIE_ZOMBONI, aWave, &aZombiePicker);
			PutZombieInWave(ZombieType::ZOMBIE_ICE, aWave, &aZombiePicker);
		}
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_FLIPPED && (aIsFinalWave))
		{
			...
			PutZombieInWave(ZombieType::ZOMBIE_POLEVAULTER, aWave, &aZombiePicker);
		}
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_GIGA && (aIsFinalWave))
		{
			PutZombieInWave(ZombieType::ZOMBIE_GIGA_FOOTBALL, aWave, &aZombiePicker);
			PutZombieInWave(ZombieType::ZOMBIE_GIGA_POLEVAULTER, aWave, &aZombiePicker);
			PutZombieInWave(ZombieType::ZOMBIE_REDEYE_GARGANTUAR, aWave, &aZombiePicker);
		}
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_BORDER && (aIsFinalWave))
		{
			PutZombieInWave(ZombieType::ZOMBIE_DIGGER, aWave, &aZombiePicker);
			PutZombieInWave(ZombieType::ZOMBIE_BALLOON, aWave, &aZombiePicker);
			PutZombieInWave(ZombieType::ZOMBIE_GARGANTUAR, aWave, &aZombiePicker);
		}
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_HIGHGROUND && (aIsFinalWave))
		{
			PutZombieInWave(ZombieType::ZOMBIE_JACK_IN_THE_BOX, aWave, &aZombiePicker);
			PutZombieInWave(ZombieType::ZOMBIE_LUMBERJACK, aWave, &aZombiePicker);
			PutZombieInWave(ZombieType::ZOMBIE_SCARECROW, aWave, &aZombiePicker);
		}
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_ECLIPSE && (aIsFinalWave))
		{
			PutZombieInWave(ZombieType::ZOMBIE_BUNGEE, aWave, &aZombiePicker);
			PutZombieInWave(ZombieType::ZOMBIE_CATAPULT, aWave, &aZombiePicker);
			PutZombieInWave(ZombieType::ZOMBIE_FOOTBALL, aWave, &aZombiePicker);
			PutZombieInWave(ZombieType::ZOMBIE_GIGA_FOOTBALL, aWave, &aZombiePicker);
		}
...
void Board::PickBackground()
{
	...
	case GameMode::GAMEMODE_ADVENTURE:
		...
		{
			mBackground = BackgroundType::BACKGROUND_7_FOREST;
		}
		else
		{
			mBackground = BackgroundType::BACKGROUND_6_BOSS;
	...
	case GameMode::GAMEMODE_CHALLENGE_SMASH:
	case GameMode::GAMEMODE_CHALLENGE_BORDER:
	case GameMode::GAMEMODE_CHALLENGE_FLIPPED:
	case GameMode::GAMEMODE_CHALLENGE_PLANTS_VS_PLANTS:
	case GameMode::GAMEMODE_CHALLENGE_VERSUS_OLD:
		mBackground = BackgroundType::BACKGROUND_1_DAY;
		break;
	...
	case GameMode::GAMEMODE_SCARY_POTTER_15:
	case GameMode::GAMEMODE_SCARY_POTTER_16:
	case GameMode::GAMEMODE_SCARY_POTTER_17:
	case GameMode::GAMEMODE_SCARY_POTTER_18:
	...
	case GameMode::GAMEMODE_CHALLENGE_GIGA:
	case GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS_5:
	case GameMode::GAMEMODE_CHALLENGE_VERSUS_POOL:
		mBackground = BackgroundType::BACKGROUND_3_POOL;
		break;
	...
	case GameMode::GAMEMODE_SCARY_POTTER_19:
	case GameMode::GAMEMODE_SCARY_POTTER_20:
	case GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_14:
	case GameMode::GAMEMODE_CHALLENGE_VERSUS_FOG:
		mBackground = BackgroundType::BACKGROUND_4_FOG;
		break;
	...
	case GameMode::GAMEMODE_CHALLENGE_VERSUS_ROOF:
		mBackground = BackgroundType::BACKGROUND_5_ROOF;
		break;
	...
	case GameMode::GAMEMODE_CHALLENGE_GLITCH:
	case GameMode::GAMEMODE_CHALLENGE_DARK:
	case GameMode::GAMEMODE_CHALLENGE_WOOD:
	case GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS_3:
	case GameMode::GAMEMODE_CHALLENGE_VERSUS_FOREST:
		mBackground = BackgroundType::BACKGROUND_7_FOREST;
		break;
	...
	case GameMode::GAMEMODE_CHALLENGE_SEASTAR:
		mBackground = BackgroundType::BACKGROUND_POOL_PARTY;
		break;
	...
	else if (mBackground == BackgroundType::BACKGROUND_POOL_PARTY)
	{
	...
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_GLADIANTUAR)
	{
		...
		mGridSquareType[5][0] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[6][0] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[7][0] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[8][0] = GridSquareType::GRIDSQUARE_NONE;
		...
		mGridSquareType[5][1] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[6][1] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[7][1] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[8][1] = GridSquareType::GRIDSQUARE_NONE;

		mGridSquareType[0][2] = GridSquareType::GRIDSQUARE_ROOF;
		mGridSquareType[1][2] = GridSquareType::GRIDSQUARE_ROOF;
		mGridSquareType[2][2] = GridSquareType::GRIDSQUARE_ROOF;
		mGridSquareType[3][2] = GridSquareType::GRIDSQUARE_ROOF;
		mGridSquareType[4][2] = GridSquareType::GRIDSQUARE_ROOF;
		mGridSquareType[5][2] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[6][2] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[7][2] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[8][2] = GridSquareType::GRIDSQUARE_NONE;
		...
		mGridSquareType[5][3] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[6][3] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[7][3] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[8][3] = GridSquareType::GRIDSQUARE_NONE;
		...
		mGridSquareType[5][4] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[6][4] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[7][4] = GridSquareType::GRIDSQUARE_NONE;
		mGridSquareType[8][4] = GridSquareType::GRIDSQUARE_NONE;
	}
	else
	{
	...
	if (mBackground == BackgroundType::BACKGROUND_HIGHGROUND)
	{
	...
	if (StageHasGraveStones())
	{
		...
		else if (mBackground == BackgroundType::BACKGROUND_2_NIGHT && mApp->mGameMode != GAMEMODE_CHALLENGE_GLADIANTUAR)
		{
			if (mApp->mGameMode == GAMEMODE_CHALLENGE_VERSUS_NIGHT)
			{
				AddAGraveStone(5, 0);
				AddAGraveStone(5, 1);
				AddAGraveStone(5, 2);
		...
		else if (mBackground == BackgroundType::BACKGROUND_7_FOREST)
		{
			if (mApp->mGameMode == GAMEMODE_CHALLENGE_WOOD)
				return;

			if (mApp->mGameMode == GAMEMODE_CHALLENGE_VERSUS_FOREST)
			{
				AddABush(5, 0);
				AddABush(5, 1);
				AddABush(5, 2);
			...
			{
				AddBushes(4, 1, aLevelRNG);
				AddBushes(5, 1, aLevelRNG);
				AddBushes(6, 2, aLevelRNG);
				AddBushes(7, 2, aLevelRNG);
				AddBushes(8, 3, aLevelRNG);
			}
			else if (mLevel == 61 || mLevel == 62 || mLevel == 63)
			{
				AddBushes(6, 1, aLevelRNG);
				AddBushes(7, 1, aLevelRNG);
				AddBushes(8, 2, aLevelRNG);
			}
			...
			{
				AddBushes(4, 1, aLevelRNG);
				AddBushes(5, 2, aLevelRNG);
				AddBushes(6, 2, aLevelRNG);
				AddBushes(7, 3, aLevelRNG);
				AddBushes(8, 3, aLevelRNG);
			}
			else if (mLevel >= 70)
			{
				AddBushes(3, 1, aLevelRNG);
				AddBushes(4, 2, aLevelRNG);
				AddBushes(5, 2, aLevelRNG);
				AddBushes(6, 2, aLevelRNG);
				AddBushes(7, 3, aLevelRNG);
				AddBushes(8, 3, aLevelRNG);
...
void Board::InitSurvivalStage()
{
	...
	mP2SeedBank->UpdateWidth();

	for (int i = 0; i < SEEDBANK_MAX; i++)
	{
		SeedPacket* aPacket = &mSeedBank->mSeedPackets[i];
		aPacket->mX = GetSeedPacketPositionX(i, true);
...
Rect Board::GetShovelButtonRect()
{
	...
	else if (mApp->IsVersusLevel())
	{
		aRect.mY = 570;
		aRect.mX = 0;
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_VERSUS_POOL)
			aRect.mX = 15;
		if (mApp->mIsKeyboardPlayer1 && !mApp->mDebugKeysEnabled)
			aRect.mY = 5000;
	}
	return aRect;
}

Rect Board::GetSpeedButtonRect()
{
	Rect aRect(735,33, 60, 40);
	return aRect;
}
...
void Board::InitLevel()
{
	...
	else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_GLADIANTUAR)
	{
		mSunMoney = 2000;
	}
	else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_SNOWPEA)
	{
		mSunMoney = 1000;
	}
	else if (mApp->IsVersusLevel())
	...
	for (int i = 0; i < SEEDBANK_MAX; i++)
	{
		...
		{
			if (i < 6) aPacket->mX = GetSeedPacketPositionX(i, true);
			else aPacket->mX = GetSeedPacketPositionX(i - 6, false);
		}
		else aPacket->mX = GetSeedPacketPositionX(i, true);
		aPacket->mY = 8;
		aPacket->mPacketType = SeedType::SEED_NONE;
	}
	/*for (int i = 0; i < SEEDBANK_MAX; i++)
	{
		SeedPacket* aPacket = &mP2SeedBank->mSeedPackets[i];
		aPacket->mIndex = i;
		aPacket->mX = GetSeedPacketPositionX(i, false);
		aPacket->mY = 80;
		aPacket->mPacketType = SeedType::SEED_NONE;
	}*/
	// 设定固定卡牌
	...
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 7);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_SUNFLOWER); 
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_PEPPER);
		mSeedBank->mSeedPackets[2].SetPacketType(SeedType::SEED_FLAMEPEA);
		mSeedBank->mSeedPackets[3].SetPacketType(SeedType::SEED_PLASMAPEA);
		mSeedBank->mSeedPackets[4].SetPacketType(SeedType::SEED_CHILLYPEPPER);
		mSeedBank->mSeedPackets[5].SetPacketType(SeedType::SEED_POTATOMINE);
		mSeedBank->mSeedPackets[6].SetPacketType(SeedType::SEED_WALLNUT);
	}
	else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_FLIPPED)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 9);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_REVERSE_PEASHOOTER);
	...
	else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_TEST)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 6);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_REED);
	...
	else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_PLANTS_VS_PLANTS)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 7);
		...
		mSeedBank->mSeedPackets[4].SetPacketType(SeedType::SEED_REPEATER);
		mSeedBank->mSeedPackets[5].SetPacketType(SeedType::SEED_SNOWPEA);
		mSeedBank->mSeedPackets[6].SetPacketType(SeedType::SEED_CHOMPER);
	}
	else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 7);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_GRAVE);
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_ZOMBIE_NORMAL);
		mSeedBank->mSeedPackets[2].SetPacketType(SeedType::SEED_ZOMBIE_TRAFFIC_CONE);
		mSeedBank->mSeedPackets[3].SetPacketType(SeedType::SEED_ZOMBIE_PAIL);
		mSeedBank->mSeedPackets[4].SetPacketType(SeedType::SEED_ZOMBIE_FOOTBALL);
		mSeedBank->mSeedPackets[5].SetPacketType(SeedType::SEED_ZOMBIE_GARGANTUAR);
		mSeedBank->mSeedPackets[6].SetPacketType(SeedType::SEED_ZOMBIE_DANCER);
	}
	...
	else if (aGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_11)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 5);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_ZOMBIE_NORMAL);
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_ZOMBIE_POLEVAULTER);
		mSeedBank->mSeedPackets[2].SetPacketType(SeedType::SEED_ZOMBIE_PAIL);
		mSeedBank->mSeedPackets[3].SetPacketType(SeedType::SEED_ZOMBIE_FOOTBALL);
		mSeedBank->mSeedPackets[4].SetPacketType(SeedType::SEED_ZOMBIE_GIGA_FOOTBALL);
		}
	else if (aGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_12)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 5);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_ZOMBIE_NORMAL);
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_ZOMBIE_PAIL);
		mSeedBank->mSeedPackets[2].SetPacketType(SeedType::SEED_ZOMBIE_POGO);
		mSeedBank->mSeedPackets[3].SetPacketType(SeedType::SEED_ZOMBIE_SCARECROW);
		mSeedBank->mSeedPackets[4].SetPacketType(SeedType::SEED_ZOMBIE_DANCER);
		}
	else if (aGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_13)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 4);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_ZOMBIE_NORMAL);
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_ZOMBIE_TRAFFIC_CONE);
		mSeedBank->mSeedPackets[2].SetPacketType(SeedType::SEED_ZOMBIE_ICE);
		mSeedBank->mSeedPackets[3].SetPacketType(SeedType::SEED_ZOMBONI);
		}
	else if (aGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_14)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 5);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_ZOMBIE_NORMAL);
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_ZOMBIE_TRAFFIC_CONE);
		mSeedBank->mSeedPackets[2].SetPacketType(SeedType::SEED_ZOMBIE_SNORKEL);
		mSeedBank->mSeedPackets[3].SetPacketType(SeedType::SEED_ZOMBIE_PAIL);
		mSeedBank->mSeedPackets[4].SetPacketType(SeedType::SEED_ZOMBIE_FOOTBALL);
		}
	else if (aGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_15)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 4);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_ZOMBIE_NORMAL);
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_ZOMBIE_TRAFFIC_CONE);
	...
	else if (aGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_17)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 4);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_ZOMBIE_NORMAL);
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_ZOMBIE_POLEVAULTER);
	...
	else if (aGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_18)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 6);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_ZOMBIE_IMP);
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_ZOMBIE_TRAFFIC_CONE);
	...
	else if (aGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_19)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 8);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_ZOMBIE_IMP);
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_ZOMBIE_TRAFFIC_CONE);
...
{
	if (mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_RAKE] == 0)
		return;
...
void Board::InitLawnMowers()
{
	...
	if (aGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED || aGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED_TWIST ||
		...
		//(mBackground == BACKGROUND_7_FOREST && !mApp->mPlayerInfo->mPurchases[StoreItem::STORE_ITEM_WOOD_CHIPPER]))
		return;
	...
	if (mBackground == BACKGROUND_7_FOREST)
	{
		//SetForestMowerMode();
	}
}
...
bool Board::ChooseSeedsOnCurrentLevel()
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ICE || 
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_FLIPPED ||
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_TEST ||
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_PLANTS_VS_PLANTS ||
...
{
	if ((mLevel == 1 || mLevel == 49) && mApp->IsFirstTimeAdventureMode())
	{
		SexyString aLevelSkip = _S("[SKIP_LEVELS_DIALOG]");
		if (mLevel == 49) aLevelSkip = _S("[SKIP_LEVELS_DIALOG2]");
		LawnDialog* aDialog = (LawnDialog*)mApp->DoDialog(
			Dialogs::DIALOG_SKIPCONFIRM,
			true,
			_S("[SKIP_LEVELS]"),
			aLevelSkip,
			_S(""),
		...
		aDialog->mLawnYesButton->mLabel = TodStringTranslate(_S("[SKIP_BUTTON]"));
	}
	mCoinBankFadeCount = 0;
...
void Board::UpdateLevelEndSequence()
{
	...
	{
		if (!IsSurvivalStageWithRepick() && !(mLevel == 9 || mLevel == 19 || mLevel == 29 || mLevel == 39 || mLevel == 49) || mLevel == 59)
		{
...
void Board::FadeOutLevel()
{
	...
	{
		mApp->mMusic->StopAllMusic();
		if (!mApp->mPlayerInfo->mHardmodeIsOff && !mApp->HasBeatenChallengeHardmode(mApp->mGameMode))
		{
	...
	if (!IsSurvivalStageWithRepick())
	{
		...
		if (mLevel == 9 || mLevel == 19 || mLevel == 29 || mLevel == 39 || mLevel == 49 || mLevel == 59)
		{
...
bool Board::IsPoolSquare(int theGridX, int theGridY)
{
	...
	{
		//TOD_ASSERT();
		theGridX < MAX_GRID_SIZE_X&& theGridY < MAX_GRID_SIZE_Y;
...
void Board::DoPlantingEffects(int theGridX, int theGridY, Plant* thePlant)
{
	...
	if (thePlant)
	{
		...
		if (thePlant->mSeedType == SeedType::SEED_FLOWERPOT)
		{
			aYPos += 30;
		}
	...
	{
		if (thePlant->mSeedType == SEED_GRAVE && mBackground != BACKGROUND_5_ROOF && mBackground != BACKGROUND_6_BOSS)
		{
			mApp->PlayFoley(FoleyType::FOLEY_DIRT_RISE);
			mApp->AddTodParticle(aXPos - 40, aYPos, RenderLayer::RENDER_LAYER_TOP, ParticleEffect::PARTICLE_GRAVE_STONE_RISE);
		}
		else
		{
			mApp->PlayFoley(FoleyType::FOLEY_PLANT);
...
Plant* Board::AddPlant(int theGridX, int theGridY, SeedType theSeedType, SeedType theImitaterType)
{
	...
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_BOMBALL && aPlant->mSeedType != SEED_WALLNUT && aPlant->mSeedType != SEED_INSTANT_COFFEE) mBombAllCounter--;
	if (aPlant->mSeedType == SEED_GRAVE)
	{
		aPlant->mGridItemCounter = 100;
		aPlant->mRenderOrder = MakeRenderOrder(RenderLayer::RENDER_LAYER_GRAVE_STONE, theGridY, 3);
	}
...
Plant* Board::GetFlowerPotAt(int theGridX, int theGridY)
{
	...
	while (IteratePlants(aPlant))
	{
		if (aPlant->mPlantCol == theGridX && aPlant->mRow == theGridY && !aPlant->NotOnGround() && (aPlant->mSeedType == SeedType::SEED_FLOWERPOT || aPlant->mSeedType == SeedType::SEED_WATERPOT))
		{
			return aPlant;
		}
	}
...
{
	thePlantOnLawn->mWaterPlant = nullptr;
	...
	if ((theGridX < 0 || theGridX >= MAX_GRID_SIZE_X || theGridY < 0 || theGridY >= MAX_GRID_SIZE_Y) && mApp->mGameMode != GAMEMODE_CHALLENGE_PLANTS_VS_PLANTS)
		return;
	...
	while (IteratePlants(aPlant))
	{
		...
		// 将植物写入 thePlantOnLawn 的记录 statements teleport
		...
		else if (aSeedType == SeedType::SEED_WATERPOT) //teleport
		{
			thePlantOnLawn->mWaterPlant = aPlant;
		}
...
Plant* Board::GetTopPlantAt(int theGridX, int theGridY, PlantPriority thePriority)
{
	...
	case PlantPriority::TOPPLANT_EATING_ORDER:
		...
		else													return aPlantOnLawn.mWaterPlant;
	...
	//case PlantPriority::TOPPLANT_ONLY_UNDER_PLANT:				return aPlantOnLawn.mWaterPlant;
	default:													TOD_ASSERT();
	}
}
...
bool Board::CanZombieSpawnOnLevel(ZombieType theZombieType, int theLevel)
{
	...
	return gZombieAllowedLevels[theZombieType].mAllowedOnLevel[ClampInt(theLevel - 1, 0, 69)];
}
...
ZombieType Board::PickGraveRisingZombieType(int theZombiePoints)
{
	...
	int aCount = 2;
	aZombieWeightArray[0].mItem = ZombieType::ZOMBIE_BASIC;
	aZombieWeightArray[0].mWeight = GetZombieDefinition(ZombieType::ZOMBIE_BASIC).mPickWeight;
	aZombieWeightArray[1].mItem = ZombieType::ZOMBIE_CONEHEAD;
	aZombieWeightArray[1].mWeight = GetZombieDefinition(ZombieType::ZOMBIE_CONEHEAD).mPickWeight;
	if (!StageHasGraveStones())
	{
		aZombieWeightArray[2].mItem = ZombieType::ZOMBIE_BUCKETHEAD;
		aZombieWeightArray[2].mWeight = GetZombieDefinition(ZombieType::ZOMBIE_BUCKETHEAD).mPickWeight;
		aCount++;
	}
	...
	for (int i = 0; i < aCount; i++)
	{
		...
		const ZombieDefinition& aZombieDef = GetZombieDefinition(aZombieType);
		if ((mApp->IsFirstTimeAdventureMode() && mLevel < aZombieDef.mStartingLevel) || (!mZombieAllowed[aZombieType] && aZombieType != ZombieType::ZOMBIE_BASIC))
		{
...
ZombieType Board::PickZombieType(int theZombiePoints, int theWaveIndex, ZombiePicker* theZombiePicker)
{
	...
	for (int aZombieType = ZombieType::ZOMBIE_BASIC; aZombieType < ZombieType::NUM_ZOMBIE_TYPES; aZombieType++)
	{
		...
		if (mApp->IsSurvivalMode())
		{
			int aFlags = GetSurvivalFlagsCompleted();
			// 伽刚特尔和雪橇车僵尸的每波出怪上限
			if (aZombieType == ZombieType::ZOMBIE_GARGANTUAR || aZombieType == ZombieType::ZOMBIE_ZOMBONI)
			{
...
bool Board::RowCanHaveZombieType(int theRow, ZombieType theZombieType)
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_SEASTAR)
	{
		return true;  // 无草皮之地关卡，无草皮的行在前 5 波不刷出僵尸
	}
	...
	// “自古一路无巨人”（生存模式除外）
	if (theRow == 0 && !mApp->IsSurvivalMode() && mApp->mGameMode != GAMEMODE_CHALLENGE_SMASH)
	{
...
PlantingReason Board::CanPlantAt(int theGridX, int theGridY, SeedType theSeedType)
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN)
	{
		if (aPlantOnLawn.mUnderPlant || aPlantOnLawn.mPumpkinPlant || aPlantOnLawn.mFlyingPlant || aPlantOnLawn.mNormalPlant || aPlantOnLawn.mWaterPlant)
		{
			return PlantingReason::PLANTING_NOT_HERE;
		}
	...
	bool aHasLog = GetWoodLogAt(theGridX, theGridY);
	bool aHasBush = GetBushAt(theGridX, theGridY);

	if (theSeedType == SeedType::SEED_GRAVEBUSTER)
	{
		if (aPlantOnLawn.mNormalPlant)
		{
			if (aPlantOnLawn.mNormalPlant->mSeedType == SEED_GRAVE)
			{
				return PlantingReason::PLANTING_OK;
			}
	...
	if (aHasLog)
	{
		return PlantingReason::PLANTING_NOT_ON_LOG;
	}

	if (aHasBush)
	{
		return PlantingReason::PLANTING_NOT_ON_BUSH;
	}
	
	Plant* aUnderPlant = aPlantOnLawn.mUnderPlant;
	Plant* aWaterPlant = aPlantOnLawn.mWaterPlant;
	...
	if (!aWaterPlant || aWaterPlant->mOnBungeeState == PlantOnBungeeState::GETTING_GRABBED_BY_BUNGEE)
	{
		aHasWaterPot = false;
	}
	else
	{
		aHasWaterPot = aWaterPlant->mSeedType == SeedType::SEED_WATERPOT;
	}
	// 部分情况下的格子中不能种植植物
	...
	if (GetScaryPotAt(theGridX, theGridY) || (IsIceAt(theGridX, theGridY) && theSeedType != SEED_GRAVE))
	{
		return PlantingReason::PLANTING_NOT_HERE;
	}
	GridSquareType aGridSquare = mGridSquareType[theGridX][theGridY];
	if (aGridSquare == GRIDSQUARE_ROOF && theSeedType != SEED_FLOWERPOT && theSeedType != SEED_WATERPOT && (!aHasFlowerPot && !aHasLilypad && !aHasWaterPot))
	{
	...
	if (theSeedType == SeedType::SEED_LILYPAD || theSeedType == SeedType::SEED_TANGLEKELP || theSeedType == SeedType::SEED_SEASHROOM || theSeedType == SeedType::SEED_PINKSTAR)
	{
		if (!IsPoolSquare(theGridX, theGridY) && !GetFlowerPotAt(theGridX, theGridY))
		{
	...
	else if (GetFlowerPotAt(theGridX, theGridY) && !aPlantOnLawn.mUnderPlant && theSeedType != SeedType::SEED_PUMPKINSHELL)
	{
		if (theSeedType == SeedType::SEED_SPIKEWEED) return PlantingReason::PLANTING_NEEDS_GROUND;
		else return PlantingReason::PLANTING_NOT_ON_WATER;
	}
	...
	// 地刺/地刺王只能种在坚固的地面
	if (theSeedType == SeedType::SEED_SPIKEWEED || theSeedType == SeedType::SEED_SPIKEROCK || theSeedType == SeedType::SEED_THORNMOSS)
	{
	...
	// 屋顶种植需要花盆
	if (StageHasRoof() && theSeedType == SEED_GRAVE)
	{
		return (aNormalPlant || aUnderPlant || aPumpkinPlant) ? PlantingReason::PLANTING_NOT_HERE : PlantingReason::PLANTING_OK;
	}
	...
	if (aNormalPlant)
	{
		...
		// 坚果包扎术
		if ((theSeedType == SeedType::SEED_WALLNUT || theSeedType == SeedType::SEED_TALLNUT || theSeedType == SeedType::SEED_FIRESHROOM || theSeedType == SeedType::SEED_GARLIC || theSeedType == SeedType::SEED_VINENUT ||
			theSeedType == SeedType::SEED_CHARD_GUARD) && aAidPurchased)
		{
		...
		if (theSeedType == SeedType::SEED_CHARD_GUARD && aAidPurchased)
		{
			if (aNormalPlant->mChardGuardLeafs < 3 &&
				aNormalPlant->mSeedType == theSeedType && aNormalPlant->mOnBungeeState != PlantOnBungeeState::GETTING_GRABBED_BY_BUNGEE)
			{
				return PlantingReason::PLANTING_OK;
			}
...
void Board::UpdateCursor()
{
	...
	if (mPaused || mBoardFadeOutCounter >= 0 || mTimeStopCounter > 0 || (mApp->mGameScene == GameScenes::SCENE_ZOMBIES_WON && !mCutScene->mContinueAfterLoss))
	{
		mApp->SetCursor(Sexy::CURSOR_POINTER);
		return;
	}

	if (mApp->mGameMode == GAMEMODE_TREE_OF_WISDOM || mApp->mGameMode == GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES)
	{
		...
		case GameObjectType::OBJECT_TYPE_SPEED:
...
void Board::HighlightPlantsForMouse(int theMouseX, int theMouseY)
{
	...
	else
	{
		...
		if (aPlant && aPlant->mSeedType != SEED_GRAVE && !aPlant->mHypnotized)
		{
			aPlant->mHighlighted = true;
			if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN)
			{
...
void Board::UpdateToolTipP2()
{
	if (mApp->mGameScene == SCENE_PLAYING && (mApp->IsVersusLevel() || !mApp->mPlayerInfo->mIsNotCoop))
	{
		SeedPacket* aPacket = &mSeedBank->mSeedPackets[mPlayer2->mSeedBankIndex];
		SeedType aSeedtype = aPacket->mPacketType;
		mToolTipP2->SetLabel(StrFormat(_S("[%s]"), GetPlantDefinition(aSeedtype).mPlantName));

		int aPlantCost = GetCurrentPlantCost(aSeedtype, aPacket->mImitaterType);
		if (!aPacket->mActive)
		{
			//mToolTipP2->SetWarningText(_S("[WAITING_FOR_SEED]"));
			mToolTipP2->mVisible = false;
			return;
		}
		else if (!CanTakeBrainMoney(aPlantCost) && !mApp->mIsKeyboardPlayer1)
		{
			//mToolTipP2->SetWarningText(_S("[NOT_ENOUGH_BRAINS]"));
			mToolTipP2->mVisible = false;
			return;
		}
		else if (!CanTakeSunMoney(aPlantCost) && mApp->mIsKeyboardPlayer1)
		{
			//mToolTipP2->SetWarningText(_S("[NOT_ENOUGH_SUN]"));
			mToolTipP2->mVisible = false;
			return;
		}
		//else mToolTipP2->SetWarningText("");

		mToolTipP2->mX = (SEED_PACKET_WIDTH - mToolTipP2->mWidth) / 2 + mSeedBank->mX + aPacket->mOffsetX + aPacket->mX;
		mToolTipP2->mY = mSeedBank->mY + aPacket->mY + 70;
...
void Board::UpdateToolTip()
{
	...
	if (mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO)
	{
		...
		if (mApp->CanShowAlmanac() && (aZombie->mZombieType < ZOMBIE_GIGA_BASIC || aZombie->mZombieType > ZOMBIE_GIGA_POLEVAULTER) && aZombie->mZombieType != ZOMBIE_REDEYE_GARGANTUAR)
		{
	...
	if (aHitResult.mObjectType == GameObjectType::OBJECT_TYPE_SHOVEL && mApp->mGameMode != GAMEMODE_TREE_OF_WISDOM && mApp->mGameMode != GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES)
	{
	...
	if (aHitResult.mObjectType == GameObjectType::OBJECT_TYPE_NEXT_GARDEN)
	{
		...
		Rect aButtonRect = GetShovelButtonRect();
		mToolTip->mX = 599;
		mToolTip->mY = aButtonRect.mY + 52 + 20;
		mToolTip->mCenter = true;
		mToolTip->mVisible = true;
		return;
	}
	...
	if (mApp->IsVersusLevel() && ((SeedPacket::IsZombiePack(aUseSeedType) && !mApp->mIsKeyboardPlayer1) || (!SeedPacket::IsZombiePack(aUseSeedType) && mApp->mIsKeyboardPlayer1)))
	{
		//mApp->SetCursor(CURSOR_POINTER);
		mToolTip->mVisible = false;
		return;
	...
	else if (aUseSeedType == SeedType::SEED_ZOMBIE_SCARECROW)
	{
		mToolTip->SetLabel(_S("[SCARECROW]"));
	}
	else if (aUseSeedType == SeedType::SEED_ZOMBIE_GIGA_FOOTBALL)
	{
		mToolTip->SetLabel(_S("[GIGA_FOOTBALL_ZOMBIE]"));
	}
	else if (aUseSeedType == SeedType::SEED_ZOMBIE_ICE)
	{
		mToolTip->SetLabel(_S("[ICE]"));
	}
	else if (aUseSeedType == SeedType::SEED_ZOMBIE_SNORKEL)
	{
		mToolTip->SetLabel(_S("[SNORKEL_ZOMBIE]"));
	}
	else if (aUseSeedType == SeedType::SEED_ZOMBIE_ZOMBOSS)
	{
		mToolTip->SetLabel(_S("[BOSS]"));
	}
	else
	{
	...
	{
		mToolTip->SetWarningText(_S("FREE_PLANTING_CHEAT"), false);
	}
	else if (!aSeedPacket->mActive && (gLawnApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED || gLawnApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED_TWIST))
	{
		...
		{
			mToolTip->SetWarningText(_S("[BEGHOULED_NO_CRATERS]"), false);
		}
		else
		{
			mToolTip->SetWarningText(_S("[BEGHOULED_SEED_ALREADY_PURCHASED]"), false);
		}
	}
	else if (!aSeedPacket->mActive)
	{
		mToolTip->SetWarningText(_S("[WAITING_FOR_SEED]"), false);
	}
	else if (!CanTakeSunMoney(aPlantCost) && !HasConveyorBeltSeedBank() && !mApp->IsSlotMachineLevel() && (!SeedPacket::IsZombiePack(aUseSeedType) || !mChallenge->IsVersusSeedType(aUseSeedType)))
	{
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES) mToolTip->SetWarningText(_S("[NOT_ENOUGH_BRAINS]"), false);
		else mToolTip->SetWarningText(_S("[NOT_ENOUGH_SUN]"), false);
	}
	else if (!CanTakeBrainMoney(aPlantCost) && !HasConveyorBeltSeedBank() && !mApp->IsSlotMachineLevel() && SeedPacket::IsZombiePack(aUseSeedType) && mChallenge->IsVersusSeedType(aUseSeedType))
	{
		mToolTip->SetWarningText(_S("[NOT_ENOUGH_BRAINS]"), false);
	}
	else if (aUseSeedType == SeedType::SEED_GATLINGPEA)
	{
		if (!PlantingRequirementsMet(aUseSeedType))
		{
			mToolTip->SetWarningText(_S("[REQUIRES_REPEATER]"), false);
		}
	}
	else if (aUseSeedType == SeedType::SEED_WINTERMELON)
	{
		if (!PlantingRequirementsMet(aUseSeedType))
		{
			mToolTip->SetWarningText(_S("[REQUIRES_MELONPULT]"), false);
		}
	}
	else if (aUseSeedType == SeedType::SEED_TWINSUNFLOWER)
	{
		if (!PlantingRequirementsMet(aUseSeedType))
		{
			mToolTip->SetWarningText(_S("[REQUIRES_SUNFLOWER]"), false);
		}
	}
	else if (aUseSeedType == SeedType::SEED_SPIKEROCK)
	{
		if (!PlantingRequirementsMet(aUseSeedType))
		{
			mToolTip->SetWarningText(_S("[REQUIRES_SPIKEWEED]"), false);
		}
	}
	else if (aUseSeedType == SeedType::SEED_COBCANNON)
	{
		if (!PlantingRequirementsMet(aUseSeedType))
		{
			mToolTip->SetWarningText(_S("[REQUIRES_KERNELPULTS]"), false);
		}
	}
	else if (aUseSeedType == SeedType::SEED_GOLD_MAGNET)
	{
		if (!PlantingRequirementsMet(aUseSeedType))
		{
			mToolTip->SetWarningText(_S("[REQUIRES_MAGNETSHROOM]"), false);
		}
	}
	else if (aUseSeedType == SeedType::SEED_GLOOMSHROOM)
	{
		if (!PlantingRequirementsMet(aUseSeedType))
		{
			mToolTip->SetWarningText(_S("[REQUIRES_FUMESHROOM]"), false);
		}
	}
	else if (aUseSeedType == SeedType::SEED_CATTAIL)
	{
		if (!PlantingRequirementsMet(aUseSeedType))
		{
			mToolTip->SetWarningText(_S("[REQUIRES_LILY_PAD]"), false);
		}
	}
...
void Board::MouseDownWithPlant(int x, int y, int theClickCount)
{
	...
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES && aPlantingSeedType != SEED_GRAVE)
	{
		mChallenge->IZombieMouseDownWithZombie(x, y, theClickCount);
		return;
	}

	if (mApp->IsVersusLevel() && aPlantingSeedType != SEED_GRAVE && SeedPacket::IsZombiePack(aPlantingSeedType))
	{
		mChallenge->IZombieMouseDownWithZombie(x, y, theClickCount);
		return;
	}
	...
	if (aReason != PlantingReason::PLANTING_OK)
	{
		...
		else if (aPlantingSeedType == SeedType::SEED_PINKSTAR)
		{
			if (aReason == PlantingReason::PLANTING_ONLY_IN_POOL)
			{
				DisplayAdvice(_S("[ADVICE_TANGLEKELP_ON_WATER]"), MessageStyle::MESSAGE_STYLE_HINT_FAST, AdviceType::ADVICE_PLANT_TANGLEKELP_ON_WATER);
		...
		else if (aReason == PlantingReason::PLANTING_NOT_PASSED_LINE_VERSUS)
		{
			DisplayAdvice(_S("[ADVICE_NOT_PASSED_LINE_VERSUS]"), MessageStyle::MESSAGE_STYLE_HINT_FAST, AdviceType::ADVICE_PLANT_NOT_PASSED_LINE);
		}
		else if (aReason == PlantingReason::PLANTING_NOT_PASSED_LINE_VERSUS_PLANTS)
		{
			DisplayAdvice(_S("[ADVICE_NOT_PASSED_LINE_VERSUS_PLANTS]"), MessageStyle::MESSAGE_STYLE_HINT_FAST, AdviceType::ADVICE_PLANT_NOT_PASSED_LINE);
		}
		...
		else if (aReason == PlantingReason::PLANTING_NOT_ON_LOG)
		{
			DisplayAdvice(_S("[ADVICE_PLANT_NOT_ON_LOG]"), MessageStyle::MESSAGE_STYLE_HINT_FAST, AdviceType::ADVICE_PLANT_NOT_ON_GRAVE);
		}
		else if (aReason == PlantingReason::PLANTING_NOT_ON_BUSH)
		{
			DisplayAdvice(_S("[ADVICE_PLANT_NOT_ON_BUSH]"), MessageStyle::MESSAGE_STYLE_HINT_FAST, AdviceType::ADVICE_PLANT_NOT_ON_GRAVE);
			}
	...
	ClearAdvice(AdviceType::ADVICE_ZOMBIE_NOT_ON_WATER);
	...
	if (!mApp->mEasyPlantingCheat && mCursorObject->mCursorType == CursorType::CURSOR_TYPE_PLANT_FROM_BANK && !HasConveyorBeltSeedBank() && !SeedPacket::IsZombiePack(aPlantingSeedType))
	{
	 ...
	 if (!mApp->mEasyPlantingCheat && mCursorObject->mCursorType == CursorType::CURSOR_TYPE_PLANT_FROM_BANK && !HasConveyorBeltSeedBank() && SeedPacket::IsZombiePack(aPlantingSeedType))
	{
		if (!TakeBrainMoney(GetCurrentPlantCost(aPlantingSeedType, SeedType::SEED_NONE)))
		{
			return;
		}
	}
	
	// 升级种植或坚果包扎术等情况时，先将原植物销毁 upgradeteleport
	bool aIsAwake = false;
	...
	/*while (IteratePlants(aPlant))
	{
		if (aPlant->mSeedType == SEED_FUMESHROOM || aPlant->mPlantCol != x || aPlant->mRow != y)
		{
			aNormalPlant = aPlant;
	...
	{
		if (aPlantingSeedType == SeedType::SEED_GLOOMSHROOM || aPlantingSeedType == SeedType::SEED_ICYFUME || aPlantingSeedType == SeedType::SEED_BRAVESHROOM)
		{
	...
	if ((aPlantingSeedType == SeedType::SEED_WALLNUT || aPlantingSeedType == SeedType::SEED_TALLNUT || aPlantingSeedType == SeedType::SEED_FIRESHROOM ||
		aPlantingSeedType == SeedType::SEED_GARLIC || aPlantingSeedType == SeedType::SEED_CHARD_GUARD) && aNormalPlant)
	{
...
void Board::MouseDownWithTool(int x, int y, int theClickCount, CursorType theCursorType)
{
	...
	else if (theCursorType == CursorType::CURSOR_TYPE_SHOVEL && aPlant->mSeedType != SEED_GRAVE && !aPlant->mHypnotized)
	{
...
bool Board::MouseHitTest(int x, int y, HitResult* theHitResult)
{
	...
	Rect aShovelButtonRect = GetShovelButtonRect(); //shovel hitbox
	Rect aSpeedButtonRect = GetSpeedButtonRect(); //speed hitbox
	...
	if ((mBackground != BackgroundType::BACKGROUND_GREENHOUSE && mBackground != BackgroundType::BACKGROUND_MUSHROOM_GARDEN&& mBackground != BackgroundType::BACKGROUND_TREEOFWISDOM &&
		!mApp->IsVersusLevel()) && aSpeedButtonRect.Contains(x, y) && CanInteractWithBoardButtons())
	{
		theHitResult->mObjectType = GameObjectType::OBJECT_TYPE_SPEED;
		return true;
	}
...
void Board::MouseDown(int x, int y, int theClickCount)
{
	...
	else if (aHitResult.mObjectType == GameObjectType::OBJECT_TYPE_SHOVEL || //teleport
	...
	else if (aHitResult.mObjectType == GameObjectType::OBJECT_TYPE_SPEED)  //toggle speedbutton
	{
		if (!spawnSpeed) return;
		if (mSpeedIsOn)
		{
...
void Board::CauseGlitch()
{
	mApp->PlayFoley(FoleyType::FOLEY_CRASH);
	DisplayAdviceAgain(("[GLITCH_ADVICE]"), MessageStyle::MESSAGE_STYLE_HINT_FAST, AdviceType::ADVICE_ALMOST_THERE);

	Plant* aPlant = nullptr;
	while (IteratePlants(aPlant))
	{
		int aPotentialX = RandRangeInt(0, 8);
		int aPotentialY = RandRangeInt(0, 4);
		
		int stopNumber = 0;
		while (CanPlantAt(aPotentialX, aPotentialY, aPlant->mSeedType) && aPlant->mSeedType != SEED_PUMPKINSHELL && stopNumber < 10)
		{
			aPotentialX = RandRangeInt(0, 8);
			aPotentialY = RandRangeInt(0, 4);
			stopNumber++;
		}
		aPlant->mPlantCol = aPotentialX;
		aPlant->mRow = aPotentialY;
		aPlant->mX = GridToPixelX(aPotentialX, aPotentialY);
		aPlant->mY = GridToPixelY(aPotentialX, aPotentialY);
		aPlant->mRenderOrder = aPlant->CalcRenderOrder();
		DoPlantingEffects(aPotentialX, aPotentialY, aPlant);
		stopNumber++;
	}
}

void Board::DeactivateBloodMoon()
{
	BloodMoonIsOn = false;
	LoadBackgroundImages();
}
...
int Board::GetGraveStonesCount()
{
	...
	while (IterateGridItems(aGridItem))
	{
		if (aGridItem->mGridItemType == GridItemType::GRIDITEM_GRAVESTONE || aGridItem->mGridItemType == GridItemType::GRIDITEM_BUSH)
		{
			aCount++;
		}
	}
...
void Board::SpawnZombiesFromGraves()
{
	...
	while (IterateGridItems(aGridItem))
	{
		if ((aGridItem->mGridItemType != GridItemType::GRIDITEM_GRAVESTONE && aGridItem->mGridItemType != GridItemType::GRIDITEM_BUSH) || aGridItem->mGridItemCounter < 100)
		{
			continue;
		}
...
void Board::SpawnZombieWave()
{
	...
	if (mApp->IsBungeeBlitzLevel())
	{
		...
		for (int i = 0; i < MAX_ZOMBIES_IN_WAVE; i++)
		{
			...
			if (aZombieType == ZombieType::ZOMBIE_BUNGEE || aZombieType == ZombieType::ZOMBIE_ZOMBONI)
			{
	...
	else
	{
		...
		for (int i = 0; i < MAX_ZOMBIES_IN_WAVE; i++)
		{
			...
			if (aZombieType == ZombieType::ZOMBIE_BOBSLED && !CanAddBobSled())
			{
				...
				{
					AddZombie(ZombieType::ZOMBIE_BASIC, mCurrentWave);  // 生成 4 只普通僵尸以代替雪橇僵尸小队
				}
			}
			else
			{
...
void Board::UpdateGameObjects()
{
	...
	/*for (int i = 0; i < mP2SeedBank->mNumPackets; i++)
	{
		mP2SeedBank->mSeedPackets[i].Update();
	}*/
}
...
void Board::SurvivalSaveScore()
{
	...
	if (mApp->mPlayerInfo->mHardmodeIsOff)
	{
	...
	{
		int aFlagsCompleted = GetSurvivalFlagsCompleted();
		int& aFlagsRecord = mApp->mPlayerInfo->mChallengeHardmodeRecords[mApp->GetCurrentChallengeIndex()];
		if (aFlagsCompleted > aFlagsRecord)
		{
			aFlagsRecord = aFlagsCompleted;
			mApp->WriteCurrentUserConfig();
...
void Board::ZombiesWon(Zombie* theZombie)
{
	...
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_FLIPPED) aReanim->SetImageOverride("ZombiesWon", IMAGE_GAME_OVER_FLIPPED);
}
...
void Board::UpdateSunSpawning()
{
	...
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_VERSUS_FOREST)
	{
		AddCoin(RandRangeInt(100, 400), 60, CoinType::COIN_SMALLSUN, CoinMotion::COIN_MOTION_FROM_SKY);
		AddCoin(RandRangeInt(600, 749), 60, CoinType::COIN_SMALLBRAIN, CoinMotion::COIN_MOTION_FROM_SKY);
	}
	else if (mApp->IsVersusLevel())
	{
		AddCoin(RandRangeInt(100, 400), 60, aSunType, CoinMotion::COIN_MOTION_FROM_SKY);
		AddCoin(RandRangeInt(600, 749), 60, CoinType::COIN_BRAIN, CoinMotion::COIN_MOTION_FROM_SKY);

		if (mApp->mGameMode == GAMEMODE_CHALLENGE_VERSUS_OLD && mBoardAge >= 30000)
		{
			AddCoin(RandRangeInt(100, 400), 60, aSunType, CoinMotion::COIN_MOTION_FROM_SKY);
			AddCoin(RandRangeInt(600, 749), 60, CoinType::COIN_BRAIN, CoinMotion::COIN_MOTION_FROM_SKY);
		}
	...
	{
		AddCoin(RandRangeInt(100, 400), 60, COIN_SMALLSUN, CoinMotion::COIN_MOTION_FROM_SKY);
	}
	else 
	{
...
void Board::UpdateIce()
{
	for (int aRow = 0; aRow < MAX_GRID_SIZE_Y; aRow++)
	{
		if (mIceTimer[aRow])
		{
			...
			{
				mIceTimerAge[aRow]++;
				if (mIceTimerAge[aRow] == 3500)
				{
					AddZombie(ZOMBIE_BOBSLED, aRow);
				}
			}
		}
	}
...
{
	if (mApp->IsFinalBossLevel() || mApp->mGameMode == GAMEMODE_CHALLENGE_GLADIANTUAR || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_SNOWPEA)
	{
...
void Board::Update()
{
	if (mBloodCounterCooldown > 0 && !mPaused)
	{
		mBloodCounterCooldown--;
		if (mBloodCounterCooldown == 0)
		{
			DeactivateBloodMoon();
		}
	}
	if (mRageDelay > 0 && !mPaused)
	{
		mRageDelay--;
		if (mRageDelay == 500)
		{
			int aLevelCounter = 600;
			if (mLevel == 51) aLevelCounter = 400;
			else if (mLevel >= 52 && mLevel <= 54) aLevelCounter = 500;
			else if (mLevel >= 55 && mLevel <= 56) aLevelCounter = 600;
			else if (mLevel >= 57 && mLevel <= 59) aLevelCounter = 700;

			Zombie* aZombie = nullptr;
			while (IterateZombies(aZombie))
			{
				if (aZombie && aZombie->mZombieType != ZOMBIE_GLADIANTUAR)
				{
					aZombie->mRageCounter = aLevelCounter;
					aZombie->UpdateAnimSpeed();
				}
			}
			mBloodCounterCooldown = aLevelCounter;
		}
	}
	...
	if (mDummyCounter == 3)
	{
		GameOverDialog* aGameOverDialog = new GameOverDialog("[PVP_DEATH_MESSAGE]", true);
		mApp->AddDialog(Dialogs::DIALOG_GAME_OVER, aGameOverDialog);
		mApp->mMusic->StopAllMusic();
		StopAllZombieSounds();
		mApp->PlaySample(Sexy::SOUND_LOSEMUSIC);
	...
	//Checks if player have only pressed the key once
	...
	{
		if (mJustPressedKeys[KeyCode::KEYCODE_BACK] && mApp->mGameMode != GAMEMODE_CHALLENGE_ZEN_GARDEN && mApp->mGameMode != GAMEMODE_TREE_OF_WISDOM && !mApp->IsVersusLevel())
		{
			mApp->PlayFoley(FoleyType::FOLEY_WAKEUP);
			mHeldSpeeding = mSpeedIsOn = true;
		}
	}
	...
	if (mHeldSpeeding && !mApp->mWidgetManager->mKeyDown[KeyCode::KEYCODE_BACK]) {
		mApp->PlayFoley(FoleyType::FOLEY_WAKEUP_REVERSE);
		mApp->PlaySample(SOUND_GRAVEBUTTON);
		mHeldSpeeding = mSpeedIsOn = false;
	}
	...
	if (mApp->mGameScene == SCENE_PLAYING || (mApp->mGameScene == SCENE_LEVEL_INTRO && SeedChooserScreen::CheckSeedchooserClosed()))
	{
		//Player Code
		PlayerController* player = mPlayer2; // I would have just replace player-> with mPlayer2-> but its too large
		...
		{
			string upKey{ player->mKeyUp };
			string leftKey{ player->mKeyLeft };
			string downKey{ player->mKeyDown };
			string rightKey{ player->mKeyRight };
			...
			//This is math issue lets fix this later... it basically centers your cursor when you release all the move buttons
			//I see
			//Bro my tooth ache is so Skibidi
			//howie
			//Bro my tooth ache hurts very much
			...
			if (xInput == 0 && yInput == 0 && mApp->mGameMode != GAMEMODE_CHALLENGE_AIR_RAID) {
				...
				float targetY = GridToPixelY(gridX, gridY) + roofY + 45;
				...
				player->mY = max(min(player->mY, (StageHas6Rows() ? 510 : 500) + LAWN_YMIN + 45.0f), 45.0f);
			}

			string aKey = "";
			aKey = player->mKeyA;
			
			if ((mJustPressedKeys[GetKeyCodeFromName(aKey)] && player->mSeedBankIndex > -1) && (mSeedBank->GetNumSeedsOnConveyorBelt() > 1 || !HasConveyorBeltSeedBank()) && mApp->mGameScene == SCENE_PLAYING) {
				SeedType seedType = mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType;

				Plant* cobBack = GetPlantAt(gridX, gridY);
				Plant* cobFront = GetPlantAt(gridX - 1, gridY);
					
				if (seedType == SEED_IMITATER)
					seedType = mSeedBank->mSeedPackets[player->mSeedBankIndex].mImitaterType;
				if (mCobActivated)
				{
					Plant* cobFire = GetPlantAt(mGridXCob, mGridYCob);
					if (cobFire) cobFire->CobCannonFire(player->mX, player->mY);
				}
				else if ((cobFront && cobFront->mSeedType == SEED_COBCANNON))
				{
					if (cobFront && cobFront->mState == PlantState::STATE_COBCANNON_READY)
					{
						mCobActivated = true;
						mGridXCob = gridX - 1;
						mGridYCob = gridY;
				...
				else if ((cobBack && cobBack->mSeedType == SEED_COBCANNON))
				{
					if (cobBack && cobBack->mState == PlantState::STATE_COBCANNON_READY)
					{
						mCobActivated = true;
						mGridXCob = gridX;
						mGridYCob = gridY;
				...
				else if (seedType != SEED_NONE /*&& seedType <= NUM_SEED_TYPES*/ && CanPlantAt(gridX, gridY, seedType) == PLANTING_OK)
				{
					if ((mChallenge->IsVersusSeedType(seedType) || mChallenge->IsZombieSeedType(seedType)) && (mApp->mGameMode != GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES || seedType != SEED_GRAVE))
					{
						bool canTakeSuns = false;
						if (mChallenge->IsVersusSeedType(seedType)) canTakeSuns = CanTakeBrainMoney(GetCurrentPlantCost(mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType, SeedType::SEED_NONE));
						else canTakeSuns = CanTakeSunMoney(GetCurrentPlantCost(mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType, SeedType::SEED_NONE));
						...
						if ((seedType == SeedType::SEED_GRAVE) && mBoardAge >= 30000)
						{
							mApp->PlaySample(SOUND_BUZZER);
						}
						else if (seedType == SeedType::SEED_GRAVE)
						{
							if (allowToPlace) {
								TakeBrainMoney(GetCurrentPlantCost(mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType, SeedType::SEED_NONE));
								...
								Plant* aPlant = AddPlant(gridX, gridY, mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType, mSeedBank->mSeedPackets[player->mSeedBankIndex].mImitaterType);
						...
						{
							if (allowToPlace) {
								if (mChallenge->IsVersusSeedType(seedType)) TakeBrainMoney(GetCurrentPlantCost(mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType, SeedType::SEED_NONE));
								else TakeSunMoney(GetCurrentPlantCost(mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType, SeedType::SEED_NONE));
								...
								//Plant* aPlant = AddPlant(gridX, gridY, mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType, mSeedBank->mSeedPackets[player->mSeedBankIndex].mImitaterType);
								//Zombie* aZombie = AddZombieInRow(ZOMBIE_NORMAL, gridY, 0);
								ZombieType aZombieType = mChallenge->IZombieSeedTypeToZombieType(mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType);
								mChallenge->IZombiePlaceZombie(aZombieType, gridX, gridY);
					...
					else
					{
						...
						if ((seedType == SeedType::SEED_SUNFLOWER) && mBoardAge >= 30000)
						{
							mApp->PlaySample(SOUND_BUZZER);
						}
						else if (allowToPlace) {
							...
							Plant* aPlant = AddPlant(gridX, gridY, mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType, mSeedBank->mSeedPackets[player->mSeedBankIndex].mImitaterType);
							if (HasConveyorBeltSeedBank() && player->mSeedBankIndex > 0) player->mSeedBankIndex--;
							if (aIsAwake)
								aPlant->SetSleeping(false);
							else
								aPlant->mWakeUpCounter = aWakeUpCounter; //read live chat bro

							/*
							if (mApp->mGameMode == GAMEMODE_CHALLENGE_BOMB_ALL)
							{
								if (mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType == SEED_POTATOMINE || mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType == SEED_CHERRY_HOVERBOMB || mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType == SEED_CHERRYBOMB || mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType == SEED_JALAPENO || mSeedBank->mSeedPackets[player->mSeedBankIndex].mPacketType == SEED_DOOMSHROOM)
			...
			string lbKey{ player->mKeyLB };
			string rbKey{ player->mKeyRB };

			if (mJustPressedKeys[GetKeyCodeFromName(lbKey)])
			{
				if (mApp->IsVersusLevel())
				{
					if (player->mSeedBankIndex == 0)
					{
						player->mSeedBankIndex = 5;
					}
					else if (player->mSeedBankIndex == 6)
					{
						player->mSeedBankIndex = 11;
					}
					else player->mSeedBankIndex--;
				}
				else 
				{
					int aNumSeeds = GetNumSeedsInBank() - 1;
					if (HasConveyorBeltSeedBank()) aNumSeeds = mSeedBank->GetNumSeedsOnConveyorBelt() - 1;
					if (player->mSeedBankIndex == 0)
					{
						player->mSeedBankIndex = aNumSeeds;
					}
					else player->mSeedBankIndex--;
				}
			}
			...
			if (mJustPressedKeys[GetKeyCodeFromName(rbKey)])
			{
				if (mApp->IsVersusLevel())
				{
					if (player->mSeedBankIndex == 5)
					{
						player->mSeedBankIndex = 0;
					}
					else if (player->mSeedBankIndex == 11)
					{
						player->mSeedBankIndex = 6;
					}
					else player->mSeedBankIndex++;
				}
				else
				{
					int aNumSeeds = GetNumSeedsInBank() - 1;
					if (HasConveyorBeltSeedBank()) aNumSeeds = mSeedBank->GetNumSeedsOnConveyorBelt() - 1;
					if (player->mSeedBankIndex == aNumSeeds)
					{
						player->mSeedBankIndex = 0;
					}
					else player->mSeedBankIndex++;
			...
			//You should fix this part since our seedbanks draw differently...
			// It only problem is that it selects seeds that are still outside the conveyor
			// We have a lot of work next is the SeedPackets.cpp lmao
			///sure/ Itre xd
			if (!mApp->mPlayerInfo->mIsNotCoop && !mApp->IsVersusLevel())
			{
				int aNumSeeds = GetNumSeedsInBank() - 1;
				player->mSeedBankIndex = max(0, min(player->mSeedBankIndex, aNumSeeds));
			}
			else
			{
				if (!mApp->mIsKeyboardPlayer1)
				{
					player->mSeedBankIndex = max(6, min(player->mSeedBankIndex, 11));
				}
				else
				{
					player->mSeedBankIndex = max(0, min(player->mSeedBankIndex, 5));
				}
			}

			string bKey{ player->mKeyB };

			if (mJustPressedKeys[GetKeyCodeFromName(bKey)])
			{
				Plant* cobFront = GetPlantAt(gridX - 1, gridY);
				if (cobFront && cobFront->mSeedType == SEED_COBCANNON)
				{
					cobFront->Die();
				...
				while (IteratePlants(aPlant))
				{
					...
					else if (aPlant->mRow == gridY && aPlant->mPlantCol == gridX)
					{
						if (aPlant->mSeedType != SeedType::SEED_GRAVE && (mApp->mIsKeyboardPlayer1 || !mApp->IsVersusLevel()) && !aPlant->mHypnotized && !mApp->IsIZombieLevel() && !aPlant->mSquished)
						{
							mApp->PlayFoley(FOLEY_USE_SHOVEL);
							Plant* aTopPlant = GetTopPlantAt(aPlant->mPlantCol, aPlant->mRow, TOPPLANT_ANY);
							aTopPlant->Die();
						}
					}
				}
			}
			//string xKey{ player->mKeyX };

			if (mJustPressedKeys[KEYCODE_SPACE])
			{
				Coin* aCoin = nullptr;
				while (IterateCoins(aCoin))
				{
					if (mApp->IsVersusLevel())
					{
						if (mApp->mIsKeyboardPlayer1 && aCoin->IsSun()) aCoin->TryAutoCollectAfterLevelAward();
						else if (!mApp->mIsKeyboardPlayer1 && aCoin->IsBrain()) aCoin->TryAutoCollectAfterLevelAward();
					}
				}
			}
		}
	...
	if ((mBackground == BackgroundType::BACKGROUND_3_POOL || mBackground == BackgroundType::BACKGROUND_POOL_PARTY) && mPoolSparklyParticleID == ParticleSystemID::PARTICLESYSTEMID_NULL && mDrawCount > 0)
	{
		...
		if (mBackground == BackgroundType::BACKGROUND_POOL_PARTY)
		{
			aRenderPosition = MakeRenderOrder(RenderLayer::RENDER_LAYER_GROUND, 2, 0);
			TodParticleSystem* aPoolParticle2 = mApp->AddTodParticle(450, 95, aRenderPosition, ParticleEffect::PARTICLE_POOL_SPARKLY);
			mPoolSparklyParticleID = mApp->ParticleGetID(aPoolParticle);

			aRenderPosition = MakeRenderOrder(RenderLayer::RENDER_LAYER_GROUND, 2, 0);
			TodParticleSystem* aPoolParticle3 = mApp->AddTodParticle(450, 445, aRenderPosition, ParticleEffect::PARTICLE_POOL_SPARKLY);
			mPoolSparklyParticleID = mApp->ParticleGetID(aPoolParticle);
		}
	}
	...
	UpdateFwoosh();
	UpdateGame();
	//if (mApp->mHardmodeIsOn)
	{
		//mApp->PlayFoley(FoleyType::FOLEY_GLADIANTUAR_DEATH);
		//mApp->mHardmodeIsOn = false;
	}
	if (mSpeedIsOn)
	{

		//UpdateGame();
		//if (HasConveyorBeltSeedBank()) mChallenge->UpdateConveyorBelt();
		//mApp->mHardmodeIsOn = false;
	...
	*/
	...
	// You should copy paste this to where mSpeedIsOn
	mPlayer2->UpdateAnimations();
	...
	if (mApp->IsVersusLevel() && mSpawnBombAll == true)
	{
		if (mApp->mGameScene != GameScenes::SCENE_ZOMBIES_WON && !mCutScene->mContinueAfterLoss && !mLevelAwardSpawned) mBoardAge++;
		if (mBoardAge % 6000 == 0)
		{
			if (mApp->mGameMode == GAMEMODE_CHALLENGE_VERSUS_POOL || mApp->mGameMode == GAMEMODE_CHALLENGE_VERSUS_FOG)
			{
				for (int x = 0; x < 3; x++)
				{
					for (int y = 0; y < 6; y++)
			...
			else if (mApp->mGameMode == GAMEMODE_CHALLENGE_VERSUS_ROOF || mApp->mGameMode == GAMEMODE_CHALLENGE_VERSUS_MOON)
			{
				for (int x = 0; x < 4; x++)
				{
					for (int y = 0; y < 6; y++)
			...
			if (mBoardAge >= 60000)
			{
				SeedType aSeedType = SEED_PEASHOOTER;
				if (mApp->mGameMode == GAMEMODE_CHALLENGE_VERSUS_ROOF || GAMEMODE_CHALLENGE_VERSUS_MOON) aSeedType = SEED_CABBAGEPULT;
				for (int x = 0; x < 1; x++)
				{
					for (int y = 0; y < 6; y++)
					{
						if (CanPlantAt(x, y, aSeedType) == PlantingReason::PLANTING_OK)
						{
							AddPlant(x, y, aSeedType, SeedType::SEED_NONE);
						}
					}
				}
			}
		}
		if (mBoardAge == 30000 && mApp->mGameMode != GAMEMODE_CHALLENGE_VERSUS_OLD)
		{
			mApp->PlayFoley(FOLEY_WAKEUP);
			mApp->PlaySample(SOUND_GRAVEBUTTON);
			mSpeedIsOn = true;
			mPlayer2->mControllerSpeed = 10;
			DisplayAdvice(_S("[ADVICE_SUDDEN_DEATH]"), MessageStyle::MESSAGE_STYLE_BIG_MIDDLE_FAST, AdviceType::ADVICE_NONE);
			...
			while (IterateGridItems(aGriditem))
			{
				...
				else if (aGriditem->mGridItemType == GRIDITEM_GRAVESTONE || aGriditem->mGridItemType == GRIDITEM_BUSH)
				{
					aGriditem->GridItemDie();
					int aRand = Rand(2);
					if (aRand == 0)  mApp->PlayFoley(FoleyType::FOLEY_BUSH_DESTROYED1);
					else if (aRand == 1)  mApp->PlayFoley(FoleyType::FOLEY_BUSH_DESTROYED2);
					else  mApp->PlayFoley(FoleyType::FOLEY_BUSH_DESTROYED3);
				}
			}
		}
	}
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_PLANTS_VS_PLANTS && mSpawnBombAll == true)
	{
		mBoardAge++;
		if (mBoardAge % 2000 == 0 && !mLevelAwardSpawned)
		{
...
void Board::DrawBackdrop(Graphics* g)
{
	...
	case BackgroundType::BACKGROUND_7_FOREST:			aBgImage = Sexy::IMAGE_BACKGROUND_FOREST;				break;
	case BackgroundType::BACKGROUND_POOL_PARTY:			aBgImage = Sexy::IMAGE_BACKGROUND_PP;					break;
	case BackgroundType::BACKGROUND_HIGHGROUND:			aBgImage = Sexy::IMAGE_BACKGROUND_HIGHGROUND;			break;
	default:											TOD_ASSERT();											break;
	}
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_ICE || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_SNOWPEA) aBgImage = Sexy::IMAGE_BACKGROUND_ICE;
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_FLIPPED) aBgImage = Sexy::IMAGE_MAP_FLIPPED;
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_GLADIANTUAR) aBgImage = Sexy::IMAGE_BACKGROUND_GLADIANTUAR;
	else if (mApp->IsDarkLevel()) aBgImage = Sexy::IMAGE_FOREST_NIGHT_MAP;

	/*	Image* merkImage = aBgImage;
	if (BloodMoonIsOn) aBgImage = IMAGE_BACKGROUND_HIGHGROUND;
	else aBgImage = merkImage;*/
	...
	else if (aBgImage)
	{
		...
		else if (mApp->IsBloodMoonLevel())
		{
			...
			{
				g->SetColorizeImages(true);
				if (mRageDelay >= 500) g->SetColor(Color(255, 255, 255, 255 - ((mRageDelay - 500) / 2)));
				else if (mRageDelay >= 250) g->SetColor(Color(255, 255, 255, 255));
				else g->SetColor(Color(255, 255, 255, mRageDelay));

				if (BloodMoonIsOn)
					aBgImage = Sexy::IMAGE_BACKGROUND_BLOODMOON;
				else
		...
		else if (mApp->mGameMode == GAMEMODE_CHALLENGE_ECLIPSE)
		{
			g->DrawImage(aBgImage, -BOARD_OFFSET, 0);
			if (mApp->mGameScene == SCENE_PLAYING && mChallenge->mChallengeStateCounter <= 500 && mChallenge->mChallengeStateCounter >= 0)
			{
	...
	if (mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO && StageHasGraveStones() && mApp->mGameMode != GAMEMODE_CHALLENGE_GLADIANTUAR && mApp->mBoard->mBackground == BACKGROUND_2_NIGHT)
	{
	...
	/*if (mBackground == BACKGROUND_7_FOREST)
	{
		for (int i = 0; i < 5; i++)
		{
			for (int j = 0; j < 9; j++)
			{
				//g->DrawImage(Sexy::IMAGE_PUDDLE, j * 80 + 40, i * 100 + 85);
			}
		}
	}*/

	if (mApp->IsBloodMoonLevel() && mCutScene->mCutsceneTime <= 8380)
	{
		g->SetColorizeImages(true);
		if (mCutScene->mCutsceneTime < 4250) g->SetColor(Color(255, 255, 255, 6 * mCutScene->mCutsceneTime / 100));
		else if (mCutScene->mCutsceneTime == 4250) g->SetColor(Color(255, 255, 255, 255));
		else if (mCutScene->mCutsceneTime > 4250) g->SetColor(Color(255, 255, 255, 255 - (6 * (mCutScene->mCutsceneTime - 4130) / 100)));
...
void Board::DrawGameObjects(Graphics* g)
{
	...
	}
	{
		...
		AddUIRenderItem(aRenderList, aRenderItemCount, RenderObjectType::RENDER_ITEM_OBJECTS, MakeRenderOrder(RenderLayer::RENDER_LAYER_OBJECTS, 0, 0));
	...
	if (mApp->IsStormyNightLevel() || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_RAINING_SEEDS || mApp->IsDarkLevel())
	{
	...
	for (int i = 0; i < aRenderItemCount; i++)
	{
		...
		case RenderObjectType::RENDER_ITEM_PLANT:
		{
			...
			if (aPlant->BeginDraw(g))
			{
				...
				{				
					...
					ReanimatorTrackInstance* aTrackInstance = aBodyReanim->GetTrackInstanceByName("anim_waterline");
					aTrackInstance->mIgnoreExtraAdditiveColor = true;
					aTrackInstance->mIgnoreColorOverride = true;
					aTrackInstance->mIgnoreClipRect = true;
					if (aPlant->mGridItemCounter > 0) aBodyReanim->AssignRenderGroupToTrack("anim_waterline", RENDER_GROUP_HIDDEN);
					else aBodyReanim->AssignRenderGroupToTrack("anim_waterline", RENDER_GROUP_NORMAL);
					if (mPlantRow[aPlant->mRow] == PLANTROW_POOL) aPlantGraphics.SetClipRect(-42, -88, 197, 160);
					else aPlantGraphics.SetClipRect(-42, -98, 197, 187);
				}
		...
		case RenderObjectType::RENDER_ITEM_OBJECTS:
		{
			if (mApp->IsDarkLevel())
			{
				g->DrawImage(Sexy::IMAGE_FOREST_NIGHT_ITEMS, -220, 0);
			}
			else if (mApp->mBoard->mBackground == BACKGROUND_7_FOREST)
			{
				g->DrawImage(Sexy::IMAGE_FOREST_OBJECTS, -220, 0);
			}
			break;
		}
...
void Board::DrawDarkness(Graphics* g)
{
	...
	if (mUpdateDarkness)
	{
		...
		aMemoryGraphics.SetDrawMode(Graphics::DRAWMODE_ERASE);
		...
		aMemoryGraphicsDarker.SetDrawMode(Graphics::DRAWMODE_ERASE);
		...
		aMemoryGraphicsDarker.mTransY += 1000;
			...
			while (IteratePlants(aPlant)) {
				if (aPlant->mSeedType == SEED_PLANTERN) {
					aMemoryGraphics.DrawImage(img, 200, 200);

					aMemoryGraphicsDarker.DrawImage(img, 520, 580);
				}
			}*/
			aMemoryGraphics.DrawImage(img, 100, 100);
	...
	offsetX += mCursorObject->mX;
	offsetY += mCursorObject->mY;

	g->mTransX += offsetX;
	g->mTransY += offsetY;

	if (mApp->mGameScene == SCENE_LEVEL_INTRO && mApp->IsDarkLevel())
	{
		g->mColor.mAlpha = TodAnimateCurve(250, 550, mDarknessCounter, targetAlpha, 0, CURVE_LINEAR);
		g->DrawImage(mDarknessImage, -200, -200);
		g->mColor.mAlpha = TodAnimateCurve(0, 250, mDarknessCounter, 0, targetAlpha, CURVE_LINEAR);
		g->DrawImage(mDarknessDarkerImage, 200, 200);
	}
	else
	{
		g->DrawImage(mDarknessImage, -200, -200);
...
void Board::DrawProgressMeter(Graphics* g)
{
	...
	{
		int pool6 = 0;
		if (StageHas6Rows()) pool6 = 1;
		SexyString aMatchStr = StrFormat(_S("%d / %d %s"), mChallenge->mChallengeScore, 5 + pool6, TodStringTranslate(_S("[BRAINS]")).c_str());
		TodDrawString(g, aMatchStr, aPosX, 589, Sexy::FONT_DWARVENTODCRAFT12, aColor, DrawStringJustification::DS_ALIGN_CENTER);
	}
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED || 
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED_TWIST ||
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBIQUARIUM || 
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_GLADIANTUAR || 
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_SNOWPEA || 
		mApp->IsSquirrelLevel() || 
		mApp->IsSlotMachineLevel() ||
		mApp->IsIZombieLevel() || 
		mApp->IsFinalBossLevel())
	...
	{
		g->DrawImageCel(Sexy::IMAGE_GIGA_PROGRESS_FLAG, aCelWidth - aHeadProgress + 580, 572, 0, 0);
	}
	...
	{
		g->DrawImageCel(Sexy::IMAGE_FLAGMETERPARTS_HARDMODE, aCelWidth - aHeadProgress + 580, 572, 0, 0);
	}
}
...
void Board::DrawHouseDoorBottom(Graphics* g)
{
	...
	//case BackgroundType::BACKGROUND_POOL_PARTY:	g->DrawImage(Sexy::IMAGE_BACKGROUND1_GAMEOVER_INTERIOR_OVERLAY, -126, 225);		break;
	default:																													break;
	}
}
...
void Board::DrawHouseDoorTop(Graphics* g)
{
	...
	{
		g->DrawImage(Sexy::IMAGE_BACKGROUND_GLADIANTUAR_GAMEOVER_MASK, -210, 161);
		return;
	}
	switch (mBackground)
	{
	case BackgroundType::BACKGROUND_POOL_PARTY: g->DrawImage(Sexy::IMAGE_BACKGROUNDPP_GAMEOVER_MASK, -210, 161);	break;
	...
	case BackgroundType::BACKGROUND_7_FOREST:	g->DrawImage(Sexy::IMAGE_BACKGROUND7_GAMEOVER_MASK, -220, 190);		break;
	default:																										break;
	}
}
...
void Board::DrawLevel(Graphics* g)
{
	...
	TodDrawString(g, aLevelStr, aPosX, aPosY, Sexy::FONT_LUCKFIXHOUSEOFTERROR15, Color(224, 187, 98), DrawStringJustification::DS_ALIGN_RIGHT);
}
...
{
	if (mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN && mApp->mGameMode != GameMode::GAMEMODE_TREE_OF_WISDOM && mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES &&
		((!mApp->IsVersusLevel() || !mApp->mIsKeyboardPlayer1 || mApp->mDebugKeysEnabled)))
	{
		if (mShowShovel)
		{
			...
			if (mCursorObject->mCursorType != CursorType::CURSOR_TYPE_SHOVEL)
			{
				...
				if (mApp->IsVersusLevel())
				{
					g->DrawImage(Sexy::IMAGE_SHOVEL_ROTATED, aShovelRect.mX - 7, aShovelRect.mY - 20);
				}
				else g->DrawImage(Sexy::IMAGE_SHOVEL, aShovelRect.mX - 7, aShovelRect.mY - 3);
				g->SetColorizeImages(false);
			}						
		}
	}
	...
	if (((mBackground != BackgroundType::BACKGROUND_GREENHOUSE && mBackground != BackgroundType::BACKGROUND_MUSHROOM_GARDEN && mBackground != BackgroundType::BACKGROUND_TREEOFWISDOM &&
		mBackground != BackgroundType::BACKGROUND_ZOMBIQUARIUM && !mApp->IsVersusLevel())) && (mShowShovel || mApp->mGameMode == GAMEMODE_CHALLENGE_WALLNUT_BOWLING ||
			mApp->mGameMode == GAMEMODE_CHALLENGE_WALLNUT_BOWLING_2 || //|| mApp->IsWallnutBowlingLevel() || //mApp->mGameMode == GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES ||
			mApp->IsIZombieLevel())) spawnSpeed = true;
}

void Board::SpawnSpeed()
{
	if (mApp->mGameMode != GAMEMODE_CHALLENGE_ZEN_GARDEN && mApp->mGameMode != GAMEMODE_TREE_OF_WISDOM && !mApp->IsVersusLevel()) spawnSpeed = true;
}

void Board::DrawSpeed(Graphics* g, bool toggle)
{
	if (spawnSpeed)
	{
		if (toggle) g->DrawImage(Sexy::IMAGE_FF2, 740, 33);
		else if (!toggle) g->DrawImage(Sexy::IMAGE_FF1, 740, 33);
		g->SetColorizeImages(false);
	}
	else return;
}
...
void Board::DrawDebugText(Graphics* g)
{
	...
	/*for (int aWave = 0; aWave < mNumWaves; aWave++)
	{
		...
		{
			ZombieType aZombieType = mZombiesInWave[aWave][aZombieIndex];
			if (aZombieType == ZombieType::ZOMBIE_INVALID)
			{
				break;
			}
			SexyString aZombieName = StrFormat(_S("[%s]"), GetZombieDefinition(aZombieType).mZombieName);
			Graphics aNameG(*g);
			aNameG.mScaleX *= 0.5f;
			TodDrawString(&aNameG, aZombieName, aWave * 100 - 100, 50 + aZombieIndex * 50, Sexy::FONT_HOUSEOFTERROR16, Color::White, DS_ALIGN_CENTER);
		}
	}*/
	
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_BOMBALL && mSpawnBombAll == true)
	{
		g->DrawImage(Sexy::IMAGE_BOMBALL, 15, 5);
		TodDrawString(g, to_string((int)mBombAllCounter), 54, 80, Sexy::FONT_HOUSEOFTERROR20, Color::White, DS_ALIGN_CENTER);
	}
	if (mApp->IsVersusLevel() && mSpawnBombAll == true)
	{
		if (!mCutScene->mContinueAfterLoss)
		{
			g->DrawImage(Sexy::IMAGE_TARGET, 500, 540);
			if (mTargetCounter >= 1) g->DrawImage(Sexy::IMAGE_CROSS, 530, 540);
			if (mTargetCounter >= 2) g->DrawImage(Sexy::IMAGE_CROSS, 575, 540);
			if (mTargetCounter >= 3) g->DrawImage(Sexy::IMAGE_CROSS, 620, 540);
		}
		if (mApp->mGameMode != GAMEMODE_CHALLENGE_VERSUS_OLD)
		{
			string aZero = "";
			if ((mBoardAge / 100) % 60 < 10) aZero = "0";
			g->DrawImageCel(Sexy::IMAGE_ICON_NEW, 355, 565, 3);
			if (mBoardAge < 30000)
			{
				TodDrawString(g, to_string(((int)mBoardAge / 100) / 60) + ":" + aZero + to_string(((int)mBoardAge / 100) % 60), 410, 598, Sexy::FONT_HOUSEOFTERROR20, Color::White, DS_ALIGN_CENTER);
			}
			else
			{
				TodDrawString(g, to_string(((int)mBoardAge / 100) / 60) + ":" + aZero + to_string(((int)mBoardAge / 100) % 60), 410, 598, Sexy::FONT_HOUSEOFTERROR20, Color::Red, DS_ALIGN_CENTER);
			}
		}
	}
}
...
void Board::DrawDebugObjectRects(Graphics* g)
{
	...
	}
	{
		...
		while (IterateGridItems(aGridItem))
		{
			Rect aAttackRect = aGridItem->GetGridItemRect();
			g->SetColor(Color(255, 0, 0));
			g->DrawRect(aAttackRect);
		}
	}
...
void Board::DrawTopRightUI(Graphics* g)
{
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN)
	{
		if (mChallenge->mChallengeState == STATECHALLENGE_ZEN_FADING)
		{
			...
			//mSpeedButton->mX = TodAnimateCurve(50, 0, mChallenge->mChallengeStateCounter, 678, 800, TodCurves::CURVE_EASE_IN_OUT);
		}
		else
		{
			mMenuButton->mY = -10;
...
void Board::DrawUIBottom(Graphics* g)
{
	...
	if (mApp->mGameScene != GameScenes::SCENE_ZOMBIES_WON && !mCutScene->mContinueAfterLoss && !mLevelAwardSpawned && mBoardAge >= 30000)
	{
		g->DrawImage(Sexy::IMAGE_CROSS, 85, 15);
		g->DrawImage(Sexy::IMAGE_CROSS, 410, 15);
	}
}
...
void Board::DrawUICoinBank(Graphics* g)
{
	...
	g->SetColor(Color(255, 255, 255, anAlpha));
	g->DrawImage(Sexy::IMAGE_COINBANK, aPosX + adjustX, aPosY + adjustY);
	...
	g->DrawString(aCoinLabel, aPosX + 116 + adjustX - Sexy::FONT_CONTINUUMBOLD14->StringWidth(aCoinLabel), aPosY + 24 + adjustY);
	g->SetColorizeImages(false);
}
...
void Board::UpdateFog()
{
	...
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_VERSUS_FOG)
	{
		int aLeft = LeftFogColumn();
		for (int x = aLeft; x < aLeft + 3; x++)
		{
	...
	while (IteratePlants(aPlant))
	{
		...
		if (aPlant->mSeedType == SeedType::SEED_PLANTERN || aPlant->mSeedType == SeedType::SEED_FLAMEWOOD || aPlant->mSeedType == SeedType::SEED_PLASMAPEA)
		{
...
void Board::DrawUITop(Graphics* g)
{
	...
	if (mApp->mGameScene == SCENE_PLAYING)
		mPlayer2->Draw(g); //Draws the plant & the selector
}
...
void Board::SetForestMowerMode()
{
	LawnMower* aLawnMower = nullptr;
	while (IterateLawnMowers(aLawnMower))
	{
		aLawnMower->EnableForestMower();
	}
}
...
void Board::KeyDown(KeyCode theKey)
{
	...
	else if (theKey == KeyCode::KEYCODE_RETURN || theKey == KeyCode::KEYCODE_SPACE)
	{
		...
		else if (theKey == KeyCode::KEYCODE_SPACE && mApp->CanPauseNow() && !mApp->IsVersusLevel())
		{
			mApp->PlaySample(Sexy::SOUND_PAUSE);
			mApp->DoPauseDialog();
		}
...
void Board::KeyChar(SexyChar theChar)
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN)
	{
		...
		if (theChar == _S(']'))
		{
			...
			if (aPottedPlant)
			{
				...
				if (aPottedPlant->mSeedType == SeedType::SEED_FLOWERPOT || aPottedPlant->mSeedType == SeedType::SEED_WATERPOT)
				{
	...
	else if (theChar == _S('z') )//&& (mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_VERSUS))
	{
	...
	if (theChar == _S('q') && (!mApp->IsVersusLevel() && mApp->mPlayerInfo->mIsNotCoop))
	{
		...
		else
		{
			...
			for (int y = 0; y < MAX_GRID_SIZE_Y; ++y)
			{
				for (int x = 0; x < MAX_GRID_SIZE_X; ++x)
				{
					...
					if (StageHasRoof() && CanPlantAt(x, y, SeedType::SEED_WATERPOT) == PlantingReason::PLANTING_OK)
					{
						AddPlant(x, y, SeedType::SEED_WATERPOT, SeedType::SEED_NONE);
					}
					if (CanPlantAt(x, y, SeedType::SEED_LILYPAD) == PlantingReason::PLANTING_OK)
					{
						AddPlant(x, y, SeedType::SEED_LILYPAD, SeedType::SEED_NONE);
	...
	if (!mApp->IsVersusLevel() && mApp->mPlayerInfo->mIsNotCoop)
	{
		if (theChar == _S('O'))
		{
			...
			for (int y = 0; y < MAX_GRID_SIZE_Y; y++)
			{
				for (int x = 0; x < 3; x++)
				{
					...
					if (CanPlantAt(x, y, SeedType::SEED_WATERPOT) == PlantingReason::PLANTING_OK)
					{
						AddPlant(x, y, SeedType::SEED_WATERPOT, SeedType::SEED_NONE);
					}
				}
		...
		if (theChar == _S('s'))
		{
			AddZombie(ZombieType::ZOMBIE_SCREENDOOR, Zombie::ZOMBIE_WAVE_DEBUG);
			return;
		}
		if (theChar == _S('L'))
		{
		...
		if (theChar == _S('y'))
		{
			AddZombie(ZombieType::ZOMBIE_HUNTER, Zombie::ZOMBIE_WAVE_DEBUG);
			return;
		}
		if (theChar == _S('a'))
		{
			AddZombie(ZombieType::ZOMBIE_LUMBERJACK, Zombie::ZOMBIE_WAVE_DEBUG);
			return;
		}
		if (theChar == _S('x'))
		{
			AddZombie(ZombieType::ZOMBIE_SKELETON, Zombie::ZOMBIE_WAVE_DEBUG);
			return;
		}
		if (theChar == _S('w'))
		{
			AddZombie(ZombieType::ZOMBIE_FISHERMAN, Zombie::ZOMBIE_WAVE_DEBUG);
			return;
		}
		if (theChar == _S('F'))
		{
		...
		if (theChar == _S('c'))
		{
			AddZombie(ZombieType::ZOMBIE_GLADIANTUAR, Zombie::ZOMBIE_WAVE_DEBUG);
			return;
		}
		if (theChar == _S('m'))
		{
		...
		if (theChar == _S('h'))
		{
			AddZombie(ZombieType::ZOMBIE_BUCKETHEAD, Zombie::ZOMBIE_WAVE_DEBUG);
			return;
		}
		//if (theChar == _S('H')
		//{
		...
		if (theChar == _S('i'))
		{
			AddZombie(ZombieType::ZOMBIE_ZOMBONI, Zombie::ZOMBIE_WAVE_DEBUG);
			return;
		}
		if (theChar == _S('C'))
		{
...
void Board::AddBrainMoney(int theAmount)
{
	mBrainMoney += theAmount;
	if (mBrainMoney > 9990)
	{
		mBrainMoney = 9990;
	}
...
int Board::CountBrainBeingCollected()
{
	int aCount = 0;
	Coin* aCoin = nullptr;
	while (IterateCoins(aCoin))
	{
		if (aCoin->mIsBeingCollected && aCoin->IsBrain())
		{
			aCount += 25;
		}
	}
...
bool Board::TakeBrainMoney(int theAmount)
{
	if (CanTakeBrainMoney(theAmount))
	{
		mBrainMoney -= theAmount;
		return true;
	}
...
bool Board::CanTakeBrainMoney(int theAmount)
{
	return theAmount <= mBrainMoney + CountBrainBeingCollected();
}
...
bool Board::HasConveyorBeltSeedBank()
{
	return
		...
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BOMBALL || 
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WOOD || 
...
int Board::GetNumSeedsInBank()
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_FLIPPED)
	{
		return 9;
	}
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_TEST)
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_PLANTS_VS_PLANTS)
	{
		return 7;
	}
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES)
	{
		return 7;
	}
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED_TWIST)
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_11 || mApp->mGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_12 ||
		mApp->mGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_14)
	{
		return 5;
	}
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_19)
	{
		return 8;
	}
...
bool Board::StageIsMoon()
{
	return
		mBackground == BackgroundType::BACKGROUND_6_BOSS;
}

bool Board::StageIsZombotany()
{
	return
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS_2 ||
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS_3 || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS_4;
}
...
bool Board::StageHasGraveStones()
{
	...
	return mBackground == BackgroundType::BACKGROUND_2_NIGHT || mBackground == BackgroundType::BACKGROUND_7_FOREST;
}
...
bool Board::StageHasShadow()
{
	return mBackground == BackgroundType::BACKGROUND_7_FOREST;
}
...
{
	return (mBackground == BackgroundType::BACKGROUND_3_POOL || mBackground == BackgroundType::BACKGROUND_4_FOG || mBackground == BackgroundType::BACKGROUND_HIGHGROUND);
}
...
bool Board::StageHasFog()
{
	return !mApp->IsStormyNightLevel() && mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_INVISIGHOUL && mBackground == BackgroundType::BACKGROUND_4_FOG &&
			mApp->mGameMode != GameMode::GAMEMODE_SCARY_POTTER_19 && mApp->mGameMode != GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_14;
}

bool Board::StageHasDarkness()
{
	return (mApp->IsAdventureMode() && mLevel == 65) || mApp->mGameMode == GAMEMODE_CHALLENGE_DARK;
}
...
int Board::LeftFogColumn()
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_VERSUS_FOG)		return 3;
...
//0x41C210
int Board::GetSeedPacketPositionX(int theIndex, bool P1)
{
	...
	if (P1)
	{
		if (mApp->IsVersusLevel()) return theIndex * 51 + 69;
	...
	{
		if (mApp->IsVersusLevel()) return theIndex * 51 + 79 + 315;

		if (mSeedBank->mNumPackets <= 7)		return theIndex * 59 + 85 + 325;
		else if (mSeedBank->mNumPackets == 8)	return theIndex * 54 + 81 + 325;
		else if (mSeedBank->mNumPackets == 9)	return theIndex * 52 + 80 + 325;
		else						 			return theIndex * 51 + 79 + 325;
	}
...
int Board::PixelToGridY(int theX, int theY)
{
	...
	else if (StageHasPool() || StageHasHighGrounds())
	{
...
int Board::GridToPixelX(int theGridX, int theGridY)
{
	...
	//TOD_ASSERT();
	theGridY >= 0 && theGridY < MAX_GRID_SIZE_Y;
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN)
	{
		if (mBackground == BackgroundType::BACKGROUND_GREENHOUSE ||
			mBackground == BackgroundType::BACKGROUND_MUSHROOM_GARDEN || 
...
void Board::KillAllPlantsInRadius(int theX, int theY, int theRadius)
{
	...
	while (IteratePlants(aPlant))
	{
		...
		if (GetCircleRectOverlap(theX, theY, theRadius, aPlant->GetPlantRect()) && aPlant->mSeedType != SEED_GRAVE)
		{
			mPlantsEaten++;
			aPlant->Die();
		}
...
unsigned int Board::SeedNotRecommendedForLevel(SeedType theSeedType)
{
	...
	if (theSeedType == SeedType::SEED_GRAVEBUSTER && ((!StageHasGraveStones() && !mApp->IsVersusLevel()) || mBackground == BACKGROUND_7_FOREST))
	{
	...
	if (theSeedType == SeedType::SEED_FLOWERPOT && !StageHasRoof() && mApp->mGameMode != GAMEMODE_CHALLENGE_GLADIANTUAR)
	{
		SetBit(aNotRec, NotRecommend::NOT_RECOMMENDED_NEEDS_ROOF, true);
	}
	if ((theSeedType == SeedType::SEED_WATERPOT) && !StageHasRoof() && mApp->mGameMode != GAMEMODE_CHALLENGE_GLADIANTUAR && mApp->mGameMode == GAMEMODE_CHALLENGE_SEASTAR)
	{
		SetBit(aNotRec, NotRecommend::NOT_RECOMMENDED_NEEDS_ROOF, true);
	}
	if ((theSeedType == SeedType::SEED_SPIKEROCK || theSeedType == SeedType::SEED_SPIKEWEED || theSeedType == SeedType::SEED_THORNMOSS || theSeedType == SeedType::SEED_POTATOMINE) && mApp->mGameMode == GAMEMODE_CHALLENGE_SEASTAR)
	{
		SetBit(aNotRec, NotRecommend::NOT_RECOMMENDED_NEEDS_POOL, true);
	}
	if (StageHasRoof() && (theSeedType == SeedType::SEED_SPIKEWEED || theSeedType == SeedType::SEED_SPIKEROCK || theSeedType == SeedType::SEED_THORNMOSS))
	{
		SetBit(aNotRec, NotRecommend::NOT_RECOMMENDED_ON_ROOF, true);
	}
	if ((!StageHasPool() && mApp->mGameMode != GAMEMODE_CHALLENGE_SEASTAR) && Plant::IsAquatic(theSeedType))
	{
		SetBit(aNotRec, NotRecommend::NOT_RECOMMENDED_NEEDS_POOL, true);
	}
	return aNotRec;
...
void Board::DropLootPiece(int thePosX, int thePosY, int theDropFactor)
{
	if (mApp->IsFirstTimeAdventureMode())
	{
		...
		if (mLevel == 74 && mCurrentWave > 5 && !mApp->mPlayerInfo->mHasUnlockedMultiplayer && CountCoinByType(CoinType::COIN_PRESENT_MULTIPLAYER_MODE) == 0)
		{
			mApp->PlayFoley(FoleyType::FOLEY_ART_CHALLENGE);
...
{
	return !mCutScene->ShouldRunUpsellBoard() && (!mApp->IsFirstTimeAdventureMode() || mLevel >= 11) && !mApp->IsVersusLevel();
}
...
Zombie* Board::GetBossZombie()
{
	...
	while (IterateZombies(aZombie))
	{
		if (aZombie->mZombieType == ZombieType::ZOMBIE_BOSS || aZombie->mZombieType == ZombieType::ZOMBIE_GLADIANTUAR_GIGA)
		{
			return aZombie;
		}
	}
...
Plant* Board::FindAloePlant(int theGridX, int theGridY)
{
	Plant* aPlant = nullptr;
	while (IteratePlants(aPlant))
	{
		if (aPlant->mSeedType == SeedType::SEED_ALOEVERA && !aPlant->NotOnGround() && GridInRange(theGridX, theGridY, aPlant->mPlantCol, aPlant->mRow, 1, 1))
		{
			return aPlant;
		}
	}
...
Plant* Board::FindFirePlant(int theGridX, int theGridY)
{
	Plant* aPlant = nullptr;
	while (IteratePlants(aPlant))
	{
		if (aPlant->IsFirePlant() && !aPlant->NotOnGround() && GridInRange(theGridX, theGridY, aPlant->mPlantCol, aPlant->mRow, 1, 1))
		{
			return aPlant;
		}
	}
...
Plant* Board::FindAmpliflower(int theGridX, int theGridY)
{
	Plant* aPlant = nullptr;
	while (IteratePlants(aPlant))
	{
		if (aPlant->mSeedType == SeedType::SEED_AMPLI_FLOWER && !aPlant->NotOnGround() && GridInRange(theGridX, theGridY, aPlant->mPlantCol, aPlant->mRow, 1, 1) && aPlant->mApliflowerCounter > 0)
		{
			return aPlant;
		}
	}
...
void Board::DoFwooshChillyPepper(int theRow)
{
	int aRenderOrder = MakeRenderOrder(RenderLayer::RENDER_LAYER_PARTICLE, theRow, 1);
	for (int i = 0; i < 12; i++)
	{
		...
		float aPosX = 750.0f * i / 11.0f + 10.0f;
		float aPosY = GetPosYBasedOnRow(aPosX + 10.0f, theRow) - 10.0f;
		Reanimation* aFwoosh = mApp->AddReanimation(aPosX, aPosY, aRenderOrder, ReanimationType::REANIM_CHILOOSH);
...
void Board::DoFwooshChilly(int theRow, int theColumn)
{
	int aRenderOrder = MakeRenderOrder(RenderLayer::RENDER_LAYER_PARTICLE, theRow, 1);
	for (int i = 0; i < 9; i++)
	{
		...
		float aPosX = 750.0f * i / 11.0f + 10.0f;
		float aPosY = 80.0f * (theColumn+1);
		Reanimation* aFwoosh = mApp->AddReanimation(aPosY, aPosX, aRenderOrder, ReanimationType::REANIM_PICKLED_FIRE);
		aFwoosh->SetFramesForLayer("anim_flame");
		aFwoosh->mLoopType = ReanimLoopType::REANIM_LOOP_FULL_LAST_FRAME;
		aFwoosh->mAnimRate *= RandRangeFloat(0.7f, 1.3f);
		...
		float aFlip = Rand(2) ? 1.0f : -1.0f;
		aFwoosh->OverrideScale(aScale * aFlip, 1);
		//aFwoosh->mColorOverride = Color(180, 250, 50, 255);
...
void Board::UpdateGridItems()
{
	...
	while (IterateGridItems(aGridItem))
	{
		if (mEnableGraveStones && (aGridItem->mGridItemType == GridItemType::GRIDITEM_GRAVESTONE || aGridItem->mGridItemType == GridItemType::GRIDITEM_BUSH || aGridItem->mGridItemType == GridItemType::GRIDITEM_WOOD_LOG) && aGridItem->mGridItemCounter < 100)
		{
...
bool Board::PlantingRequirementsMet(SeedType theSeedType)
{
	...
	case SeedType::SEED_SUPERCHOMP:			return CountPlantByType(SeedType::SEED_CHOMPER);
	case SeedType::SEED_ICYFUME:			return CountPlantByType(SeedType::SEED_FUMESHROOM);
	case SeedType::SEED_QUEENPEA:			return CountPlantByType(SeedType::SEED_SNOWPEA);
	case SeedType::SEED_PLASMAPEA:			return CountPlantByType(SeedType::SEED_FLAMEPEA);
	case SeedType::SEED_SHOOTINGSTAR:		return CountPlantByType(SeedType::SEED_STARFRUIT) || CountPlantByType(SeedType::SEED_PINKSTAR);
	case SeedType::SEED_FLAMEWOOD:			return CountPlantByType(SeedType::SEED_TORCHWOOD);
	case SeedType::SEED_VINENUT:			return CountPlantByType(SeedType::SEED_TALLNUT);
	case SeedType::SEED_THORNMOSS:			return CountPlantByType(SeedType::SEED_SPIKEWEED);
	case SeedType::SEED_BRAVESHROOM:		return CountPlantByType(SeedType::SEED_SCAREDYSHROOM);
	case SeedType::SEED_ECACTUS:			return CountPlantByType(SeedType::SEED_CACTUS);
	default:								return true;
	}
}

//0x41D8A0
void Board::KillAllZombiesInRadius(int theRow, int theX, int theY, int theRadius, int theRowRange, bool theBurn, int theDamageRangeFlags, int doom, int damage, bool theHypnotize)
{
	Plant* aPlant = nullptr;
	while (IteratePlants(aPlant))
	{
		...
		if (aPlant->mSeedType == SEED_DUMMY)
			continue;

		Rect aPlantRect = aPlant->GetPlantRect();
		int aRowDist = aPlant->mRow - theRow;

		if (aRowDist <= theRowRange && aRowDist >= -theRowRange && GetCircleRectOverlap(theX, theY, theRadius, aPlantRect) && aPlant->mHypnotized && !theHypnotize)
		{
			aPlant->Die();
		}
		if (aRowDist <= theRowRange && aRowDist >= -theRowRange && GetCircleRectOverlap(theX, theY, theRadius, aPlantRect) && !aPlant->mHypnotized && theHypnotize)
	...
	while (IterateZombies(aZombie))
	{
		if (aZombie->EffectedByDamage(theDamageRangeFlags) && (aZombie->mZombieType != ZOMBIE_GLADIANTUAR_GIGA || doom != 1) && aZombie->mZombieType != ZOMBIE_TARGET)
		{
	...
	while (IterateGridItems(aGridItem))
	{
		if ((aGridItem->mGridItemType == GridItemType::GRIDITEM_LADDER || aGridItem->mGridItemType == GridItemType::GRIDITEM_WOOD_LOG || aGridItem->mGridItemType == GridItemType::GRIDITEM_BUSH))
		{
			if (GridInRange(aGridItem->mGridX, aGridItem->mGridY, aGridX, aGridY, theRowRange, theRowRange))
			{
				aGridItem->GridItemDie();
				mApp->AddReanimation(GridToPixelX(aGridItem->mGridX, aGridItem->mGridY) - 30.0f, GridToPixelY(aGridItem->mGridX, aGridItem->mGridY) - 15.0f, aGridItem->mRenderOrder, REANIM_ZOMBIE_CHARRED_BUSH);
			}
		}
	}
}

void Board::ShrinkAllZombiesInRadius(int theRow, int theX, int theY, int theRadius, int theRowRange, bool theBurn, int theDamageRangeFlags, int doom)
{
	Zombie* aZombie = nullptr;
	while (IterateZombies(aZombie))
	{
...
void Board::IncreaseDummyCounter(bool theHypnotize)
{
	if (theHypnotize)
		mDummyCounterEnemy++;
	else
...
bool Board::CheckZombiePack(SeedType theSeedtype)
{
	return SeedPacket::IsZombiePack(theSeedtype);
}

Plant* Board::GetPlantAt(int theGridX, int theGridY)
{
	Plant* aPlant = nullptr;
	while (IteratePlants(aPlant))
	{
...
void Board::SpawnHypnotizedPlants()
{
	if (mBoardAge <= 6000)
	{
		SeedType aSeedType = SEED_NONE;
		int plantIndex = RandRangeInt(0, 2);
		int plantColumn = 0;
		int plantRow = RandRangeInt(0, 4);

		if (plantIndex == 0)
		{
			aSeedType = SEED_WALLNUT;
			plantColumn = 5;
		}
		else
		{
			aSeedType = SEED_PEASHOOTER;
			plantColumn = RandRangeInt(6, 8);
		}

		Plant* aDeletePlant = nullptr;
		while (IteratePlants(aDeletePlant))
		{
			if (!aDeletePlant->mHypnotized && aDeletePlant->mPlantCol >= 4)
				aDeletePlant->Die();
		}

		while (CanPlantAt(plantColumn, plantRow, aSeedType))
		{
			plantIndex = RandRangeInt(0, 2);
			plantColumn = 0;
			plantRow = RandRangeInt(0, 4);
		...
		Plant* aPlant = AddPlant(plantColumn, plantRow, aSeedType, SEED_NONE);
		aPlant->mHypnotized = true;
	}
	else if (mBoardAge <= 12000)
	{
		SeedType aSeedType = SEED_NONE;
		int plantIndex = RandRangeInt(0, 2);
		int plantColumn = 0;
		...
		if (plantIndex == 0)
		{
			aSeedType = SEED_SNOWPEA;
		}
		else
		{
			aSeedType = SEED_REPEATER;
		}
		plantColumn = RandRangeInt(6, 8);
	...
	else if (mBoardAge <= 18000)
	{
		if (RandRangeInt(0, 2) == 0)
		{
			//---------------------------------------------------------------------------------------------------------------------------

			int plantColumn = 4;
			int plantRow = RandRangeInt(1, 3);
			...
			Plant* aPlant = AddPlant(plantColumn, plantRow, SEED_CHERRYBOMB, SEED_NONE);
			aPlant->mHypnotized = true;
		...
		else
		{
			...
			aPlant = AddPlant(plantColumn, plantRow, aSeedType, SEED_NONE);
			aPlant->mHypnotized = true;

			//---------------------------------------------------------------------------------------------------------------------------
		}
	}
	else if (mBoardAge == 20000)
	{
		ClearAdviceImmediately();
		DisplayAdviceAgain(_S("[ADVICE_HUGE_WAVE_PLANTS]"), MessageStyle::MESSAGE_STYLE_HUGE_WAVE, AdviceType::ADVICE_HUGE_WAVE);
		mApp->PlaySample(Sexy::SOUND_AWOOGA);
		mApp->PlaySample(Sexy::SOUND_SIREN);
		...
		while (CanPlantAt(plantColumn, plantRow, SEED_WALLNUT))
		{
			plantColumn = 5;
			plantRow = RandRangeInt(0, 4);
		}

		aPlant = AddPlant(plantColumn, plantRow, SEED_WALLNUT, SEED_NONE);
		aPlant->mHypnotized = true;
		...
		while (CanPlantAt(plantColumn, plantRow, SEED_REPEATER))
		{
			plantColumn = RandRangeInt(6, 8);
			plantRow = RandRangeInt(0, 4);
		}

		aPlant = AddPlant(plantColumn, plantRow, SEED_REPEATER, SEED_NONE);
		aPlant->mHypnotized = true;
		...
		while (CanPlantAt(plantColumn, plantRow, SEED_SNOWPEA))
		{
			plantColumn = RandRangeInt(6, 8);
			plantRow = RandRangeInt(0, 4);
		}

		aPlant = AddPlant(plantColumn, plantRow, SEED_SNOWPEA, SEED_NONE);
		aPlant->mHypnotized = true;
	...
	else
	{
		...
		{
			int amountPeas = 0;
			int amountWallnuts = 0;
			Plant* aCountPlant = nullptr;
			while (IteratePlants(aCountPlant))
			{
				if (aCountPlant->mHypnotized && (aCountPlant->mSeedType == SEED_REPEATER || aCountPlant->mSeedType == SEED_SNOWPEA || aCountPlant->mSeedType == SEED_PEASHOOTER))
					amountPeas++;

				if (aCountPlant->mHypnotized && aCountPlant->mSeedType == SEED_WALLNUT)
					amountWallnuts++;
			}

			//---------------------------------------------------------------------------------------------------------------------------

			if (amountPeas < 15 && amountWallnuts < 5)
			{
				SeedType aSeedType = SEED_NONE;
				int plantIndex = RandRangeInt(0, 2);
				int plantColumn = 0;
				...
				Plant* aPlant = AddPlant(plantColumn, plantRow, aSeedType, SEED_NONE);
				aPlant->mHypnotized = true;
				if (aSeedType == SEED_PEASHOOTER) amountPeas++;
			}
