...
CutScene::CutScene()
{
	...
	mSpawnIceZombies = false;
	mContinueAfterLoss = false;
}
...
void CutScene::PlaceAZombie(ZombieType theZombieType, int theGridX, int theGridY)
{
	...
	if (theZombieType == ZombieType::ZOMBIE_DUCKY_TUBE && mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_GIGA)
	{
	...
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_FLIPPED)
	{
		if (Rand(8) == 0)
		{
			aZombie->mPosY = RandRangeFloat(250, 350);
			aZombie->mPosX = RandRangeFloat(750, 800);
		}
		else if (Rand(2) == 0)
		{
			aZombie->mPosY = RandRangeFloat(475, 525);
			aZombie->mPosX = RandRangeFloat(900, 1100);
		}
		else
		{
			aZombie->mPosY = RandRangeFloat(-25, 100);
			aZombie->mPosX = RandRangeFloat(950, 1050);
		}
	}
	...
	//if (mApp->IsBloodMoonLevel() && theZombieType != ZOMBIE_GLADIANTUAR) aZombie->mRageCounter = INT_MAX;
}
...
bool CutScene::CanZombieGoInGridSpot(ZombieType theZombieType, int theGridX, int theGridY, bool theZombieGrid[5][5])
{
	...
	if (Is2x2Zombie(theZombieType) || 
		theZombieType == ZombieType::ZOMBIE_ZOMBONI || 
...
void CutScene::PreloadResources()
{
	...
	if (mApp->IsFirstTimeAdventureMode() && mBoard->mLevel <= 70)
	{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_UPSELL)
	{
		...
		Zombie::PreloadZombieResources(ZombieType::ZOMBIE_BASIC);
		Zombie::PreloadZombieResources(ZombieType::ZOMBIE_CONEHEAD);
		Zombie::PreloadZombieResources(ZombieType::ZOMBIE_BUCKETHEAD);
		Zombie::PreloadZombieResources(ZombieType::ZOMBIE_ZOMBONI);
...
void CutScene::PlaceStreetZombies()
{
	...
	{
		if (aZombieTypeCount[(int)aZombieType] && (Is2x2Zombie(aZombieType) || aZombieType == ZombieType::ZOMBIE_ZOMBONI))
		{
...
void CutScene::PlaceLawnItems()
{
	...
	if (!IsSurvivalRepick())
	{
		...
		/*if (mBoard->mBackground == BackgroundType::BACKGROUND_POOL_PARTY)
		{
			for (int x = 0; x < 3; x++)
			{
				for (int y = 0; y < MAX_GRID_SIZE_Y; y++)
				{
					if (mApp->mGameMode == GAMEMODE_CHALLENGE_SEASTAR && x > 0 && y == 2)
						continue;
...
bool CutScene::IsNonScrollingCutscene()
{
	return
		...
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WALLNUT_BOWLING ||
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WALLNUT_BOWLING_2 ||
		mBoard->mLevel == 5 ||
...
void CutScene::StartLevelIntro()
{
	...
	{
		mCrazyDaveDialogStart = 3501;
	}
	else if (mApp->IsFirstTimeAdventureMode() && aLevel == 55)
	{
		mCrazyDaveDialogStart = 3601;
	}
	else if (mApp->IsFirstTimeAdventureMode() && aLevel == 61)
	{
		mCrazyDaveDialogStart = 3801;
	}
	else if (mApp->IsFirstTimeAdventureMode() && aLevel == 65)
	{
		mCrazyDaveDialogStart = 3901;
	}
	...
	if (/*mBoard->mBackground == BackgroundType::BACKGROUND_POOL_PARTY ||*/ mBoard->StageHasRoof() || mApp->IsFinalBossLevel() || mApp->IsScaryPotterLevel() || mApp->IsWallnutBowlingLevel())
	{
		PreloadResources();
		PlaceLawnItems();
	}
	...
	if (mCrazyDaveTime <= 0 && mApp->mGameMode != GameMode::GAMEMODE_INTRO)
	{
		...
		else if (mApp->IsAdventureMode())
		{
			...
			else if (mBoard->mBackground == BackgroundType::BACKGROUND_7_FOREST)
			{
				aHouseMessage = TodStringTranslate(_S("[PLAYERS_FOREST]"));
			}
			else
			{
				TOD_ASSERT();
		...
		else aHouseMessage = mApp->GetCurrentChallengeDef().mChallengeName;
	}
	...
	{
		//if (mApp->IsVersusLevel()) mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_VERSUS_LOBBY);
...
void CutScene::AddFlowerPots()
{
	...
	else if (mBoard->mLevel == 42 || mBoard->mLevel == 52 || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_VERSUS_ROOF || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_VERSUS_MOON)
	{
		aPotColumns = 4;
	}
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_GLADIANTUAR)
	{
		for (int x = 0; x < 3; x++)
		{
			if (mBoard->CanPlantAt(x, 2, SeedType::SEED_FLOWERPOT) == PlantingReason::PLANTING_OK)
			{
				mBoard->NewPlant(x, 2, SeedType::SEED_FLOWERPOT, SeedType::SEED_NONE);
			}
		}
	}
}

/*void CutScene::AddBushes()
{
	int aBushColumns = 0;
	if (mBoard->mLevel == 61)
	{
	...
	for (int x = 0; x < aBushColumns; x++)
	{
		for (int y = 0; y < MAX_GRID_SIZE_Y; y++)
		{
			if (mBoard->CanPlantAt(x, y, SeedType::SEED_FLOWERPOT) == PlantingReason::PLANTING_OK)
			{
				mBoard->AddABush(8 - x, y);
			}
		}
	}
}*/
...
void CutScene::AnimateBoard()
{
	...
	{
		ReanimationType aReanimationType = REANIM_READYSETPLANT;
		if (mApp->IsVersusLevel()) aReanimationType = REANIM_READY_SET_FIGHT;
		else if (mApp->mGameMode == GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES) aReanimationType = REANIM_READY_SET_ZOMBIE;
		mApp->AddReanimation(400, 324, Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_SCREEN_FADE, 0, 0), aReanimationType);
...
void CutScene::SpawnZombies()
{
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_ICE && !mSpawnIceZombies)
	{
		Zombie* aZombie = mBoard->AddZombie(ZombieType::ZOMBIE_CONEHEAD, Zombie::ZOMBIE_WAVE_DEBUG);
		aZombie->mPosX = 580.0f;
		aZombie->mPosY = 250.0f;
		aZombie->SetRow(2);
		aZombie->HitIceTrap();
		aZombie->mIceTrapCounter = 1000000;

		aZombie = mBoard->AddZombie(ZombieType::ZOMBIE_BASIC, Zombie::ZOMBIE_WAVE_DEBUG);
		aZombie->mPosX = 415.0f;
		aZombie->mPosY = 250.0f;
		aZombie->SetRow(2);
		aZombie->HitIceTrap();
		aZombie->mIceTrapCounter = 1000000;
		...
		aZombie = mBoard->AddZombie(ZombieType::ZOMBIE_BASIC, Zombie::ZOMBIE_WAVE_DEBUG);
		aZombie->mPosX = 335.0f;
		aZombie->mPosY = 150.0f;
		aZombie->SetRow(1);
		aZombie->HitIceTrap();
		aZombie->mIceTrapCounter = 1000000;

		aZombie = mBoard->AddZombie(ZombieType::ZOMBIE_BASIC, Zombie::ZOMBIE_WAVE_DEBUG);
		aZombie->mPosX = 255.0f;
		aZombie->mPosY = 350.0f;
		aZombie->SetRow(3);
		aZombie->HitIceTrap();
		aZombie->mIceTrapCounter = 1000000;

		aZombie = mBoard->AddZombie(ZombieType::ZOMBIE_BASIC, Zombie::ZOMBIE_WAVE_DEBUG);
		aZombie->mPosX = 100.0f;
		aZombie->mPosY = 350.0f;
		aZombie->SetRow(3);
		aZombie->HitIceTrap();
		aZombie->mIceTrapCounter = 1000000;

		mSpawnIceZombies = true;
	}
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_GLADIANTUAR && !mSpawnIceZombies)
	{
		Zombie* aZombie = mBoard->AddZombie(ZombieType::ZOMBIE_GLADIANTUAR_GIGA, Zombie::ZOMBIE_WAVE_DEBUG);
		aZombie->mPosX = 580.0f;
		aZombie->mPosY = 250.0f;
		aZombie->SetRow(2);
		//aZombie->RiseFromGrave(7, 2);
		mApp->PlayFoley(FOLEY_THUMP);
		aZombie->PlayZombieReanim("anim_enter", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0, 0.0f);
		mSpawnIceZombies = true;
	}
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_SNOWPEA && !mSpawnIceZombies)
	{
		Zombie* aZombie = mBoard->AddZombie(ZombieType::ZOMBIE_SNOWPEA_BOSS, Zombie::ZOMBIE_WAVE_DEBUG);
		aZombie->mPosX = 580.0f;
		aZombie->mPosY = 250.0f;
		aZombie->SetRow(2);
		//aZombie->RiseFromGrave(7, 2);
	...
	else if ((mApp->mGameMode == GAMEMODE_CHALLENGE_VERSUS_POOL || mApp->mGameMode == GAMEMODE_CHALLENGE_VERSUS_FOG) && !mSpawnIceZombies)
	{
		for (int i = 0; i < 6; i++)
		{
			Zombie* aZombie = mBoard->AddZombie(ZombieType::ZOMBIE_TARGET, Zombie::ZOMBIE_WAVE_DEBUG);
			aZombie->mPosX = 750.0f;
			aZombie->mPosY = mBoard->GridToPixelY(8, i) - 30.0f;
			aZombie->SetRow(i);
		}
		...
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 2; j++)
			{
				mBoard->AddPlant(i, j + 2, SEED_LILYPAD, SEED_NONE);
			}
		}

		mBoard->AddPlant(0, 1, SEED_SUNFLOWER, SEED_NONE);
		mBoard->AddPlant(0, 4, SEED_SUNFLOWER, SEED_NONE);
		mBoard->AddPlant(8, 1, SEED_GRAVE, SEED_NONE);
		mBoard->AddPlant(8, 4, SEED_GRAVE, SEED_NONE);

		mBoard->mBoardAge = 0;
	}
	else if (mApp->IsVersusLevel() && !mSpawnIceZombies)
	{
		for (int i = 0; i < 5; i++)
		{
			Zombie* aZombie = mBoard->AddZombie(ZombieType::ZOMBIE_TARGET, Zombie::ZOMBIE_WAVE_DEBUG);
		...
		Plant* aPlant = mBoard->AddPlant(0, 1, SEED_SUNFLOWER, SEED_NONE);
		Plant* bPlant = mBoard->AddPlant(0, 3, SEED_SUNFLOWER, SEED_NONE);
		Plant* cPlant = mBoard->AddPlant(8, 1, SEED_GRAVE, SEED_NONE);
		Plant* dPlant = mBoard->AddPlant(8, 3, SEED_GRAVE, SEED_NONE);

		mBoard->mBoardAge = 0;
	}
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_WOOD && !mSpawnIceZombies)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				mBoard->AddAWoodLog(i + 1, j)->mGridItemCounter = 1800;
			}
		}
		mSpawnIceZombies = true;
	}
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_PLANTS_VS_PLANTS && !mSpawnIceZombies)
	{
		for (int i = 0; i < 5; i++)
		{
			Plant* aPlant = mBoard->AddPlant(0, i, SEED_DUMMY, SEED_NONE);
			aPlant->mX -= 80.0f;
			aPlant->mPlantCol--;
			Plant* aEnemyPlant = mBoard->AddPlant(8, i, SEED_DUMMY, SEED_NONE);
			aEnemyPlant->mX += 80.0f;
			aEnemyPlant->mPlantCol++;
			aEnemyPlant->mHypnotized = true;
		}
		mSpawnIceZombies = true;
	}
}
...
void CutScene::ShowShovel()
{
	...
	mBoard->SpawnSpeed();
	mBoard->mSpawnBombAll = true;
	if (mApp->IsWhackAZombieLevel() || 
		mApp->IsWallnutBowlingLevel() ||  //teleport show shovel
...
{
	// 画面滚动
	if (mCutsceneTime > LostTimePanRightStart && mCutsceneTime <= LostTimePanRightEnd && !mContinueAfterLoss)
	{
		mBoard->Move(CalcPosition(LostTimePanRightStart, LostTimePanRightEnd, 0, BOARD_OFFSET), 0);
	}
	else if (mCutsceneTime == LostTimePanRightEnd && mContinueAfterLoss)
	{
		Plant* aPlant = nullptr;
		while (mBoard->IteratePlants(aPlant))
		{
		...
		mBoard->mChallenge->SpawnLevelAward(4, 2);
	}
	else if (mCutsceneTime > LostTimePanRightStart && mCutsceneTime <= LostTimePanRightEnd && mContinueAfterLoss)
	{
		mBoard->Move(-CalcPosition(LostTimePanRightStart, LostTimePanRightEnd, -BOARD_OFFSET, 0), 0);
	}
	...
	// 啃食脑子的音效
	if ((mCutsceneTime == LostTimeBrainGraphicStart - 400 || mCutsceneTime == LostTimeBrainGraphicStart - 900) && mApp->mGameMode != GAMEMODE_CHALLENGE_FLIPPED)
	{
	...
	if (mCutsceneTime == LostTimeBrainGraphicStart)
	{
		if (!mApp->IsVersusLevel())
		{
			...
			if (mApp->mGameMode == GAMEMODE_CHALLENGE_FLIPPED) aReanimation->SetImageOverride("ZombiesWon", IMAGE_GAME_OVER_FLIPPED);
		}
	...
	if (mCutsceneTime == LostTimeEnd)
	{
		...
		{		
			if (!mContinueAfterLoss)
			{
				mContinueAfterLoss = true;
				//mApp->ReanimationGet(mZombiesWonReanimID)->ReanimationDie();
				mCutsceneTime = 1500;
			}
...
{
	if ((IsCutSceneOver() || mApp->mTodCheatKeys) && !mContinueAfterLoss)
	{
		mApp->EndLevel();
	}
}
...
void CutScene::LoadIntroBoard()
{
	...
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 460, 0);
	AddUpsellZombie(ZombieType::ZOMBIE_FOOTBALL, 680, 0);
	AddUpsellZombie(ZombieType::ZOMBIE_CONEHEAD, 730, 0);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 810, 0);
	AddUpsellZombie(ZombieType::ZOMBIE_CONEHEAD, 670, 1);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 740, 1);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 880, 1);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 500, 2);
	AddUpsellZombie(ZombieType::ZOMBIE_CONEHEAD, 680, 2);
	AddUpsellZombie(ZombieType::ZOMBIE_BUCKETHEAD, 604, 3);
	AddUpsellZombie(ZombieType::ZOMBIE_SNORKEL, 880, 3);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 600, 4);
	AddUpsellZombie(ZombieType::ZOMBIE_BUCKETHEAD, 690, 4);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 780, 4);
	AddUpsellZombie(ZombieType::ZOMBIE_CATAPULT, 730, 5);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 590, 5);
...
void CutScene::LoadUpsellBoardPool()
{
	...
	mBoard->NewPlant(7, 1, SeedType::SEED_SPIKEWEED, SeedType::SEED_NONE);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 460, 0);
	AddUpsellZombie(ZombieType::ZOMBIE_ZOMBONI, 680, 0);
	AddUpsellZombie(ZombieType::ZOMBIE_CONEHEAD, 670, 1);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 740, 1);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 500, 2);
	AddUpsellZombie(ZombieType::ZOMBIE_CONEHEAD, 680, 2);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 604, 3);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 690, 4);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 740, 4);
	AddUpsellZombie(ZombieType::ZOMBIE_BUCKETHEAD, 730, 5);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 590, 5);
...
void CutScene::LoadUpsellBoardFog()
{
	...
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 460, 0);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 680, 0);
	AddUpsellZombie(ZombieType::ZOMBIE_BALLOON, 780, 0);
	AddUpsellZombie(ZombieType::ZOMBIE_CONEHEAD, 670, 1);
	AddUpsellZombie(ZombieType::ZOMBIE_BALLOON, 640, 1);
	AddUpsellZombie(ZombieType::ZOMBIE_BUCKETHEAD, 640, 2);
	AddUpsellZombie(ZombieType::ZOMBIE_CONEHEAD, 780, 3);
	AddUpsellZombie(ZombieType::ZOMBIE_BALLOON, 704, 4);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 690, 4);
	AddUpsellZombie(ZombieType::ZOMBIE_BUCKETHEAD, 590, 5);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 740, 5);
...
void CutScene::LoadUpsellBoardRoof()
{
	...
	AddUpsellZombie(ZombieType::ZOMBIE_CONEHEAD, 670, 1);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 580, 0);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 540, 1);
	AddUpsellZombie(ZombieType::ZOMBIE_BUCKETHEAD, 500, 1);
	AddUpsellZombie(ZombieType::ZOMBIE_BUCKETHEAD, 640, 2);
	AddUpsellZombie(ZombieType::ZOMBIE_CONEHEAD, 780, 3);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 380, 3);
	AddUpsellZombie(ZombieType::ZOMBIE_CATAPULT, 704, 4);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 690, 4);
	AddUpsellZombie(ZombieType::ZOMBIE_BASIC, 590, 4);
