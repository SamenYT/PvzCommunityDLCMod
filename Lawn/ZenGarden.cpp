...
void ZenGarden::AddHappyEffect(Plant* thePlant)
{
    ...
    {
        thePlant->AddAttachedParticle(thePlant->mX + 40, thePlant->mY + 60, thePlant->mRenderOrder - 1, ParticleEffect::PARTICLE_POTTED_ZEN_GLOW, false);
    }
    else if (Plant::IsAquatic(thePlant->mSeedType))
    {
        aFlowerPot->AddAttachedParticle(aFlowerPot->mX + 40, aFlowerPot->mY + 61, aFlowerPot->mRenderOrder - 1, ParticleEffect::PARTICLE_POTTED_WATER_PLANT_GLOW, false);
    }
    else
    {
        aFlowerPot->AddAttachedParticle(aFlowerPot->mX + 40, aFlowerPot->mY + 63, aFlowerPot->mRenderOrder - 1, ParticleEffect::PARTICLE_POTTED_ZEN_GLOW, false);
    }
}
...
float ZenGarden::PlantPottedDrawHeightOffset(SeedType theSeedType, float theScale)
{
    ...
    else if (theSeedType == SeedType::SEED_CHOMPER)
    {
        ...
        theSeedType == SeedType::SEED_REPEATER || theSeedType == SeedType::SEED_LEFTPEATER || theSeedType == SeedType::SEED_SNOWPEA || theSeedType == SeedType::SEED_REVERSE_PEASHOOTER || theSeedType == SeedType::SEED_REVERSE_SNOWPEA ||
...
void ZenGarden::SetPlantAnimSpeed(Plant* thePlant)
{
    ...
    float aTargetRate;
    if (thePlant->mSeedType == SeedType::SEED_PEASHOOTER || thePlant->mSeedType == SeedType::SEED_SNOWPEA || thePlant->mSeedType == SeedType::SEED_REPEATER || thePlant->mSeedType == SeedType::SEED_REVERSE_PEASHOOTER || thePlant->mSeedType == SeedType::SEED_REVERSE_SNOWPEA ||
...
{
    SeedType aSeedList[50];
    int aSeedCount = 0;
    for (int i = 0; i < 50; i++)
    {
        SeedType aSeedType = (SeedType)i;
        if (aSeedType != SeedType::SEED_MARIGOLD && aSeedType != SeedType::SEED_FLOWERPOT && aSeedType != SeedType::SEED_WATERPOT)
        {
