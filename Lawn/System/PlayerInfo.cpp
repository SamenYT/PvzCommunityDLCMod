...
void PlayerInfo::SyncDetails(DataSync& theSync)
{
	...
	//Sigma
	theSync.SyncBytes(&mChallengeRecords, sizeof(mChallengeRecords));
	theSync.SyncBytes(&mPurchases, sizeof(mPurchases));
	...
	theSync.SyncBool(mIsNotCoop);
	theSync.SyncBool(mHardmodeIsOff);
...
void PlayerInfo::LoadDetails()
{
	...
	{
		Buffer aBuffer;
		std::string aFileName = GetAppDataFolder() + StrFormat("userdataDLC/user%d.dat", mId);
