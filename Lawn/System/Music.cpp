...
void Music::SetupMusicFileForTune(MusicFile theMusicFile, MusicTune theMusicTune)
{
	...
	/*case MusicTune::MUSIC_TUNE_MOON_GRAZETHEMOON:
		switch (theMusicFile) {
		case MusicFile::MUSIC_FILE_MOON_GRAZETHEMOON:		  aTrackCount = 29;	aTrackStart1 = 0;	aTrackEnd1 = 17;									break;
		case MusicFile::MUSIC_FILE_HIHATS_MOON_GRAZETHEMOON:  aTrackCount = 29;	aTrackStart1 = 16;	aTrackEnd1 = 22;									break;
		} break; */
	default:
...
//#endif
	...
	LoadSong(MusicFile::MUSIC_FILE_HIHATS, "sounds\\mainmusic.mo3");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
...
//#ifdef _DEBUG
	LoadSong(MusicFile::MUSIC_FILE_MOON_GRASSTHEMOON, "sounds\\grassthemoon.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
	LoadSong(MusicFile::MUSIC_FILE_MOON_GRASSTHEMOON_HIHATS, "sounds\\grassthemoon_hihats.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
	LoadSong(MusicFile::MUSIC_FILE_MOON_GRASSTHEMOON_DRUMS, "sounds\\grassthemoon_drums.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
//#endif
//#ifdef _DEBUG
	LoadSong(MusicFile::MUSIC_FILE_MOON_ZOMBOTANY, "sounds\\zombotany.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
//#endif
//#ifdef _DEBUG
	LoadSong(MusicFile::MUSIC_FILE_ICE, "sounds\\icelevel.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
//#endif
//#ifdef _DEBUG
	LoadSong(MusicFile::MUSIC_FILE_GLADIANTUAR, "sounds\\gladiantuarboss.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
//#endif
//#ifdef _DEBUG
	LoadSong(MusicFile::MUSIC_FILE_SNOWPEA, "sounds\\snowpea_boss.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
	//#endif
//#ifdef _DEBUG
	LoadSong(MusicFile::MUSIC_FILE_VERSUS, "sounds\\versus_mode.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
	//#endif
	// //#ifdef _DEBUG
	LoadSong(MusicFile::MUSIC_FILE_VERSUS_LOBBY, "sounds\\versusmode_lobby.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
	//#endif
//#ifdef _DEBUG
	LoadSong(MusicFile::MUSIC_FILE_FOREST, "sounds\\woodytombstone.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
	LoadSong(MusicFile::MUSIC_FILE_FOREST_HIHATS, "sounds\\woodytombstone_hihats.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
	LoadSong(MusicFile::MUSIC_FILE_FOREST_DRUMS, "sounds\\woodytombstone_drums.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
	//#endif
//#ifdef _DEBUG
	LoadSong(MusicFile::MUSIC_FILE_GLITCH, "sounds\\woody_tombstones_glitched.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
	LoadSong(MusicFile::MUSIC_FILE_BASS_GLITCH, "sounds\\woody_tombstones_horde_glitched.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
	//#endif
//#ifdef _DEBUG
	LoadSong(MusicFile::MUSIC_FILE_ZOMBIESVSZOMBIES, "sounds\\zombiesvszombies.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
//#endif
//#ifdef _DEBUG
	LoadSong(MusicFile::MUSIC_TWILLIGHT_TERROR, "sounds\\twilightterror.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
//#endif
// 	   //#ifdef _DEBUG
	LoadSong(MusicFile::MUSIC_PVP, "sounds\\plants_vs_plants.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
///#endif
//#ifdef _DEBUG
	LoadSong(MusicFile::MUSIC_FILE_POOL_PARTY, "sounds\\poolparty.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
//#endif
//#ifdef _DEBUG
	LoadSong(MusicFile::MUSIC_FILE_BOMB_ALL, "sounds\\bomball.ogg");
	mApp->mCompletedLoadingThreadTasks += /*原版*/3500;///*内测版*/800;
//#endif
}
...
void Music::PlayMusic(MusicTune theMusicTune, int theOffset, int theDrumsOffset)
{
	...
	case MusicTune::MUSIC_TUNE_MOON_GRASSTHEMOON:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_MOON_GRASSTHEMOON;
		mCurMusicFileHihats = MusicFile::MUSIC_FILE_MOON_GRASSTHEMOON_HIHATS;
		mCurMusicFileDrums = MusicFile::MUSIC_FILE_MOON_GRASSTHEMOON_DRUMS;
		if (theOffset == -1)
			theOffset = 0;
		mBaseBPM = 1;
		mBaseModSpeed = 1;
	...
	case MusicTune::MUSIC_TUNE_ZOMBOTANY:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_MOON_ZOMBOTANY;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;
	case MusicTune::MUSIC_TUNE_ICE:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_ICE;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;
	case MusicTune::MUSIC_TUNE_GLADIANTUAR:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_GLADIANTUAR;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;
	case MusicTune::MUSIC_TUNE_SNOWPEA:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_SNOWPEA;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;
	case MusicTune::MUSIC_TUNE_VERSUS:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_VERSUS;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;
	case MusicTune::MUSIC_TUNE_VERSUS_LOBBY:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_VERSUS_LOBBY;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;
	case MusicTune::MUSIC_TUNE_TWILLIGHT_TERROR:
		mCurMusicFileMain = MusicFile::MUSIC_TWILLIGHT_TERROR;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;
	case MusicTune::MUSIC_TUNE_PVP:
		mCurMusicFileMain = MusicFile::MUSIC_PVP;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;
	case MusicTune::MUSIC_TUNE_FOREST:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_FOREST;
		mCurMusicFileHihats = MusicFile::MUSIC_FILE_FOREST_HIHATS;
		mCurMusicFileDrums = MusicFile::MUSIC_FILE_FOREST_DRUMS;
		if (theOffset == -1)
			theOffset = 0;
		mBaseBPM = 1;
		mBaseModSpeed = 1;
		...
		PlayFromOffset(mCurMusicFileDrums, theOffset, 0.0);
		break;
	case MusicTune::MUSIC_TUNE_GLITCH:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_GLITCH;
		mCurMusicFileHihats = MusicFile::MUSIC_FILE_BASS_GLITCH;
		if (theOffset == -1)
			theOffset = 0;
		mBaseBPM = 1;
		mBaseModSpeed = 1;
		...
		PlayFromOffset(mCurMusicFileHihats, theOffset, 0.0);
		break;
	case MusicTune::MUSIC_TUNE_ZOMBIESVSZOMBIES:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_ZOMBIESVSZOMBIES;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;
	case MusicTune::MUSIC_TUNE_POOL_PARTY:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_POOL_PARTY;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;
	case MusicTune::MUSIC_TUNE_BOMB_ALL:
		mCurMusicFileMain = MusicFile::MUSIC_FILE_BOMB_ALL;
		if (theOffset == -1)
			theOffset = 0;
		PlayFromOffset(mCurMusicFileMain, theOffset, 1.0);
		break;
	...
	if (theMusicTune == MUSIC_TUNE_MOON_GRASSTHEMOON || theMusicTune == MUSIC_TUNE_FOREST || theMusicTune == MUSIC_TUNE_GLITCH)
	{
		return;
	}
...
{
	if (mCurMusicTune == MUSIC_TUNE_MOON_GRASSTHEMOON || mCurMusicTune == MUSIC_TUNE_FOREST || mCurMusicTune == MUSIC_TUNE_GLITCH)
	{
		return;
	}
...
void Music::UpdateMusicBurst()
{
	...
	if (mCurMusicTune == MusicTune::MUSIC_TUNE_DAY_GRASSWALK || mCurMusicTune == MusicTune::MUSIC_TUNE_POOL_WATERYGRAVES ||
		...
		mCurMusicTune == MusicTune::MUSIC_TUNE_MOON_GRASSTHEMOON || mCurMusicTune == MusicTune::MUSIC_TUNE_FOREST || 
		mCurMusicTune == MusicTune::MUSIC_TUNE_GLITCH)
		aBurstScheme = 1;
	...
	if (mCurMusicTune == MusicTune::MUSIC_TUNE_MOON_GRASSTHEMOON || mCurMusicTune == MusicTune::MUSIC_TUNE_FOREST || mCurMusicTune == MusicTune::MUSIC_TUNE_GLITCH) {
	...
	case MusicBurstState::MUSIC_BURST_OFF:
		if ((mApp->mBoard->CountZombiesOnScreen() >= 10 || mApp->IsBloodMoonLevel() && mApp->mBoard->mRageDelay > 0) && mApp->mGameMode != GameMode::GAMEMODE_INTRO || mBurstOverride == 1)
			StartBurst();
		break;
...
void Music::StartGameMusic()
{
	...
	else if (mApp->mBoard && mApp->mGameMode == GAMEMODE_CHALLENGE_PLANTS_VS_PLANTS)
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_PVP);
	else if (mApp->mBoard && mApp->mGameMode == GAMEMODE_CHALLENGE_ECLIPSE)
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_NIGHT_MOONGRAINS);
	else if (mApp->mBoard && mApp->mGameMode == GAMEMODE_CHALLENGE_GLITCH)
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GLITCH);
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_WAR_AND_PEAS)
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_DAY_GRASSWALK);
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_WAR_AND_PEAS_2)
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_POOL_WATERYGRAVES);
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_ICE)
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_ICE);
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_GLADIANTUAR)
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_GLADIANTUAR);
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_SNOWPEA)
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_SNOWPEA);
	//else if (mApp->IsVersusLevel())
		//MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_VERSUS);
	else if (mApp->IsDarkLevel())
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_TWILLIGHT_TERROR);
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES)
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_PVP); // MUSIC_TUNE_ZOMBIESVSZOMBIES
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_SEASTAR)
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_POOL_PARTY);
	else if (mApp->mGameMode == GAMEMODE_CHALLENGE_BOMBALL)
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_DAY_GRASSWALK);
	else if (mApp->mBoard->StageIsZombotany())
	...
	else if (mApp->IsWallnutBowlingLevel() || mApp->IsWhackAZombieLevel() || mApp->IsLittleTroubleLevel() || mApp->IsBungeeBlitzLevel() || mApp->IsBigTroubleLevel() ||
	...
	else if ((mApp->IsAdventureMode() && (mApp->mPlayerInfo->GetLevel() == 10 || mApp->mPlayerInfo->GetLevel() == 20 || mApp->mPlayerInfo->GetLevel() == 30)) || mApp->mPlayerInfo->GetLevel() == 50 || mApp->mPlayerInfo->GetLevel() == 70 ||
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_COLUMN)
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_CONVEYER);
	else if (mApp->mBoard && mApp->mBoard->mBackground == BackgroundType::BACKGROUND_7_FOREST)
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_FOREST);
	...
	else if (mApp->mBoard->StageHasFog() || mApp->mGameMode == GAMEMODE_CHALLENGE_INVISIGHOUL)
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_FOG_RIGORMORMIST);
	else if (mApp->mBoard->StageIsMoon())
		MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_MOON_GRASSTHEMOON);
