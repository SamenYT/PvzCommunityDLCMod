...
void CursorObject::Draw(Graphics* g)
{
    ...
    case CursorType::CURSOR_TYPE_PLANT_FROM_DUPLICATOR:
    {
        ...
        else if (mApp->IsVersusLevel() && mBoard->CheckZombiePack(aPlantingSeedType) && aPlantingSeedType != SEED_GRAVE)
        {
            aOffsetX -= 50.0f;
            aOffsetY -= 50.0f;
    ...
    if (mBoard && (mApp->IsVersusLevel() || !mApp->mPlayerInfo->mIsNotCoop))
    {
        if ((!mApp->mPlayerInfo->mIsNotCoop && !mApp->IsVersusLevel()) || (!mApp->mIsKeyboardPlayer1 && mApp->IsVersusLevel()))
        {
            g->DrawImageF(IMAGE_P1_2, 7.5f, 7.5f);
        }
        else
        {
            g->DrawImageF(IMAGE_P2_2, 7.5f, 7.5f);
            //g->DrawImageF(IMAGE_P1_3, 15.0f, 20.0f);
        }
    }
...
void CursorPreview::Draw(Graphics* g)
{
    ...
    else
    {
        ...
        Plant::DrawSeedType(&seedG, mBoard->mCursorObject->mType, mBoard->mCursorObject->mImitaterType, DrawVariation::VARIATION_NORMAL, aOffsetX, aOffsetY);
    }
    ...
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_COLUMN)
    {
        for (int y = 0; y < MAX_GRID_SIZE_Y; y++)
        {
            if (y != mGridY && mBoard->CanPlantAt(mGridX, y, aSeedType) == PlantingReason::PLANTING_OK)
            {
                ...
                Plant::DrawSeedType(&seedG, mBoard->mCursorObject->mType, mBoard->mCursorObject->mImitaterType, DrawVariation::VARIATION_NORMAL, 0.0f, aOffsetY);
            }
        }
    }
