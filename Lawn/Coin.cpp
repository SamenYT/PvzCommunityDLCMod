...
void Coin::CoinInitialize(int theX, int theY, CoinType theCoinType, CoinMotion theCoinMotion)
{
    ...
    if (IsSun())
    {
        ...
        if (mApp->mGameMode == GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES) 
             aSunReanim = mApp->AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_SUN_BRAIN);
        else aSunReanim = mApp->AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_SUN);
    ...
    else if (IsBrain())
    {
        float aPosX = mWidth * 0.5f;
        float aPosY = mHeight * 0.5f;
        Reanimation* aBrainReanim;
        aBrainReanim = mApp->AddReanimation(0.0f, 0.0f, 0, ReanimationType::REANIM_SUN_BRAIN);
        aBrainReanim->SetPosition(mPosX + aPosX, mPosY + aPosY);
        aBrainReanim->mLoopType = ReanimLoopType::REANIM_LOOP;
        aBrainReanim->mAnimRate = 6.0f;
        AttachReanim(mAttachmentID, aBrainReanim, aPosX, aPosY);
    }
    ...
    else if (mType == CoinType::COIN_TROPHY || mType == CoinType::COIN_DIAMOND_TROPHY || mType == CoinType::COIN_PLANT_TROPHY || mType == CoinType::COIN_ZOMBIE_TROPHY)
    {
    ...
    else if (mType == CoinType::COIN_GAME)
    {
        mWidth = IMAGE_GAME->GetCelWidth();
        mHeight = IMAGE_GAME->GetCelHeight();
        mRenderOrder = Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_ABOVE_UI, 0, 0);
    }
    ...
    case CoinMotion::COIN_MOTION_COIN:
        ...
        if (mType == CoinType::COIN_FINAL_SEED_PACKET || 
            ...
            mType == CoinType::COIN_PLANT_TROPHY || 
            mType == CoinType::COIN_ZOMBIE_TROPHY || 
            mType == CoinType::COIN_CARKEYS || 
            mType == CoinType::COIN_ALMANAC || 
            mType == CoinType::COIN_VASE || 
            mType == CoinType::COIN_WATERING_CAN || 
            mType == CoinType::COIN_GAME || 
            mType == CoinType::COIN_TACO || 
            mType == CoinType::COIN_RUBY || 
            mType == CoinType::COIN_GREEN || 
            mType == CoinType::COIN_WHITE || 
            mType == CoinType::COIN_PURPLE || 
            mType == CoinType::COIN_YELLOW || 
            mType == CoinType::COIN_ORANGE || 
    ...
    else if (mType == CoinType::COIN_SMALLBRAIN)
    {
        aScale = 0.5f;
    }
...
bool Coin::IsMoney(CoinType theType)
{
    ...
    return theType == CoinType::COIN_SILVER || theType == CoinType::COIN_GOLD || theType == CoinType::COIN_DIAMOND || theType == CoinType::COIN_RUBY || theType == CoinType::COIN_GREEN || theType == CoinType::COIN_WHITE || theType == CoinType::COIN_PURPLE || theType == CoinType::COIN_YELLOW || theType == CoinType::COIN_ORANGE;
}
...
bool Coin::IsBrain()
{
    return mType == CoinType::COIN_BRAIN || mType == CoinType::COIN_SMALLBRAIN;
}
...
void Coin::ScoreCoin()
{
    ...
    if (IsSun() || (IsBrain() && mApp->mGameMode == GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES))
    {
    ...
    {
        int aBrainValue = GetBrainValue();
        mBoard->AddBrainMoney(aBrainValue);
    }
    else if (IsMoney())
    {
...
void Coin::UpdateFall()
{
    ...
    else
    {
        if (mNeedsBouncyArrow && !mHasBouncyArrow)
        {
            ...
            if (mType == CoinType::COIN_TROPHY || mType == CoinType::COIN_DIAMOND_TROPHY || mType == CoinType::COIN_PLANT_TROPHY || mType == CoinType::COIN_ZOMBIE_TROPHY)
            {
...
void Coin::Update()
{
    ...
    if (!mIsBeingCollected) {
        for (int i = 0; i < 2; i++) {
            ...
            float dX = player->mX - mPosX - mWidth / 2.0f;
            float dY = player->mY - mPosY - mHeight / 2.0f;
            float distance = sqrtf(dX * dX + dY * dY);
            if (distance <= 90 && ((!mApp->mIsKeyboardPlayer1 && !IsSun()) || (mApp->mIsKeyboardPlayer1 && !IsBrain())) && mApp->IsVersusLevel()) {
                PlayCollectSound();
                Collect();
            ...
            else if (distance <= 90 && (!mApp->mPlayerInfo->mIsNotCoop && !mApp->IsVersusLevel()))
            {
                PlayCollectSound();
                Collect();
            }
...
{
    if (mApp->IsFirstTimeAdventureMode() && mBoard && mBoard->mLevel <= 70)
    {
...
void Coin::Draw(Graphics* g)
{
    ...
    else if (mType == CoinType::COIN_PLANT_TROPHY)
    {
        aImage = IMAGE_PLANT_TROPHY_HIGH_RES;
        aOffsetX -= mWidth / 2;
        aOffsetY -= mHeight / 2;
        aDrawScale *= 0.6f;
    }
    else if (mType == CoinType::COIN_ZOMBIE_TROPHY)
    {
        aImage = IMAGE_ZOMBIE_TROPHY_HIGH_RES;
        aOffsetX -= mWidth / 2;
        aOffsetY -= mHeight / 2;
        aDrawScale *= 0.6f;
    }
    ...
    else if (mType == CoinType::COIN_RUBY)
    {
        aImage = IMAGE_GEM_RED;
    }
    else if (mType == CoinType::COIN_GREEN)
    {
        aImage = IMAGE_GEM_GREEN;
        }
    else if (mType == CoinType::COIN_WHITE)
    {
        aImage = IMAGE_GEM_WHITE;
        }
    else if (mType == CoinType::COIN_PURPLE)
    {
        aImage = IMAGE_GEM_PURPLE;
        }
    else if (mType == CoinType::COIN_YELLOW)
    {
        aImage = IMAGE_GEM_YELLOW;
        }
    else if (mType == CoinType::COIN_ORANGE)
    {
        aImage = IMAGE_GEM_ORANGE;
        }
    else if (mType == CoinType::COIN_VASE)
    {
...
void Coin::Collect()
{
    ...
    if (IsLevelAward())
    {
        ...
        else if (mApp->IsFirstTimeAdventureMode() && (mBoard->mLevel == 24 || mBoard->mLevel == 34 || mBoard->mLevel == 44 || mBoard->mLevel == 54 || mBoard->mLevel == 64))
        {
        ...
        else if (mType == CoinType::COIN_PLANT_TROPHY || mType == CoinType::COIN_ZOMBIE_TROPHY)
        {
    ...
    if ((IsSun() || IsBrain() && mApp->mGameMode == GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES) && mBoard && !mBoard->HasConveyorBeltSeedBank()) //teleport sun flash
    {
        for (int i = 0; i < mBoard->mSeedBank->mNumPackets; i++)
        {
            ...
            if (aSunProfit >= 0 && aSunProfit < GetSunValue() && !SeedPacket::IsZombiePack(aSeedPacket->mPacketType))
            {
        ...
        {
            mRenderOrder = Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_ABOVE_UI, 0, 2); //teleport render order reference
        }
    }
    else if (IsBrain() && mBoard && !mBoard->HasConveyorBeltSeedBank())
    {
        for (int i = 0; i < mBoard->mSeedBank->mNumPackets; i++)
        {
            SeedPacket* aSeedPacket = &mBoard->mSeedBank->mSeedPackets[i];
            int aCost = mBoard->GetCurrentPlantCost(aSeedPacket->mPacketType, aSeedPacket->mImitaterType);
            int aBrainProfit = mBoard->mBrainMoney + mBoard->CountBrainBeingCollected() - aCost;
            if (aBrainProfit >= 0 && aBrainProfit < 25 && SeedPacket::IsZombiePack(aSeedPacket->mPacketType))
            {
                aSeedPacket->FlashIfReady();
            }
        }
...
float Coin::GetBrainScale()
{
    return mType == CoinType::COIN_SMALLBRAIN ? 0.5f : 1.0f;
}
...
int Coin::GetBrainValue()
{
    return mType == CoinType::COIN_BRAIN ? 25 : mType == CoinType::COIN_SMALLBRAIN ? 15 : 0;
}

//0x4329D0
int Coin::GetCoinValue(CoinType theCoinType)
{
    return theCoinType == CoinType::COIN_SILVER ? 1 : theCoinType == CoinType::COIN_GOLD ? 5 : theCoinType == CoinType::COIN_DIAMOND ? 100 : theCoinType == CoinType::COIN_RUBY ? 200 : theCoinType == CoinType::COIN_GREEN ? 75 : theCoinType == CoinType::COIN_WHITE ? 300 : theCoinType == CoinType::COIN_PURPLE ? 150 : theCoinType == CoinType::COIN_YELLOW ? 25 : theCoinType == CoinType::COIN_ORANGE ? 50 : 0;
}
...
void Coin::PlayCollectSound()
{
    ...
    if (mType == CoinType::COIN_DIAMOND || mType == CoinType::COIN_RUBY || mType == CoinType::COIN_GREEN || mType == CoinType::COIN_WHITE || mType == CoinType::COIN_PURPLE || mType == CoinType::COIN_YELLOW || mType == CoinType::COIN_ORANGE || mType == CoinType::COIN_PLANT_TROPHY || mType == CoinType::COIN_ZOMBIE_TROPHY)
    {
        mApp->PlaySample(SOUND_DIAMOND);
        return;
    }

    if (IsSun() && mApp->mGameMode != GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES)
    {
    ...
    {
        mApp->PlayFoleyPitch(FoleyType::FOLEY_SLURP, 10.0f);
        return;
    }
    ...
    {
        mApp->PlayFoleyPitch(FoleyType::FOLEY_SLURP, 4.0f);
    }
}
...
bool Coin::MouseHitTest(int theX, int theY, HitResult* theHitResult)
{
    ...
    if (mType == CoinType::COIN_SUN || mType == CoinType::COIN_BRAIN)
    {
        aExtraClickSize = 15;
    }
