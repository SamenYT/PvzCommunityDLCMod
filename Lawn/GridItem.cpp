...
void GridItem::DrawGridItem(Graphics* g)
{
    ...
    case GridItemType::GRIDITEM_WOOD_LOG:           DrawWoodLog(g);                                 break;
    case GridItemType::GRIDITEM_BUSH:               DrawBush(g);                                    break;
...
void GridItem::DrawWoodLog(Graphics* g)
{
    if (mGridItemCounter <= 0)
        return;
...
void GridItem::DrawBush(Sexy::Graphics* g)
{
...
void GridItem::DrawCrater(Graphics* g)
{
    ...
    else if (mBoard->StageHasRoof() || (mApp->mGameMode == GAMEMODE_CHALLENGE_GLADIANTUAR && mGridY == 2))
    {
        if (mGridX < 5)
        {
        ...
        if (mBoard->StageIsMoon() || (mApp->mGameMode == GAMEMODE_CHALLENGE_GLADIANTUAR && mGridY == 2))
        {
            g->SetColorizeImages(true);
            g->SetColor(Color(128, 0, 255, 156));
        }
    }
...
void GridItem::DrawScaryPot(Graphics* g)
{
    ...
    if (mHighlighted && !mBoard->mPaused)
    {
        g->SetDrawMode(Graphics::DRAWMODE_ADDITIVE);
        g->SetColorizeImages(true);
        if (mTransparentCounter == 0)
...
{
    if (mGridItemCounter == 1)
    {
        int randomNumber = Rand(3);
        ReanimationType aReanimType = ReanimationType::REANIM_NONE;
        if (randomNumber == 2)
        {
            if (mBoard->StageIsNight())
            {
                aReanimType = ReanimationType::REANIM_BUSH4;
            }
            else
            {
                aReanimType = ReanimationType::REANIM_BUSH1;
            }
        }
        else if (randomNumber == 1)
        {
            if (mBoard->StageIsNight())
            {
                aReanimType = ReanimationType::REANIM_BUSH5;
            }
            else
            {
                aReanimType = ReanimationType::REANIM_BUSH2;
            }
        }
        else
        {
            if (mBoard->StageIsNight())
            {
                aReanimType = ReanimationType::REANIM_BUSH6;
            }
            else
            {
                aReanimType = ReanimationType::REANIM_BUSH3;
            }
        }
        Reanimation* aReanim = mApp->AddReanimation(mGridX * 80.0f + 0.0f, mGridY * 100.0f + 80.0f, mRenderOrder, aReanimType);
        aReanim->mLoopType = ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD;
        aReanim->OverrideScale(0.0f, 0.0f);
        mGridItemReanimID = mApp->ReanimationGetID(aReanim);
    }
    else if (mGridItemCounter <= 9 && mGridItemCounter > 0)
    {
        if (mGridItemCounter == 9)
        {
            int aRand = Rand(2);
        ...
        float growNumber = (float)mGridItemCounter;
        Reanimation* aBushReanim = mApp->ReanimationTryToGet(mGridItemReanimID);
        aBushReanim->OverrideScale(growNumber / 18, growNumber / 18);
        aBushReanim->SetPosition(mGridX * 80 + (7.5f * (8 - (growNumber - 1))), mGridY * 100 + 80 + (5.0f * (8 - (growNumber - 1))));
    }
}

void GridItem::AnimateBush()
{
    Reanimation* aBushReanim = mApp->ReanimationTryToGet(mGridItemReanimID);
    if (aBushReanim)
    {
        if ((aBushReanim->mLoopCount > 0))
        {
            aBushReanim->PlayReanim("anim_rustle", REANIM_PLAY_ONCE_AND_HOLD, 10, RandRangeFloat(8.0f, 10.0f));
            int aRand = Rand(2);
            if (aRand == 0) mApp->PlayFoley(FOLEY_BUSH_RUSTLING1);
            else if (aRand == 1) mApp->PlayFoley(FOLEY_BUSH_RUSTLING2);
            else mApp->PlayFoley(FOLEY_BUSH_RUSTLING3);
...
void GridItem::Update()
{
    ...
    if (mGridItemType == GridItemType::GRIDITEM_WOOD_LOG)
    {
        UpdateWoodLog();
    }
    if (mGridItemType == GridItemType::GRIDITEM_BUSH)
    {
        UpdateBush();
    }
}
...
Rect GridItem::GetGridItemRect()
{
    Rect aGridItemRect = mGridItemRect;
    mGridItemRect.mX = 65 + 80 * mGridX;
    mGridItemRect.mY = 95 + 100 * mGridY;

    return mGridItemRect;
}
