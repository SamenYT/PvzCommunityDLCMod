...
void Zombie::UpdateZombieDancer()
{
    ...
    if (mSummonCounter > 0)
    {
        ...
        {
            bool doPoint = false;
            if (mMindControlled || mApp->mGameMode == GAMEMODE_CHALLENGE_ZOMBIESVSZOMBIES)
            {
                doPoint = mPosX > 100.0f;
            }
            else
            {
                doPoint = mPosX < 700.0f;
            }

            if (GetDancerFrame() == 12 && mHasHead && doPoint)
            {
        ...
        if (mZombiePhase == ZombiePhase::PHASE_DANCER_SNAPPING_FINGERS_HOLD)
        {
            ...
            for (int i = 0; i < NUM_BACKUP_DANCERS; i++)
            {
                ...
                if (aDancer)
                {
                    ...
                    if (!aDancer->mIsEating) aDancer->PlayZombieReanim("anim_walk", ReanimLoopType::REANIM_LOOP, 20, 0.0f);
                }
            }
        }
...
//hi riginal stats are all commented incase you want
//also i added his own speedstat in PickRandomSpeed
...
void Zombie::CheckForBoardEdge()
{
    ...
    if (!mMindControlled && mX <= aEdgeX && mHasHead)
    {
        if (mApp->IsIZombieLevel())
        {
            DieNoLoot();
...
{
    if (mMindControlled && mZombiePhase != ZombiePhase::PHASE_DANCER_DANCING_IN)
        return true;
...
void Zombie::TakeBodyDamage(int theDamage, unsigned int theDamageFlags)
{
    ...
    {
        if (mBodyHealth < mBodyMaxHealth / 2 && mHasArm)
        {
            DropArm(theDamageFlags);
        }
        if (mBodyHealth <= 0)
