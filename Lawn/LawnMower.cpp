...
void LawnMower::LawnMowerInitialize(int theRow)
{
    ...
    else if (mBoard->mBackground == BACKGROUND_7_FOREST && mApp->mPlayerInfo->mPurchases[StoreItem::STORE_ITEM_WOOD_CHIPPER])
    {
        mMowerType = LawnMowerType::LAWNMOWER_FOREST_MOWER;
        aReanimType = ReanimationType::REANIM_WOOD_CHIPPER;
    }
    else
    {
    ...
    if (mBoard->mBackground == BACKGROUND_7_FOREST)
    {
        EnableForestMower();
    }
}
...
void LawnMower::UpdateForest()
{
    GridItem* aGridItem = nullptr;
    while (mBoard->IterateGridItems(aGridItem))
    {
        if (aGridItem->mGridY == mRow && aGridItem->mGridX * 80 <= mPosX && aGridItem->mGridItemType == GRIDITEM_BUSH)
        {
            if (mMowerType == LawnMowerType::LAWNMOWER_FOREST_MOWER)
            {
                mApp->AddTodParticle(mBoard->GridToPixelX(aGridItem->mGridX, aGridItem->mGridY), mBoard->GridToPixelY(aGridItem->mGridX, aGridItem->mGridY), (int)RenderLayer::RENDER_LAYER_TOP, ParticleEffect::PARTICLE_BUSH);
            ...
            else
            {
                ...
                if (mApp->mSuperMowerMode) 
                    mApp->AddTodParticle(mPosX + 80.0f, mPosY + 60.0f, mRenderOrder + 1, ParticleEffect::PARTICLE_LAWNMOWER_EXPLOSION_TRICKED_OUT);
                else
                    mApp->AddTodParticle(mPosX + 80.0f, mPosY + 60.0f, mRenderOrder + 1, ParticleEffect::PARTICLE_LAWNMOWER_EXPLOSION);
                Die();
            }
        }
    }
...
void LawnMower::MowZombie(Zombie* theZombie)
{
    ...
    if (mMowerType == LawnMowerType::LAWNMOWER_POOL)
    {
        ...
        if (theZombie->mZombieType != ZOMBIE_TARGET) theZombie->DieWithLoot();
    }
    else
    {
        mApp->PlayFoley(FoleyType::FOLEY_SPLAT);
        if (theZombie->mZombieType != ZOMBIE_TARGET) theZombie->MowDown();
    }
}
...
{
    if ((!mApp->mPlayerInfo->mHardmodeIsOff && !mApp->IsVersusLevel()) || mApp->mGameMode == GAMEMODE_CHALLENGE_PLANTS_VS_PLANTS) Die();
    ...
    while (mBoard->IterateZombies(aZombie))
    {
            ...
            aZombie->EffectedByDamage(127U))
        {
            ...
            if (aOverlap > (aZombie->mZombieType == ZombieType::ZOMBIE_BALLOON ? 20 : 0))
            {
                ...
                {
                    if (aZombie->mZombieType == ZOMBIE_GLADIANTUAR_GIGA) SquishMower();
                    else MowZombie(aZombie);
                }
            }
        }
    }
    ...
    if (mMowerType == LawnMowerType::LAWNMOWER_FOREST_MOWER || mMowerType == LawnMowerType::LAWNMOWER_LAWN)
    {
        UpdateForest();
    }
...
void LawnMower::Draw(Graphics* g)
{
    ...
    else if (mMowerType == LawnMowerType::LAWNMOWER_FOREST_MOWER)
    {
        //aMowerGraphics.mTransY += 10.0f;
    }
    ...
    else
    {
        ...
        {
            aMowerType = LawnMowerType::LAWNMOWER_FOREST_MOWER;
        }
...
void LawnMower::StartMower()
{
    ...
    else if (mMowerType == LawnMowerType::LAWNMOWER_FOREST_MOWER)
    {
        aMowerReanim->mAnimRate = 35.0f;
        //mApp->PlayFoley(FoleyType::FOLEY_POOL_CLEANER);
        mApp->PlaySample(SOUND_WOODCHIPPER);
    }
    else
    {
...
void LawnMower::EnableForestMower()
{
    //Reanimation* aMowerReanim = mApp->ReanimationGet(mReanimID);
    //aMowerReanim->SetImageOverride("LawnMower_body", IMAGE_FOREST_MOWER_BODY);
    //aMowerReanim->SetImageOverride("LawnMower_engine", IMAGE_FOREST_MOWER_ENGINE);
}
