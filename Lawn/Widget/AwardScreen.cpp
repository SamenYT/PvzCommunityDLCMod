...
AwardScreen::AwardScreen(LawnApp* theApp, AwardType theAwardType)
{
    ...
    else if (mApp->IsAdventureMode())
    {
        ...
        else if (aLevel == 50)
        {
            ...
            TodLoadResources("DelayLoad_ZombieNote");
            TodLoadResources("DelayLoad_ZombieNote5");
        }
        else if (aLevel == 60)
        {
            TodLoadResources("DelayLoad_Background1");
        ...
        else if (aLevel == 70)
        {
            TodLoadResources("DelayLoad_Background1");
            TodLoadResources("DelayLoad_ZombieNote");
            TodLoadResources("DelayLoad_ZombieNote6");
        }
    }
    ...
    else
    {
        if (!mApp->IsAdventureMode())
        {
            if (mApp->EarnedDiamondTrophy())
            {
                ...
                TodDrawImageCenterScaledF(g, Sexy::IMAGE_DIAMOND_SUNFLOWER_TROPHY, 330, 80, 0.7f, 0.7f);
            }
        ...
        {
            g->DrawImage(Sexy::IMAGE_BACKGROUND5, -700, -300, 2800, 1200);
            g->DrawImage(Sexy::IMAGE_ZOMBIE_NOTE, 80, 80);
            g->DrawImage(Sexy::IMAGE_ZOMBIE_NOTE5, 102, 117);
            TodDrawString(g, _S("[FOUND_NOTE]"), BOARD_WIDTH / 2, 70, Sexy::FONT_DWARVENTODCRAFT24, Color(255, 200, 0, 255), DS_ALIGN_CENTER);
        }
        else if (aLevel == 60)
        {
        ...
        {
            DrawBottom(g, _S("[FOUND_GAME]"), _S("[GAME]"), _S("[GAME_DESCRIPTION]"));
            g->DrawImage(Sexy::IMAGE_GAME, BOARD_WIDTH / 2 - Sexy::IMAGE_ALMANAC->mWidth / 2, 160);
        }
        else if (aLevel == 70)
        {
            g->DrawImage(Sexy::IMAGE_WOOD, 0, 0, 1280, 720);
            g->DrawImage(Sexy::IMAGE_WOODS_NOTE, 0, 0);
            //g->DrawImage(Sexy::IMAGE_ZOMBIE_NOTE6, 114, 138);
            TodDrawString(g, _S("[FOUND_NOTE]"), BOARD_WIDTH / 2, 70, Sexy::FONT_DWARVENTODCRAFT24, Color(255, 200, 0, 255), DS_ALIGN_CENTER);
        }
