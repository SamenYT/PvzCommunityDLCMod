...
GameSelector::GameSelector(LawnApp* theApp)
{
	...
	/*mVersusModeButton = MakeNewButton(
		...
		"",
		nullptr,
		Sexy::IMAGE_VERSUS_BUTTON,
		Sexy::IMAGE_VERSUS_BUTTON_HIGHLIGHTED,
		Sexy::IMAGE_VERSUS_BUTTON_HIGHLIGHTED
	);
	mVersusModeButton->Resize(0, 0, 65, 65);
	...
	mModCreditsButton = MakeNewButton(
		...
		"",
		nullptr,
		Sexy::IMAGE_CREDITS_BTN1,
		Sexy::IMAGE_CREDITS_BTN2,
		Sexy::IMAGE_CREDITS_BTN2
	);
	...
	mDisclaimerButton = MakeNewButton(
		...
		"",
		nullptr,
		Sexy::IMAGE_DISCLAIMER_BTN1,
		Sexy::IMAGE_DISCLAIMER_BTN2,
		Sexy::IMAGE_DISCLAIMER_BTN2
	);
	mDisclaimerButton->Resize(35, 5170, Sexy::IMAGE_DISCLAIMER_BTN2->mWidth, Sexy::IMAGE_DISCLAIMER_BTN2->mHeight);
	mDisclaimerButton->mMouseVisible = false;
...
GameSelector::~GameSelector()
{
	...
	if (mVersusModeButton)
		delete mVersusModeButton;
	if (mModCreditsButton)
		delete mModCreditsButton;
	if (mChangeLogsButton)
		delete mChangeLogsButton;
	if (mDisclaimerButton)
		delete mDisclaimerButton;
...
void GameSelector::SyncButtons()
{
	...
	bool aVersusModeOpen = false;

	if (mApp->mPlayerInfo)
	{
		aVersusModeOpen = mApp->mPlayerInfo->mLevel > 64 || mApp->mPlayerInfo->mFinishedAdventure;
	}

	if (mUnlockSelectorCheat)
	{
		aVersusModeOpen = true;
	}
...
void GameSelector::Draw(Graphics* g)
{
	...
	if (mHasTrophy)
	{
		...
		{
			//TodDrawImageScaledF(g, Sexy::IMAGE_DIAMOND_SUNFLOWER_TROPHY, aTransformLeft.mTransX + 10.0f + 370 - 6, aTransformLeft.mTransY + 390.0f + 60, 1, 1);
			TodDrawImageScaledF(g, Sexy::IMAGE_DIAMOND_SUNFLOWER_TROPHY, aTransformLeft.mTransX + 10.0f, aTransformLeft.mTransY + 390.0f, 1, 1);
		}
...
void GameSelector::DrawOverlay(Graphics* g)
{
	...
	if (!mApp->IsIceDemo() && !mShowStartButton)
	{
		...
		int aStage = ClampInt((mLevel - 1) / 10 + 1, 1, 8);  // 大关
		...
		TodDrawImageCelF(g, Sexy::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransAreaX + 486.0f + 4.0f, aTransAreaY + 125.0f - 78.0f, aStage, 0);  // 绘制大关数
		if (aSub < 10)
		{
			TodDrawImageCelF(g, Sexy::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransSubX + 504.0f + 4.0f, aTransSubY + 128.0f - 78.0f, aSub, 0);
		}
		else if (aSub == 10)
		{
			TodDrawImageCelF(g, Sexy::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransSubX + 504.0f + 4.0f, aTransSubY + 128.0f - 78.0f, 1, 0);
			TodDrawImageCelF(g, Sexy::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransSubX + 513.0f + 4.0f, aTransSubY + 129.0f - 78.0f, 0, 0);
		}
...
{
	if (mApp->GetDialog(Dialogs::DIALOG_MESSAGE))
		return;

	if (changeLogs)
	{
		mApp->LawnMessageBox(Dialogs::DIALOG_MESSAGE, _S("[CHANGE_LOGS_TITLE]"), _S("[CHANGE_LOGS_MESSAGE]"), _S("[DIALOG_BUTTON_OK]"), _S(""), Dialog::BUTTONS_FOOTER);
		changeLogs = false;
	}
	...
	if (mHasTrophy)
	{
		...
		if (aMouseX >= 50 && aMouseX < 135 && aMouseY >= 325 && aMouseY <= 550)
		{
			if (mApp->EarnedDiamondTrophy())
			{
				mToolTip->SetLabel(_S("[DIAMOND_SUNFLOWER_TOOLTIP]"));
				mToolTip->mX = 20;
				mToolTip->mY = 495;
...
void GameSelector::Update()
{
	...
	if (mStartingGame)
	{
		...
		if (mStartingGameCounter > 450)
		{
			...
			if (mApp->IsIceDemo())
			{
				mApp->PreNewGame(GameMode::GAMEMODE_CHALLENGE_TEST, false);
				return;
			}
			...
			if (mApp->mPlayerInfo->mNeedsMagicTacoReward && (mLevel == 35 || mLevel == 55))
			{
	...
	case SelectorAnimState::SELECTOR_OPEN:
		...
		if (aSelectorReanim->mLoopCount > 0)
		{
			...
			mVersusModeButton->mBtnNoDraw = false;
			mChangeLogsButton->mBtnNoDraw = false;
			mModCreditsButton->mBtnNoDraw = false;
			...
			mVersusModeButton->mMouseVisible = true;
			mModCreditsButton->mMouseVisible = true;
			mChangeLogsButton->mMouseVisible = true;
			mDisclaimerButton->mMouseVisible = true;
	...
	//TrackButton(mVersusModeButton, "SelectorScreen_BG_Right", 645.0f, 200.0f);
	TrackButton(mChangeLogsButton, "SelectorScreen_BG_Right", 580.0f, -40.0f);
	TrackButton(mModCreditsButton, "SelectorScreen_BG_Right", 600.0f, 510.0f);
...
void GameSelector::AddedToManager(WidgetManager* theWidgetManager)
{
	...
	theWidgetManager->AddWidget(mVersusModeButton);
	theWidgetManager->AddWidget(mModCreditsButton);
	theWidgetManager->AddWidget(mChangeLogsButton);
	theWidgetManager->AddWidget(mDisclaimerButton);
...
void GameSelector::RemovedFromManager(WidgetManager* theWidgetManager)
{
	...
	theWidgetManager->RemoveWidget(mVersusModeButton);
	theWidgetManager->RemoveWidget(mModCreditsButton);
	theWidgetManager->RemoveWidget(mChangeLogsButton);
	theWidgetManager->RemoveWidget(mDisclaimerButton);
...
void GameSelector::OrderInManagerChanged()
{
	...
	mWidgetManager->PutInfront(mVersusModeButton, this);
	mWidgetManager->PutInfront(mModCreditsButton, this);
	mWidgetManager->PutInfront(mChangeLogsButton, this);
	mWidgetManager->PutInfront(mDisclaimerButton, this);
...
void GameSelector::KeyChar(char theChar)
{
	...
	if ((gIsPartnerBuild || mApp->mDebugKeysEnabled) && theChar == 'u' && mApp->mPlayerInfo)
	{
		...
		mApp->mPlayerInfo->mLevel = 69;

		for (int i = 1; i < (int)NUM_GAME_MODES; i++)
		{
				...
				i != (int)GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_ENDLESS && i != (int)GameMode::GAMEMODE_SURVIVAL_ENDLESS_STAGE_3)
			{
				...
				mApp->mPlayerInfo->mChallengeHardmodeRecords[i - 1] = 20;
			}
		}
		SyncProfile(true);
...
void GameSelector::ClickedAdventure()
{
	...
	mVersusModeButton->SetDisabled(true);
	mModCreditsButton->SetDisabled(true);
	mChangeLogsButton->SetDisabled(true);
	mDisclaimerButton->SetDisabled(true);
...
void GameSelector::ButtonDepress(int theId)
{
	...
	case GameSelector::GameSelector_Survival:
		...
		break;
	case GameSelector::GameSelector_Modcredits:
		mApp->KillGameSelector();
		mApp->ShowChallengeScreen(ChallengePage::CHALLENGE_PAGE_CREDITS);
		mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_TITLE_CRAZY_DAVE_MAIN_THEME);
		//mApp->DoNewModOptions(true);
		//changeLogs = true;
		break;
	case GameSelector::GameSelector_Changelogs:
		changeLogs = true;
		//mApp->DoCheatDialog();
		//mApp->mPlayerInfo->mLevel = 59;
		break;
	case GameSelector::GameSelector_Disclaimer:
		mApp->KillGameSelector();
		mApp->ShowChallengeScreen(ChallengePage::CHALLENGE_PAGE_DISCLAIMER);
		mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_TITLE_CRAZY_DAVE_MAIN_THEME);
		break;
	...
	case GameSelector::GameSelector_Versus:
		//mApp->KillGameSelector();
		//mApp->PreNewGame(GameMode::GAMEMODE_CHALLENGE_VERSUS, false);
		mApp->KillGameSelector();
		mApp->ShowChallengeScreen(ChallengePage::CHALLENGE_PAGE_VERSUS);
		break;
	}
}
...
void GameSelector::AddPreviewProfiles()
{
	...
	if (aProfile)
	{
		...
		aProfile->mPurchases[StoreItem::STORE_ITEM_WOOD_CHIPPER] = 1;
	...
	if (aProfile)
	{
		...
		aProfile->mPurchases[StoreItem::STORE_ITEM_PLANT_FIRE_PEASHOOTER] = 1;
		aProfile->mPurchases[StoreItem::STORE_ITEM_PLANT_PEPPERPULT] = 1;
		aProfile->mPurchases[StoreItem::STORE_ITEM_PLANT_CHILLYPEPPER] = 1;
		aProfile->mPurchases[StoreItem::STORE_ITEM_PLANT_PLASMAPEA] = 1;
		aProfile->mPurchases[StoreItem::STORE_ITEM_PLANT_SHOOTINGSTAR] = 1;
