...
void TitleScreen::Draw(Graphics* g)
{
	...
	if (!mStopTheReanimDraw)
	{
		Reanimation* aTitlescreenReanim;
		Reanimation* aDiscordReanim;
		aTitlescreenReanim = mApp->AddReanimation(20, 0, 0, REANIM_TITLESCREEN);
		aTitlescreenReanim->PlayReanim("anim_enter", REANIM_PLAY_ONCE_AND_HOLD, 0, 12);
		aDiscordReanim = mApp->AddReanimation(20, -50, 0, REANIM_TITLESCREEN);
		aDiscordReanim->PlayReanim("anim_discordenter", REANIM_PLAY_ONCE_AND_HOLD, 0, 12);
		mStopTheReanimDraw = true;
	}
	...
	g->DrawImage(IMAGE_NEW_LOGO, mWidth / 2 - IMAGE_NEW_LOGO->mWidth / 2, aLogoY);
	...
	for (int i = 0; i < LENGTH(aTriggerPoint); i++)
	{
		...
		if (aPrevWidth < aTriggerPoint[i] && mCurBarWidth >= aTriggerPoint[i])
		{
			...
			/*if (i == 0)
			{
				Reanimation* aTitlescreenReanim;
				Reanimation* aDiscordReanim;
				aTitlescreenReanim = mApp->AddReanimation(0, 0, 0, REANIM_TITLESCREEN);
				aTitlescreenReanim->PlayReanim("anim_enter", REANIM_PLAY_ONCE_AND_HOLD, 0, 12);

				aDiscordReanim = mApp->AddReanimation(0, -50, 0, REANIM_TITLESCREEN);
				aDiscordReanim->PlayReanim("anim_discordenter", REANIM_PLAY_ONCE_AND_HOLD, 0, 12);
			}*/
			if (i == 1 || i == 3)
			{
