...
void AlmanacDialog::SetupPlant()
{
	...
	if (mSelectedSeed == SEED_TALLNUT || mSelectedSeed == SEED_CHARD_GUARD || mSelectedSeed == SEED_VINENUT)				
													aPosY += 18;
	...
	if (mPlant->mSeedType == SEED_SHRINK) mPlant->PlayBodyReanim("anim_idle_almanac", ReanimLoopType::REANIM_LOOP, 20, 18.0f);
	mPlant->mX = aPosX;
	mPlant->mY = aPosY;
}
...
void AlmanacDialog::SetPage(AlmanacPage thePage)
{
	...
	if (mOpenPage == AlmanacPage::ALMANAC_PAGE_INDEX)
	{
		...
		mZombie = new Zombie();
		mZombie->mBoard = nullptr;
		mZombie->ZombieInitialize(0, ZombieType::ZOMBIE_BASIC, false, nullptr, Zombie::ZOMBIE_WAVE_UI);
...
void AlmanacDialog::Update()
{
	...
	mIndexedPage = ClampInt(mIndexedPage, 0, 2);
...
{
	if (mIndexedPage == 0) g->DrawImage(Sexy::IMAGE_ALMANAC_PLANTBACK, 0, 0);
	else g->DrawImage(Sexy::IMAGE_ALMANAC_PLANTBACK_2, 0, 0);
	...
	if (mSelectedSeed == SeedType::SEED_LILYPAD || mSelectedSeed == SeedType::SEED_TANGLEKELP || mSelectedSeed == SeedType::SEED_PINKSTAR ||
	...
	else
	{
		g->DrawImage(
			...
			mSelectedSeed == SeedType::SEED_FLOWERPOT || mSelectedSeed == SeedType::SEED_WATERPOT || mSelectedSeed == SeedType::SEED_BLOODORANGE ? Sexy::IMAGE_ALMANAC_GROUNDROOF : Sexy::IMAGE_ALMANAC_GROUNDDAY,
			521, 107
		);
	}
	...
	if (mSelectedSeed != SeedType::SEED_IMITATER)
	{
		...
		SexyString aRechargeStr = TodReplaceString(
			...
			_S("{WAIT_TIME_LENGTH}"),
			aPlantDef.mRefreshTime == 750 ? _S("[WAIT_TIME_SHORT]") : aPlantDef.mRefreshTime == 3000 ? _S("[WAIT_TIME_LONG]") : aPlantDef.mRefreshTime == 5000 ? _S("[WAIT_TIME_VERY_LONG]") : _S("[WAIT_TIME_VERT_LONG]")
		);
...
void AlmanacDialog::DrawZombies(Graphics* g)
{
	...
	else if (mIndexedPage == 2) {
		g->DrawImage(Sexy::IMAGE_ALMANAC_ZOMBIEBACK, 0, 0);
	}
	else if (mIndexedPage == 3) {
		g->DrawImage(Sexy::IMAGE_ALMANAC_ZOMBIEBACK, 0, 0);
	}
	g->DrawImage(Sexy::IMAGE_ALMANAC_ZOMBIEBACK, 0, 0);
	...
	for (int i = 0; i < NUM_ALMANAC_ZOMBIES; i++)
	{
		...
		{
			if (!ZombieIsShown(aZombieType) || !ZombieHasDescription(aZombieType))
			...
			else
			{
				...
				if (aZombieType == ZombieType::ZOMBIE_GLADIANTUAR_GIGA)
				{
					aZombieGraphics.mScaleX = 0.8f;
					aZombieGraphics.mScaleY = 0.8f;
				}
				else if (aZombieType == ZombieType::ZOMBIE_LEAFBLOWER)
				{
					aZombieGraphics.mScaleX = 0.6f;
					aZombieGraphics.mScaleY = 0.6f;
				}
				else
				{
				...
				case ZombieType::ZOMBIE_BUCKETHEAD:			aZombieGraphics.TranslateF(0, 9);		break;
				case ZombieType::ZOMBIE_FOOTBALL:
				case ZombieType::ZOMBIE_GIGA_FOOTBALL:	aZombieGraphics.TranslateF(-15, -1);	break;
				case ZombieType::ZOMBIE_ZOMBONI:		aZombieGraphics.TranslateF(0, 3);		break;
				...
				case ZombieType::ZOMBIE_GLADIANTUAR_GIGA:aZombieGraphics.TranslateF(15, 17);	break;
				case ZombieType::ZOMBIE_LEAFBLOWER:     aZombieGraphics.TranslateF(-10, -10);	break;
				...
				case ZombieType::ZOMBIE_HUNTER:			aZombieGraphics.TranslateF(0, -2);		break;
				case ZombieType::ZOMBIE_BIGFOOT:		aZombieGraphics.TranslateF(0, 4);		break;
				...
				case ZombieType::ZOMBIE_RAVEN:			aZombieGraphics.TranslateF(0, 15);		break;
				}
	...
	g->DrawImage(mZombie->mZombieType == ZombieType::ZOMBIE_ZOMBONI || mZombie->mZombieType == ZombieType::ZOMBIE_BOBSLED ?
	...
	if (mZombie && !ZombieHasSilhouette(mZombie->mZombieType))
	{
		...
		{
		case ZombieType::ZOMBIE_ZOMBONI:				aZombieGraphics.TranslateF(0, 5);		break;
		case ZombieType::ZOMBIE_GARGANTUAR:				aZombieGraphics.TranslateF(0, 20);		break;
		...
		case ZombieType::ZOMBIE_REDEYE_GARGANTUAR:		aZombieGraphics.TranslateF(0, 20);		break;
		case ZombieType::ZOMBIE_GLADIANTUAR:			aZombieGraphics.TranslateF(10, 30);		break;
		case ZombieType::ZOMBIE_BOSS:					aZombieGraphics.TranslateF(-540, -175);	break;
		case ZombieType::ZOMBIE_GLADIANTUAR_GIGA:		aZombieGraphics.TranslateF(75, 165);	break;
		case ZombieType::ZOMBIE_SNOWPEA_BOSS:		    aZombieGraphics.TranslateF(25, 100);	break;
		}
		if (mZombie->mZombieType != ZombieType::ZOMBIE_BUNGEE && mZombie->mZombieType != ZombieType::ZOMBIE_BOSS &&
			mZombie->mZombieType != ZombieType::ZOMBIE_ZOMBONI && mZombie->mZombieType != ZombieType::ZOMBIE_CATAPULT)
			mZombie->DrawShadow(&aZombieGraphics);
		if (mZombie->mZombieType == ZOMBIE_GLADIANTUAR_GIGA) aZombieGraphics.SetClipRect(-115, -250, 197, 300);
		else if (mZombie->mZombieType == ZOMBIE_SNOWPEA_BOSS) aZombieGraphics.SetClipRect(-140, -250, 300, 300);
		mZombie->Draw(&aZombieGraphics);
	}
	...
	SexyString aName;
	if ((!ZombieIsShown(mZombie->mZombieType) || !ZombieHasDescription(mSelectedZombie)) && mZombie) aName = "???";
	else aName = ZombieHasSilhouette(mSelectedZombie) ? _S("???") : StrFormat(_S("[%s]"), aZombieDef.mZombieName);
	...
	else
	{
		...
		if (!ZombieIsShown(mZombie->mZombieType) || !ZombieHasDescription(mSelectedZombie))
			g->DrawImage(Sexy::IMAGE_ALMANAC_ZOMBIEBLANK, 505, 120, 225, 200);
	}
	for (TodStringListFormat& aFormat : gLawnStringFormats)
	{
...
void AlmanacDialog::GetZombiePosition(ZombieType theZombieType, int& x, int& y)
{
    if (theZombieType == ZombieType::ZOMBIE_BOSS)
		...
		x = 85 * 2 + 22, y = 80 * 0 + 86;
	else if (theZombieType == ZombieType::ZOMBIE_TARGET)
		x = 85 * 3 + 22, y = 80 * 0 + 86;
	else if (theZombieType == ZombieType::ZOMBIE_TRASHCAN)
		x = 85 * 4 + 22, y = 80 * 0 + 86;
	else if (theZombieType == ZombieType::ZOMBIE_GLITCH)
		x = 85 * 0 + 22, y = 80 * 1 + 86;
	else
	{
		int numberOfZombies;
		if (mIndexedPage == 2) numberOfZombies = 32;
		else numberOfZombies = 30;
		int aFinalZombeType = mIndexedPage == 0 ? theZombieType : theZombieType - numberOfZombies;
		x = aFinalZombeType % 5 * 85 + 22;
		y = aFinalZombeType / 5 * 80 + 86;
	}

	/*	for (int i = 0; i <= (ZOMBIE_EPEA_HEAD - ZOMBIE_PEA_HEAD); i++)
	{
		if (theZombieType == ZombieType::ZOMBIE_PEA_HEAD + i)
			x = 85 * (i % 5) + 22, y = 80 * (i / 5) + 86;
	}*/
}
