	...
	Dialog(nullptr, nullptr, theId, isModal, theDialogHeader, theDialogLines, _S(""), BUTTONS_NONE)
{
    ...
    else if (theButtonMode == 3)
    {
        ...
        mRightButton = MakeNewButton(LawnDialog::LawnDialog_Arrow, this, _S(""), nullptr, Sexy::IMAGE_STONEBUTTON_RIGHT, IMAGE_STONEBUTTON_RIGHT_HOVER, IMAGE_STONEBUTTON_RIGHT_HOVER);
        mRightButton->Resize(490, 498, mRightButton->mWidth, mRightButton->mHeight);

        mLeftButton = MakeNewButton(LawnDialog::LawnDialog_Arrow, this, _S(""), nullptr, Sexy::IMAGE_STONEBUTTON_LEFT, IMAGE_STONEBUTTON_LEFT_HOVER, IMAGE_STONEBUTTON_LEFT_HOVER);
        mLeftButton->Resize(30, 498, mRightButton->mWidth, mRightButton->mHeight);
    }
    else
    {
...
void LawnDialog::CalcSize(int theExtraX, int theExtraY)
{
    ...
    int correctionChangelog = 0;
    if (mDialogHeader == TodStringTranslate(_S("[CHANGE_LOGS_TITLE]"))) correctionChangelog = 200;
    Resize(mX, mY, aWidth + correctionChangelog, aHeight); //changelog dialog size
}
...
void LawnDialog::AddedToManager(WidgetManager* theWidgetManager)
{
    ...
    if (mRightButton) AddWidget(mRightButton);
    if (mLeftButton) AddWidget(mLeftButton);
}
...
void LawnDialog::RemovedFromManager(WidgetManager* theWidgetManager)
{
    ...
    if (mRightButton) RemoveWidget(mRightButton);
    if (mLeftButton) RemoveWidget(mLeftButton);
...
{
    if (theId == LawnDialog::LawnDialog_Arrow)
    {
        if (mDialogLines == TodStringTranslate(_S("[CHANGE_LOGS_MESSAGE]")))
            mDialogLines = TodStringTranslate(_S("[CHANGE_LOGS_MESSAGE_2]"));
        else if (mDialogLines == TodStringTranslate(_S("[CHANGE_LOGS_MESSAGE_2]")))
            mDialogLines = TodStringTranslate(_S("[CHANGE_LOGS_MESSAGE]"));
    }
    else if (mUpdateCnt > mButtonDelay)
    {
...
void LawnDialog::Resize(int theX, int theY, int theWidth, int theHeight)
{
    ...
    else if (mLawnYesButton)
    {
        ...
        int correctionChangelog = 0;
        if (mDialogHeader == TodStringTranslate(_S("[CHANGE_LOGS_TITLE]"))) correctionChangelog = 200;
        int aButtonWidth = aButtonMinWidth + aButtonExtraWidth - correctionChangelog;

        correctionChangelog = 0;
        if (mDialogHeader == TodStringTranslate(_S("[CHANGE_LOGS_TITLE]"))) correctionChangelog = 5;
        mLawnYesButton->Resize(aButtonAreaX + (aButtonAreaWidth - aButtonWidth) / 2 + correctionChangelog, aButtonAreaY, aButtonWidth, IMAGE_BUTTON_LEFT->mHeight);
    }
...
void LawnDialog::Draw(Graphics* g)
{
    ...
    if (mDialogHeader.size() > 0)
    {
        ...
        if (mDialogHeader == TodStringTranslate(_S("[CHANGE_LOGS_TITLE]")))g->mTransX -= 15;
    ...
    if (mDialogHeader == TodStringTranslate(_S("[CHANGE_LOGS_TITLE]"))) aRect.mX -= 15;
    ...
    Dialog::BUTTONS_FOOTER)
{
    ...
    gLawnApp->mBoard->spawnSpeed = false;
