...
static StoreItem gStoreItemSpots[NUM_STORE_PAGES][MAX_PAGE_SPOTS] =
{
    ...
    { STORE_ITEM_PLANT_SPIKEROCK,       STORE_ITEM_PLANT_GOLD_MAGNET,   STORE_ITEM_PLANT_WINTERMELON,   STORE_ITEM_PLANT_COBCANNON,
      STORE_ITEM_PLANT_IMITATER,        STORE_ITEM_FIRSTAID,            STORE_ITEM_WOOD_CHIPPER,        STORE_ITEM_INVALID },
    ...
    { STORE_ITEM_PLANT_FIRE_PEASHOOTER, STORE_ITEM_PLANT_PICKLEDPEPPER, STORE_ITEM_PLANT_PEPPERPULT,    STORE_ITEM_PLANT_FIRESHROOM,
      STORE_ITEM_PLANT_WATERPOT,        STORE_ITEM_PLANT_SEASTAR,       STORE_ITEM_PLANT_CHILLYPEPPER,  STORE_ITEM_PLANT_EPEA }
};
...
StoreScreen::StoreScreen(LawnApp* theApp) : Dialog(nullptr, nullptr, DIALOG_STORE, true, _S("Store"), _S(""), _S(""), BUTTONS_NONE)
{
    ...
    mNewPrevButton = new GameButton(StoreScreen::StoreScreen_Prev);
    mNewPrevButton->mButtonImage = IMAGE_STORE_PREVBUTTON;
    mNewPrevButton->mOverImage = IMAGE_STORE_PREVBUTTONHIGHLIGHT;
    mNewPrevButton->Resize(252, 402, IMAGE_STORE_PREVBUTTON->mWidth, IMAGE_STORE_PREVBUTTON->mHeight);
    mNewPrevButton->mParentWidget = this;
...
bool StoreScreen::IsComingSoon(StoreItem theStoreItem)
{
    ...
    else if (theStoreItem == STORE_ITEM_PLANT_PLASMAPEA)
        return !mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_FIRE_PEASHOOTER] > 0;
    else if (theStoreItem == STORE_ITEM_PLANT_SHOOTINGSTAR)
        return !mApp->EarnedDiamondTrophy();
    return false;

    /*if (theStoreItem == STORE_ITEM_PLANT_PLASMAPEA)
  {
     // mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_FIRE_PEASHOOTER] > 0;
      return !mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_FIRE_PEASHOOTER] > 0;
  }*/
}
...
bool StoreScreen::IsItemUnavailable(StoreItem theStoreItem)
{
    ...
    */
    ...
    if (theStoreItem == STORE_ITEM_WOOD_CHIPPER)
    {
        return mApp->IsTrialStageLocked() || (!mApp->HasFinishedAdventure() && mApp->mPlayerInfo->GetLevel() < 61);
    }
    ...
    if (theStoreItem == STORE_ITEM_PLANT_ICYFUME)
    {
        return !mApp->HasFinishedAdventure() && mApp->mPlayerInfo->GetLevel() < 51;
    }
    if (theStoreItem == STORE_ITEM_PLANT_SUPERCHOMP)
    {
        return !mApp->HasFinishedAdventure() && mApp->mPlayerInfo->GetLevel() < 51;
    }
    if (theStoreItem == STORE_ITEM_PLANT_FLAMEWOOD)
    {
        return !mApp->HasFinishedAdventure() && mApp->mPlayerInfo->GetLevel() < 55;
    }
    if (theStoreItem == STORE_ITEM_PLANT_ICEQUEEN)
    {
        return !mApp->HasFinishedAdventure() && mApp->mPlayerInfo->GetLevel() < 55;
    }
    if (theStoreItem == STORE_ITEM_PLANT_VINENUT)
    {
        return !mApp->HasFinishedAdventure() && mApp->mPlayerInfo->GetLevel() < 60;
    }
    if (theStoreItem == STORE_ITEM_PLANT_THORNMOSS)
    {
        return !mApp->HasFinishedAdventure() && mApp->mPlayerInfo->GetLevel() < 60;
    }
    if (theStoreItem == STORE_ITEM_PLANT_BRAVESHROOM)
    {
        return !mApp->HasFinishedAdventure() && mApp->mPlayerInfo->GetLevel() < 65;
    }
    if (theStoreItem == STORE_ITEM_PLANT_ECACTUS)
    {
        return !mApp->HasFinishedAdventure() && mApp->mPlayerInfo->GetLevel() < 65;
    }
...
void StoreScreen::DrawItemIcon(Graphics* g, int theItemPosition, StoreItem theItemType, bool theIsForHighlight)
{
    ...
    else if (theItemType == STORE_ITEM_WOOD_CHIPPER)
    {
        g->DrawImage(Sexy::IMAGE_ICON_WOODCHIPPER, aPosX - 15, aPosY + 8);
    }
...
void StoreScreen::Draw(Graphics* g)
{
    ...
    if (!mHatchTimer && mHatchOpen)
    {
        ...
        if (mApp->IsNight())
        {
            g->DrawImage(Sexy::IMAGE_STORE_CAR_NIGHT, mShakeX + 664, mShakeY + 193);
        }
    }
    else
    {
    ...
    if (!mPrevButton->mDisabled)
    {
        ...
        TodDrawString(g, aPageString, STORESCREEN_PAGESTRING_X, STORESCREEN_COINBANK_Y - 60, Sexy::FONT_BRIANNETOD12, Color(80, 80, 80), DS_ALIGN_CENTER);
    }
}
...
void StoreScreen::UpdateMouse()
{
    ...
    for (int aItemPos = 0; aItemPos < MAX_PAGE_SPOTS; aItemPos++)
    {
        ...
        if (aItemType != STORE_ITEM_INVALID && !IsItemUnavailable(aItemType))
        {
            ...
            if (Rect(aItemX, aItemY, 50, 87).Contains(aMouseX, aMouseY))
            {
                ...
                case STORE_ITEM_PLANT_ICYFUME:          aMessageIndex = 2035;                           break;
                case STORE_ITEM_PLANT_SUPERCHOMP:       aMessageIndex = 2036;                           break;
                case STORE_ITEM_PLANT_FLAMEWOOD:        aMessageIndex = 2037;                           break;
                case STORE_ITEM_PLANT_ICEQUEEN:         aMessageIndex = 2038;                           break;
                case STORE_ITEM_PLANT_VINENUT:          aMessageIndex = 2039;                           break;
                case STORE_ITEM_PLANT_THORNMOSS:        aMessageIndex = 2040;                           break;
                case STORE_ITEM_PLANT_BRAVESHROOM:      aMessageIndex = 2041;                           break;
                case STORE_ITEM_PLANT_ECACTUS:          aMessageIndex = 2042;                           break;
                ...
                case STORE_ITEM_WOOD_CHIPPER:           aMessageIndex = 2050;                           break;
    ...
    mApp->SetCursor(mBackButton->mIsOver || mNewPrevButton->mIsOver || mPrevButton->mIsOver || mNextButton->mIsOver || aShowFinger ? CURSOR_HAND : CURSOR_POINTER);
}
...
bool StoreScreen::IsPageShown(StorePages thePage)
{
    ...
    // 冒险模式未完成时，不显示智慧树工具页
    if (thePage == thePage == STORE_PAGE_PLANT_UPGRADES2) return mApp->mPlayerInfo->mLevel >= 52;
    if (thePage == thePage == STORE_PAGE_PLANT_PURCHASES) return mApp->mPlayerInfo->mLevel >= 52;
    ...
    else if (theId == StoreScreen::StoreScreen_Prev || theId == StoreScreen::StoreScreen_Next)
    {
        ...
        do
        {
            if (theId == StoreScreen::StoreScreen_Prev)
            {
                ...
                {
                    mPage = STORE_PAGE_PLANT_PURCHASES;
                }
            }
            else
            {
...
int StoreScreen::GetItemCost(StoreItem theStoreItem)
{
    ...
    case STORE_ITEM_PLANT_ICYFUME:                      return 1000;
    case STORE_ITEM_PLANT_SUPERCHOMP:                   return 300;
    case STORE_ITEM_PLANT_FLAMEWOOD:                    return 1500;
    case STORE_ITEM_PLANT_ICEQUEEN:                     return 750;
    case STORE_ITEM_PLANT_VINENUT:                      return 300;
    case STORE_ITEM_PLANT_THORNMOSS:                    return 500;
    case STORE_ITEM_PLANT_BRAVESHROOM:                  return 500;
    case STORE_ITEM_PLANT_ECACTUS:                      return 1000;
    ...
    case STORE_ITEM_WOOD_CHIPPER:                       return 300;
